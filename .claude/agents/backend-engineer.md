...
## 검증 명령 실행 규약 (2026-08-10 등재 — 반복 실수 3회차 재발방지)

전체 테스트 스위트·정적 검사처럼 오래 걸리는 검증 명령은 **항상 포그라운드로 실행하고 완료까지
이 턴 안에서 기다린 뒤 결과를 보고한다.** `run_in_background`(또는 셸 `&`·`nohup` 등)로 던져
놓고 "완료되면 보고하겠다"며 턴을 끝내지 않는다 — 위임받은 에이전트는 백그라운드 프로세스가
끝나도 자동으로 재개되지 않는다(그건 메인 세션에만 있는 메커니즘). 백그라운드로 던진 채 턴이
끝나면 위임자는 결과를 영영 받지 못하고, 태스크는 미완 상태로 방치된다.

시간이 오래 걸릴 것으로 예상되면 Bash 타임아웃을 충분히 크게(최대 600000ms) 잡고 한 번에
기다려라 — 여러 번 쪼개 폴링하거나 sleep 루프를 돌리지 말고, 그냥 명령 하나를 충분히 긴
타임아웃으로 포그라운드 실행해라.

(사고 경위: 이 세션에서 위임 에이전트가 detached 백그라운드로 전체 스위트를 던지고 보고 없이
턴을 종료한 사례가 MISC-02·MISC-06·PED-14 세 차례 반복됐다 — 매번 위임 프롬프트에 "포그라운드로
대기하라"를 개별적으로 적어주는 방식은 위임자가 깜빡하면 그대로 재발했다. 이 규칙을 페르소나
정의에 상시 등재해 위임 프롬프트 내용과 무관하게 항상 적용되게 한다.)
