...
- ✅ **낙관적 동시성 — ETag + If-Match** 완료 (메인 직접, 2026-05-29) — PATCH last-write-wins 동시편집 덮어쓰기를 HTTP 표준(RFC 7232)으로 차단. **메커니즘 결정: `updated_at` 대신 content-hash 강한 ETag** — 조사서 `Problem.updated_at`엔 `onupdate` 없고(수정 시 안 바뀜) `from_schema`가 timestamp를 구값/None으로 실어 타임스탬프 ETag는 마이그레이션+bump 처리가 얽힘 → ETag = `sha256(to_schema().model_dump_json())[:16]`로 두면 **DB/스키마/마이그레이션 변경 0**, 내용 바뀌면 ETag 바뀜(클라는 불투명 토큰). 신규 `api/_concurrency.py`(`etag_for`·`ensure_if_match`[None=무조건진행·`*`=매칭·불일치=412]). concept·problem 라우터: GET단건·POST·PATCH 응답에 `ETag` 헤더, PATCH·DELETE에 `If-Match` 헤더(불일치 412). **비파괴**: If-Match 미전송 시 기존대로 무조건 진행(기존 테스트·클라 호환). **테스트**: hermetic +11(TestConcurrency 양 도메인 — ETag 노출·If-Match 일치 200/불일치 412/`*`/미전송·DELETE 412) + 통합 +1(`@integration` — GET ETag→일치 PATCH 200·ETag 갱신→옛 ETag 재PATCH **412**·내용 미변경 확인). **메인 실검증**: 로컬 PG16서 API 통합 7종 passed(412 동시수정 차단 실동작). **4게이트 green**: black(61)·ruff·mypy-strict(57)·pytest **964 passed**(953→+11)·20 skip(+1 통합)·`_concurrency`·`api/{concepts,problems}` cov **100%**·TOTAL 99%. **후속**: 강제모드(428 Precondition Required)·GET 캐싱(If-None-Match/304)·전용 version 정수·user 도메인(인증 선행).
