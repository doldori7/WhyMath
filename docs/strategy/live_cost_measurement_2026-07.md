...
## 부록: 2차 실측 — 게이트 ② 재판정 (2026-07-16 · `ops/cost_probe` 대표 트래픽)

**경위**: S1-13 라우터 보정(74/358) 후 대표 트래픽 재측정. 1·2차 시도가 "0건"으로 실패하며 측정
인프라 결함 3건이 실측으로 드러나 순차 수정됐다 — ① `problem_corpus_accumulate`는 파이프라인
우회라 `l3_routing` 이벤트 미발생(→ **`ops/cost_probe` 신설**: 대표 요청 믹스를 `pipeline.generate`로
구동·로컬 비율 인프로세스 판정) ② Langfuse 키가 자리표시자(`pk-lf-…`)로 등록돼 있었음(실키 재등록)
③ **sink가 langfuse v2(2.60.10)에서 쓰기 전멸** — `create_event`는 v3/v4 전용 표면(SDK 버전 적응
수정 `6a882b0`). 상세: MEMORY 2026-07-16 결정 로그 3건.

**측정**: `cost_probe --rounds 3` (라운드당 로컬형 9:클라우드형 1 — free-우세 페르소나 A 트래픽
모델·티어는 라우터 결정). probe_g4 = 30/30 성공·기록 실패 0건.

### 결과표 (cost_report_g4 · fill_live_cost_table 산출 그대로)

| cost_tier | 호출 수 | cost_krw (합/평균) | input/output tokens (평균) | 비고 |
|---|---|---|---|---|
| LOCAL | 23 | 0.0000 / 0.0000 | 59 / 131 | |
| CLOUD_MID | 2 | 10.0069 / 5.0035 | 83 / 200 | |
| CLOUD_HIGH | — | — | — | 표본 0(미측정) |
| **로컬:클라우드 비율** | | | | **PASS (로컬 92.0% vs ≥80%)** |

| 경로 | p50 latency_ms | p90 latency_ms |
|---|---|---|
| LOCAL (Ollama) | 1015 | 5169 |
| CLOUD_MID | 4571 | 4901 |

- 프로브 인프로세스 판정(라우터 결정 30건 전수): **local 27 : cloud_mid 3 = 90.0% PASS**
- Langfuse 재확인(25이벤트 — 30건 중 5건 인그레션 지연·실패 아님): **local 23 : cloud 2 = 92.0%**
- 전역 분포: input p50 62 · output p50 124 · cost 합 10.0069원 · 캐시 적중 0/25(프로브가 미스 강제 — 의도)

### 판정 (2026-07-16)
- [x] **게이트 ② PASS** — 대표 믹스 실측 로컬 90.0%(인프로세스)·92.0%(Langfuse) ≥ 80%, 측정 인프라
  정상 작동 실증(기록 실패 0·실분포 집계). Kiki 통과 조건("키를 정확하게 올리고, 앞으로 정상적으로
  작업하는 것이 보장될 때 통과") 충족 → **S1 탈출 3종 완성**.
- 대표성 캐비엇(정직): 믹스는 문서화된 트래픽 모델(free-우세 9:1)에서 라우터가 결정한 결과다 —
  실학생 트래픽 혼합비 확보 시 재검증(비차단).

### 차기 튜닝 입력 (선택·비차단)
- `_EST_ASSUMED_INPUT_TOKENS` 74 → **62** · `_EST_ASSUMED_OUTPUT_TOKENS` 358 → **124** (대표 믹스 p50 —
  측정 스모크 섞인 1차(74/358)보다 대표성 높음. 재보정은 별도 슬라이스.)
