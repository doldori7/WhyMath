...
1. **[높음] 검증 게이트 미강제 (G3)** — stateless `/v1/coach` 경로는 verify 실패해도 코칭을 반환한다. "학생 응답은 검증 통과 후에만"(CLAUDE.md 금기)이 **coach 경로에 미강제**. 강제는 WH-1 하네스 `_exec_end_turn`("verify 없는 finalize 거부", `wh1_loop.py:20-21`)에만 존재 → **S1-b에서 coach를 하네스 경유로 전환**해야 게이트가 자동 적용.
   - **✅ 상환 완료(2026-07-08·게이트 ③ 코드 증명)**: 재조사로 확정 — coach가 학생에게 방출/저장하는 건 **전부 정적 결정론 템플릿**(LLM 발화 0)이라 미검증 LLM 발화가 학생에게 닿을 표면 자체가 없다. 정답(`expected_answer`)은 shadow sink(`observe_step_breaks`)로만 흐르고 응답 payload·`DialogueTurn.content`엔 미노출. 이 안전 불변식을 4각도 거버넌스/계약 테스트로 **회귀 차단 봉인**: `tests/backend/harness/test_gate3_student_verification_governance.py`(A `.coach(` allowlist frozen·api/* 하드 가드 / B coach 미검증 verdict 정답 미노출 hermetic 실증 / C 하네스 verify-before-finalize·정답 억제 재봉인 / D `/v1/generate` 비학생 계약). **coach→하네스 전면 수렴은 별도 미래 작업**(LLM 주도 코칭 도입 시)이며 게이트 ③의 블로커 아님 — (A) allowlist가 그 미래 배선을 red로 강제 심사하므로, 수렴 전엔 "LLM 발화 표면 부재"로·이후엔 "하네스 검증 경유 강제"로 게이트 ③이 자연 승계된다.
