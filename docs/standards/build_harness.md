...
7. `DISJUNCTIVE`·`STAGE_BLOCKED`인데 태스크가 `blocked`가 아니면 위반 — 아래 "차단 강제"
...
### 차단 강제 — 분류만 하고 막지 않으면 경고만 없앤 것이다

`DISJUNCTIVE`·`STAGE_BLOCKED`는 "`depends_on`으로는 순서를 강제할 수 없다"는 뜻이다. 그러면
스케줄러 제외를 **다른 수단**이 담당해야 한다 — 이 저장소에서는 `status=blocked`다. 그래서 두
코드는 태스크가 `blocked`일 것을 계약으로 요구한다. 실측 배경: `EOS-50`이 택일 선행 둘 다
미완인데 `todo`라서 착수 후보 111건에 들어 있었다(분류가 유일한 경고를 없앤 상태).
