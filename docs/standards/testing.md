...
게이트 대상·바닥선의 **정본은 스크립트의 `LAYER_FLOORS`**이며, 값은 실측 baseline 기준
ratchet(회귀 방지)으로 아래 지향치까지 점진 상향한다:

| 서브패키지 | 지향치 | 강제 |
|---|---|---|
| 도메인 로직 `l4` | 90% | ✅ 게이트(ratchet floor → 지향치 점진) |
| 데이터 `l1` | 80% | ✅ 게이트(ratchet floor) |
| ML 모델 `l2` | 80% | ✅ 게이트(ratchet floor) |
| LLM 통합 `l3` | 70% (모킹) | ✅ 게이트(ratchet floor) |
| API `api` (L5) | 80% | ✅ 게이트(ratchet floor) |
| Flutter UI | 60% (위젯·골든) | ✅ 게이트(라인 ≥ 60%) |

`l5`(상호작용·OCR/Manim 등 외부 의존)·`l6`·횡단(db·schema·ops·privacy·whs)은 계층 게이트를
부여하지 않고 집계 70%로 커버한다(외부 의존 과다 또는 미선언). 바닥선을 지향치까지 끌어올리는
것은 계층별 테스트 보강과 함께 점진 진행한다(현재 floor는 스크립트 참조).
