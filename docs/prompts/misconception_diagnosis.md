...
| `root-loss-by-dividing` | A4 | `x²=2x … x=2` (x=0 누락) | `\A(?!.*x=0).*x2=(?P<b>\d+)x.*x=(?P=b)(?!\d)` | 앞머리 부정 전방탐색이 **"x=0을 끝내 적지 않았다"**를 본다 — 정답은 반드시 x=0을 적으므로 미매치 |
...
(Wilson 상한 0/20=0.1192로 완벽한 채널도 FAIL) 도달 가능성 자체를 매 실행 재검사한다.
