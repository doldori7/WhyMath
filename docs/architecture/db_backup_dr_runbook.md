...
| 보존 | 14일(`-RetentionDays`), 최신 1개는 무조건 보존. 오프사이트 폴더에도 같은 정책(§4-1c) — 클라우드 전파는 삭제 전파 프로브로 1회 실측·휴지통 30일이 파기 창에 가산 |
...
- **오프사이트 삭제 전파 미확인 (잔존 · 재확인 지점 명시)**: §4-1c 삭제 전파 프로브의 *로컬* 단계(생성 7B → 삭제 → `Test-Path False`)는 2026-09-07 완료됐으나, **웹 측 관측(보였다가 사라짐)이 회신되지 않았다** — 프로브가 업로드되기 전에 삭제됐을 수 있어 현재 결과는 "전파됨"과 "업로드 안 됨"을 구별하지 못한다(변별력 0). 그때까지 §4-3의 파기 창 선언은 **로컬 기준으로만** 성립한다. 재확인 지점: Kiki의 프로브 웹 관측 회신 즉시, 늦어도 **다음 분기 리허설(§4-1d)** — 그 시점까지 미확인이면 만료 사본을 웹에서 수동 삭제하는 절차를 §4-3에 적고 "수동 관리 중"으로 표기한다(만료 없는 유예 금지).
