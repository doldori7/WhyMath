...
### 6. 학습 경로 (Learning Path) — 스코프 한정 (2026-07-30 명시)

L2가 소유하는 학습 경로는 **후방 결손 복구 경로**다 — *이미 막힌 선수개념 집합 내부*의 복습
순서를 위상정렬로 산출한다(`l2/learning_path.py` · Kahn · 근본→말단 · LTHC).

**"다음에 배울 개념 순서"(전방 진도 경로)는 L2 소관이 아니다.** 그것은 L1+L6 자동 커리큘럼
정렬의 차원 1(활성 개념 풀)·6(인접 개념 순서 — 교과서 단원 배열)이며 미구현·데이터 미비다
(`06_application_modes.md` §차원 1·6). 두 축을 한 이름으로 다루면 "학습 경로는 구현됐다"는
오인이 생기므로 경계를 여기에 못박는다.

계약:
- **경로는 상태가 아니라 파생**이다. 요청 시 진단으로부터 계산하며 영속 사본을 두지 않는다 —
  `assessment.recommended_path`(JSONB) 슬롯은 소생하지 않는다(진단과 어긋나는 순간 이중 진실
  원천). 파생 즉시성이 곧 "적응형 경로"의 실시간 재구성 요건을 충족한다.
- **탐색 범위는 막힌 집합 내부로 한정**한다. 전역 그래프 순회(현재→목표 최단경로)는 노드·관계
  폭발 방어와 Minimal Reasoning Subgraph 예산(depth ≤ 2 · max_nodes ≤ 12~20)을 어긴다.
- **선수 관계의 진실 원천은 개념그래프 엣지 하나**다. `curriculum_entry.prerequisite_concept_ids`
  를 traversal에 동시 소비하지 않는다(이중 진실원천).
- **개념 난이도(`intrinsic_difficulty`)는 위상 동등 노드 사이의 tiebreak로만** 쓴다 — 위상 제약을
  역전시키지 않으며, 미주입(NULL) 시 기존 순서와 동일해야 한다(하위호환·데이터 0 안전).
- **사이클·예산 절단은 정직하게 노출**한다. 위상정렬이 방출하지 못한 잔여는 조용히 버리지 않고
  `is_cycle_residual`·`has_cycle`로 표시한다(현행 구현이 이미 준수).

상세 대조·불채택 근거: `learning_path_module_gap_review.md`(외부 EOS 틀 모듈 54~62 대조).
