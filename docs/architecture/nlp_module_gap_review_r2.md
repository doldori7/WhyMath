...
### D1 (OCR 학생 도달 0회) — **trunk 기준 전면 유효 · 구현분은 미머지**
...
2. **실효 커버리지가 구조적으로 좁다** — `derive_verify_inputs:93-130`은 모든 조건의 `formal`
   ...
   `unverifiable_rate`도 `not_derivable`도 값이 없다.
...
### D3 (단계 분해 계약) — **trunk 기준 불변 · 구현분은 미머지**
...
### D5 — 관측 리포트를 만들고 실행하지 않는다 (신규 태스크 · 횡단)
...
### D6 — 채점 권위 이관 임계의 사전 등록 (문서 정본화 · 태스크 신설 0)
...
**신규 ⑨ — `student_intent` writer는 여전히 0**: `schema/enums.py:934-943`(5종)·
