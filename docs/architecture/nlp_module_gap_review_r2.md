...
### ⑥ Δ3이 중복이었다 — `NLP-06` 범위 축소 + 대장 사각 4번째 (2026-08-11 2차 병합)

**이 절은 이 문서가 자기 실수를 적는 자리다.** §8.11-①이 "Δ2의 최대 발견은 최초 발견이 아니었다"를
기록했는데, 2차 병합에서 **Δ3도 같은 일을 당했다** — 이번엔 결과물이 *중복 태스크*였다.

| 태스크 | 소유 | 축 |
|---|---|---|
| `COLLAB-04-contract-fixture-ci-trigger-axis` (협업 r3 §3 D5 · **main 착륙**) | 타 세션 | `data/*.json` 계약 8종 → **backend·web** 필터 + 재발 차단 장치 |
| `NLP-06-contract-golden-ci-trigger` (§8.3 Δ3 · **이 문서**) | 이 세션 | 최초 등재는 **backend·mobile·web 3축** — 앞 둘이 중복 |

**폐기하지 않고 분업으로 정리한 근거**는 `COLLAB-04` 자신의 acceptance ⑥이다:

> *"다른 잡(**mobile**/data-pipeline)의 필터 확장은 이 태스크 범위 밖"*

즉 **mobile 축은 명시적으로 무주공산**이며, 그 축의 사실(§8.3에서 실측한
`ci.yml:83` mobile 필터에 `data/`가 통째로 없음 → `segmentation_contract_test.dart`가
*계약을 고쳐도* *백엔드 구현을 고쳐도* 안 깨어남)은 여전히 유효하다. 따라서 `NLP-06`을
**mobile 축 전용으로 축소**하고 `depends_on: COLLAB-04`를 걸었다(필터 규약·재발 차단 장치를
그쪽이 먼저 정해야 답습이 가능하다 — 두 필터가 서로 다른 규약을 쓰면 다음 누락이 어느 쪽에서
날지 알 수 없다). **`COLLAB-04`는 수정하지 않았다**(타 세션 소유·main 착륙분).

#### 왜 기계가 못 잡았는가 — 대장 사각 4번째 (`HARN-29` 등재)

`backlog.py overlap NLP-06`은 **"겹침 없음"을 출력했다.** 두 태스크의 `paths`가
`['.github/workflows/ci.yml', 'tests/infra/**']`로 **완전히 동일한데도** 그렇다 —
`cmd_overlap`(`backlog.py:1403-1407`)이 비교 대상을 `status in ("in_progress","review")`로
한정하고 **`todo`를 제외**하기 때문이다. `add --path`의 겹침 검사도 같은 제한을 공유한다
(`NLP-05` 등재 때 `in_progress`인 `S4-16`은 잡혔고, `todo`인 것들은 애초에 보이지 않았다).

"착수 전 확인" 용도로는 의도된 설계다. 그러나 **갭 리뷰 시리즈가 실제로 필요한 것은
"등재 시점 중복 검사"**이고, 그것은 어느 장치도 하지 않는다. 이번 중복을 실제로 잡은 것은
사람이 커밋 로그를 읽은 것이지 기계가 아니다. → **`HARN-29`**.

**이 세션이 실측한 대장 사각 4종** (§8.0은 2종만 알았다):

| # | 사각 | 소유 |
|---|---|---|
| ① | 세션 브리핑의 "미머지 신규 설계 문서" 목록이 이 문서를 미탐 | **원인 규명됨** — `remote_claims.py`의 `_DOC_SERIES_SUFFIX='_review.md'`라 `_r2.md`가 안 잡힌다. **`HARN-25`**(타 세션 인플라이트)가 소유 |
| ② | `next`의 "이미 완료(미머지)" 경고는 *태스크*만 보고 *문서*를 안 본다 | 미소유(§8.0 기록) |
| ③ | `validate`가 `done`+`artifacts`인데 커밋이 main 조상이 아닌 것을 green 통과 | 추천 r3 발견 · **`REC-09`** |
| ④ | `overlap`/`add`가 `todo`끼리의 범위 중복을 안 본다 | **`HARN-29`**(이 문서) |

①의 원인 규명은 이 세션이 `HARN-25`를 발견하면서 얻었다 — **이 문서가 브리핑에 안 뜬 이유가
파일명 접미어였다.** `_review.md`만 보는 탐지기에게 `nlp_module_gap_review_r2.md`는 존재하지
않는 문서였다. §8.0이 "방어 장치가 놓쳤다"고만 적은 자리에 이제 **기계적 원인**이 들어간다.

#### 이 세션의 최종 자기 평가

같은 외부 틀 하나를 대조하는 동안 **병렬 세션이 내 발견의 일부를 먼저 한 것이 3회**다
(Δ2↔`REC-05` · Δ1↔`PED-19` · Δ3↔`COLLAB-04`). 그리고 그 3회를 발견한 수단은 매번
**사람이 커밋 로그·YAML을 읽은 것**이었다. §8.0이 "고립된 판단은 재발견 비용으로 청구된다"고
썼는데, 청구서는 **다른 세션이 아니라 이 세션에게** 왔다. 이 문서의 주장은 이 문서 자신에 의해
세 번 실증됐다 — 그것이 §8이 남길 수 있는 가장 정직한 결론이다.

#### 후기 — 다섯 번째 충돌은 이 절을 쓰는 동안 일어났다

`HARN-29`는 **처음에 `HARN-27`로 등재됐다.** CLI 가드가 `HARN-25`·`HARN-26`을 선점으로 거부하고
`HARN-27`을 제안해 그대로 받았는데, **그 직후** 타 세션
(`claude/multi-session-conflict-resolution-3tpr0u`)이 `HARN-26`·`27`·`28`을 원격 claim으로 가져갔다.
다음 세션 브리핑이 `HARN-27-reap-enforcement-site`를 "착수 금지"로 띄우면서 충돌이 드러났고,
상대가 인플라이트이므로 **이쪽을 개명**했다(선례 `f60642f7` — `HARN-22`→`HARN-25`).

이것은 `HARN-22`가 추적하는 **TOCTOU**의 교과서적 사례다 — 가드는 *관측*만 하고 번호를
**예약하지 않는다**. 등재 시점의 "빈 번호"는 등재 완료 시점에도 비어 있다는 보장이 아니다.

그리고 이 세션의 **다섯 번째** 병렬 충돌이다:

| # | 충돌 | 상대 | 발견 수단 |
|---|---|---|---|
| 1 | 문서 자체가 이미 존재 | PR #697(자기 자신의 7일 전 판) | `--diff-filter=A` 경로 스캔 |
| 2 | Δ2 = `REC-05`가 먼저 발견 | 추천 r2(고립) | 커밋 로그 수동 조사 |
| 3 | Δ1 정밀화 = `PED-19` 소유 | AI 튜터 r2 | 병합 시 태스크 대조 |
| 4 | Δ3 = `COLLAB-04`와 중복 | 협업 r3 | 병합 시 태스크 대조 |
| 5 | `HARN-27` 번호 선점 | 다중세션 충돌해소 세션 | **세션 브리핑 claim 목록**(유일하게 기계가 잡았다) |

**5회 중 기계가 잡은 것은 1회(#5)뿐**이고, 나머지 4회는 전부 사람이 로그·YAML을 읽어서 찾았다.
`HARN-29`가 고치려는 것이 정확히 이 비율이다.

#### 종결 — Δ3은 `COLLAB-04` 착지로 닫혔다 (`NLP-06` cancelled)

머지 직전 재확인에서 **`COLLAB-04`가 done**이고 그 구현이 **자기 acceptance ⑥을 초과해 mobile 축까지
덮은 것**을 확인했다. `ci.yml:102` mobile 필터에 `data/{notation,segmentation,scene,render}_contract.json`
4종이 전부 들어갔고, `:97` 주석이 *"`data/*_contract.json` 4종은 `src/mobile/test/`의 Dart 골든이
`File('../../data/...')`로 읽는다"*고 사유까지 적었다. 재발 차단 장치도
`tests/infra/test_ci_contract_fixture_trigger_wiring.py`로 착지했다.

→ **`NLP-06`은 `cancelled`.** 위 §8.11-⑥에서 "분업"이라 정리했던 것이 실제로는 **상대가 전부 가져간
것**이었다. 이 문서의 Δ3은 **갭 판정으로서는 옳았고 태스크로서는 불필요**했다.

**잔여 축 1건은 살아 있으나 등재하지 않는다**: mobile 필터에 `src/backend/` 경로가 없으므로
**백엔드 구현(`l5/ocr/text_segmentation.py`)만 바뀐 PR에서는 여전히 Dart 미러가 검사되지 않는다.**
`NLP-03` D3의 설계 목적이 *"같은 파일을 읽는 교차 골든"*이었으므로 **백엔드→클라 방향은 아직 절반**
이다. 신설하지 않는 이유는 `COLLAB-04`가 이 축의 소유자로 확립됐고 재발 차단 장치도 그쪽에 있어서
**여기서 만들면 여섯 번째 중복**이 되기 때문이다 — 사실만 여기 남긴다.

이로써 이 세션의 병렬 충돌은 **6회**가 됐고(§8.11-⑥ 표 5회 + 이번 종결),
그중 기계가 잡은 것은 여전히 **1회**(번호 선점)뿐이다.
