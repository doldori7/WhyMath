...
**설계**:
- **국가교육과정 이수구분 참고 데이터 신설**(22행 — 새 코퍼스, 기존 26 코퍼스 패턴). `standards_v1`
  의 `subject` prefix가 `'10'`이면 1학년 과정(공통수학1·2·기본수학1·2·수학)임이 이미 명확하지만,
  `'12'` prefix 18과목은 **2~3학년 풀을 뭉뚱그린다**(실측: 대수·미적Ⅰ·확통·기하와 미적Ⅱ·경제수학·
  인공지능수학·직무수학이 구분 없이 섞여 있음). 국가교육과정의 **일반선택/진로선택/융합선택**
  공식 이수구분(2022 개정 고시 — 공공 사실정보)을 22과목에 1:1로 매핑해 "2학년 과정 참고 편성"
  (일반선택 4과목)과 "3학년 과정 참고 편성"(진로·융합선택 나머지)으로 그룹만 짓는다. **참고 편성
  이지 강제가 아님을 명시** — 학교마다 배정 학년이 다를 수 있다(Kiki 원문 그대로).
...
출력 예시(사용자 제시 형식 그대로):
```
3학년 과정(참고 편성 4과목: 미적분Ⅱ·경제수학·인공지능수학·직무수학) 중 경제수학 진행 중
  → 경제수학 4단원 중 2단원(함수와 경제) 진행 중
    → 그 단원 N소단원 중 M번째
```
