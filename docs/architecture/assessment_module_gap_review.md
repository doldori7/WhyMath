...
> ⚠️ **이 문서는 v1이며 일부가 stale이다 — `assessment_module_gap_review_r2.md`(2026-08-11)가
> 5곳을 정정하고 판정을 갱신한다**(모듈 51 "완전 부재"·D1 "writer 0"·§3 등재요약 2건·§5-⑥
> 발화 조건 오설정). v1 본문은 기록 보존을 위해 재작성하지 않으므로, **판정을 인용하려면 r2를
> 함께 읽어야 한다.**
...
부딪힌다. ~~**현재 이 필드를 읽어 학생 대면 응답에 싣는 코드는 0건**~~(⚠️ **부정확 — D2 §정정 참조**:
값을 *쓰는* 코드가 0건일 뿐, *노출 경로는 이미 완성*돼 있다)이지만, 그 봉인이 왜 필요한지·
...
**왜 지금까지 안 드러났는가**: D1과 같은 이유로 ~~**이 필드들을 읽는 코드 자체가 0건**~~(⚠️ **부정확 —
아래 §정정 참조**: 값을 *쓰는* writer가 0건일 뿐이고 *노출 경로는 이미 배선 완료*다)이라 실제
...
#### D2 §정정 — "노출 경로 0건"은 사실이 아니다 (2026-08-08 실측)

위 본문과 `(횡단)` 절이 공통으로 적은 **"이 필드를 읽어 학생 대면 응답에 싣는 코드가 0건"**은
**부정확하다.** 축을 분리해 실측한 결과는 이렇다.

| 축 | 실측 | 판정 |
|---|---|---|
| **writer**(값 채움) | 5필드에 값을 쓰는 코드 0건. `ASM-03`의 `POST /v1/me/assessments/capture`도 `theta`·`standard_error`·`measurement_sufficient`만 쓰고 예측 5필드는 명시적으로 제외 | ✅ 원 서술 정확 |
| **노출 경로** | `GET /v1/me/assessments`(`api/me.py`)가 `response_model=list[AssessmentSchema]`이고 `schema/assessment.py`의 `AssessmentSchema`가 **5필드를 전부 포함**한다. 인증 스코프는 `ConsentedUser`(=학생 토큰) | ❌ **원 서술 오류** |
| **직렬화** | `response_model_exclude_none`은 저장소 전체 **0건**이고, 라우트 수준 `response_model_exclude`는 `api/users.py`의 PII 제외 2건뿐 — **assessment 라우트에는 어떤 제외 설정도 없다**. 따라서 오늘도 학생에게 가는 JSON 바디에 `"estimated_grade": null`·`"admission_probability": null`이 **키째로 나간다**. OpenAPI 스키마에도 광고된다 | ❌ 이미 새는 중 |
| **테스트 동결** | `tests/backend/api/test_me.py`의 capture 테스트가 5필드 전건에 `assert body["assessment"][field] is None`을 건다 — 딕셔너리 **키 접근**이므로 키가 없으면 `KeyError`다. 즉 이 테스트는 값이 `None`임과 동시에 **키가 응답 바디에 실재함**을 함께 동결한다 (⚠️ 소스 판독 근거. 이 조사 컨테이너에 백엔드 의존성이 없어 **실행 확인은 못 했다** — CI 판정에 위임) | ❌ 추론 아님 |
| **노출 라우트** | `GET /v1/me/assessments` + `POST /v1/me/assessments/capture` **2개** — 후자도 조립된 assessment를 응답에 싣는다 | 봉인 대상 2곳 |
| **클라 소비** | `src/mobile/lib`에서 5필드·`/me/assessments` 소비 0건 | 실피해 아직 0 |

**따라서 현행은 "노출 경로 없음"이 아니라 "노출 대기 — 배관은 완성됐고 값만 비어 있다"이다.**
누군가 writer를 붙이는 순간 **API 층 코드 변경 0으로** 학생에게 흘러간다. 본문이 "언젠가 소비
코드가 붙는 것은 시간 문제"라며 미래 위험으로 적은 것은, 실제로는 **이미 배선이 끝나 값만 기다리는
상태**였다. 이 정정이 아래 §봉인 방향의 답을 바꾼다 — 런타임 필터로는 부족하고 **구조적 배제**가
필요하다.

> 이 정정은 CLAUDE.md의 **"정본화를 집행으로 착각한 완료 선언 금지"**(2026-08-04 · `PED-08` 경위)와
> 같은 계열의 발견이다. 그 항목이 *"계약을 서빙 코드가 실제로 부르는가"*를 물었다면, 여기서는
> *"필드가 실제로 서빙 응답에 실려 나가는가"*를 물었어야 했다. 갭 리뷰가 writer만 grep하고
> `response_model`을 보지 않은 것이 원인이다.

#### D2 §결정 — 노출 정책 (2026-08-08 · 최종 확정은 Kiki)

**권고 = (c) 학생 영구 비노출 · 보호자/교사 노출은 Phase 3에서 별도 결정.**

| 옵션 | 판정 |
|---|---|
| (a) 원시값 노출 | ❌ 의사결정 우선순위 ①(학생 안전·웰빙) 정면 위반. 미성년에게 등급 1~9·합격확률 원시값은 서열화·불안 조장의 전형 |
| (b) 구간·서술로 감싸 노출 | ❌ 성적 축은 완화 가능하나 **합격확률은 서술로 감싸도 본질이 예측**이다. `narrate_calibration_brier` 선례는 "낮을수록 좋음" 오독 방지용이지 예측 자체의 완화 장치가 아니다 |
| **(c) 학생 비노출** | ✅ **권고**. 필드를 보존해 PRD #86(D-100 합격 예측) 제품 약속을 버리지 않으면서 학생 대면만 닫는다. 보호자·교사 노출은 `07_community.md`("비교·랭킹 ❌")·`collaboration_landing_design.md` §4.3(보호자=주 1회 요약·Phase 3)에 걸리므로 **지금 결정하지 않고 Phase 3로 명시 이연** |
| (d) 필드 폐기 | ⚠️ 봉인은 가장 강하나 **PRD 특성 #86을 제품에서 내리는 결정**이라 `ASM-02`의 범위(결정 기록·코드 0)를 넘고 마이그레이션이 필요하다. (c)로 닫고 필요 시 후속 격상 |

**남는 미결(의도적)**
- **보호자·교사 노출 형태는 미결** — Phase 3에서 `collaboration_landing_design.md` §4.3(보호자
  접근 모델 ⓒ/ⓑ)과 함께 결정한다.
- **`AssessmentSchema` 자체의 5필드는 존치** — 내부·영속 계약이며, 분리 대상은 학생 대면 응답
  스키마뿐이다.
- (c) 채택 시 PRD #86은 살아 있으나 **학생 대면 경로가 없다** — 이 기능이 최종적으로 누구에게
  무엇으로 전달되는지는 위 Phase 3 결정에 종속된다.

#### D2 §봉인 방향 — 구조적 배제 + 거버넌스 테스트 2층 (구현은 범위 밖)

`PED-08` acceptance ③의 선례를 승계한다 — *"런타임 필터가 아니라 응답 스키마에 필드 자체를 두지
않는다(필터는 꺼질 수 있으나 부재는 안 꺼진다)"*. §정정이 밝힌 "노출 대기" 상태에서는 화이트리스트
필터만으로는 부족하다.

1. **구조적 배제** — 학생 대면 2개 라우트(`GET /v1/me/assessments`·`POST /v1/me/assessments/capture`)의
   응답 모델을 5필드가 없는 별도 스키마로 분리한다. 현재 새고 있는 `null` 키 노출과 OpenAPI 광고도
   함께 막힌다.
   - ⚠️ **`api/users.py`의 `response_model_exclude=_PII_EXCLUDE` 선례를 복사하지 않는다.** 그것은
     라우트 데코레이터의 *런타임 필터*라 데코레이터 인자 한 줄이 빠지면 조용히 무력화되고, 스키마
     자체에는 필드가 남아 OpenAPI 광고도 계속된다. 예측 5필드는 필터가 아니라 **스키마에서 부재**
     시켜야 한다(PII는 별개 축이므로 `users.py` 현행을 이 태스크에서 바꾸지는 않는다).
2. **거버넌스 테스트** — 학생 대면 라우트의 응답 모델에 5필드명이 재등장하면 red. `ARCH-26`·
   `ARCH-27` 기법(경로 스코프·무력화 하한 assert·양방향 변별력)을 답습한다.

**변별력 증거**: 구조적 배제를 구현하면 위 §정정이 인용한 `test_me.py`의 capture 테스트가
`KeyError`로 **red로 뒤집히는 것이 정상**이다. 그 전환 자체가 봉인이 실제로 물렸다는 증거이므로,
구현 태스크의 acceptance에 "이 테스트의 red 전환 확인 후 기대값 갱신"을 명시한다.

**`ARCH-27`과의 정합**: `ARCH-27`(비교 파생 봉인 게이트) acceptance ⑧이 "assessment 테이블 필드의
존폐는 `ASM-02` 소관"이라 동결해 뒀다. (c) 채택 시 `percentile` 어근을 `ARCH-27` 봉인 목록에
