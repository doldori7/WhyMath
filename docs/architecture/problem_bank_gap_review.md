...
> 22 3/6 방식 보유·계보 미영속. 진짜 갭 9건을 설계(D1~D9)하고 실행 9건을 백로그에 등재했다.
> 의도적 미채택 6건 · 정직한 공백 4건 · 유보(발화 조건 명시) 4건.

관련 정본: `03_content_generation.md`(L3 생성·SolutionPath) · `docs/data/problem_type_graph_v1.md`
...
### D6. 다중 풀이 최소 가동 — 정체성 갭 (백로그 `S4-09`)
...
**설계**: 최소 가동 슬라이스 — 표본 단원 1개(예: 이차방정식 — 인수분해·근의 공식·완전제곱
3접근이 자연스러운 밴드)에서 문제당 ≥2 접근의 검증된 풀이 생성 → 각 풀이가 수용 게이트
(Tier1/2) 통과 → 접근 간 동치성은 `canonicalize`·`symbolic_equivalence` 휴리스틱 + 검수 큐
(동치성 확정은 미해결 연구 난제 — 분류만, `acceptance.py` 스탠스 승계). **소비 슬라이스
동반**(dead data 금지): 학생이 자기 풀이 제출 *후* "다른 접근" 노출(L4 연계·바로 정답 제공
금지 원칙과 정합). WH-S PRM 학습쌍(good/bad)의 접근 다양화 부수 효과.

### D7. SymPy 불가 영역 경로 — 검증 모델의 정직한 확장 (백로그 `S4-10`)
...
### D8. 변형 계보 영속 — problem_relation dead seat 해소 (백로그 `S4-11`)
...
### D9. 실응답 난이도 루프 — 틀의 자동 업데이트 완성 (백로그 `S4-12` · `S3-01` 잠금)
...
| 공백 | 사유 | 해소 시점 |
|---|---|---|
| 실응답 기반 통계 전부(D9의 가동) | 실학생 응답 0 — 파일럿 전 착수는 입력 없는 파이프라인 | `S3-01` 후 (S4-12 잠금 등재) |
...
- 기존 추적 승계(중복 등재 금지 대장): `S4-01-math-k12-complete`(초·중 확장) ·
  `S4-02-proof-learning-support`(학생 증명 피드백) · Phase 3b(Problem↔ProblemType) ·
  Phase 5b(formula_refs) · `S3-01-pilot-cohort`(D9 잠금 게이트)
