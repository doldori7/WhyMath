...
> **⚠️ 2차 대조(R2) 있음 — 이 문서의 §0 수치·일부 판정은 2026-08-03 실측으로 갱신됐다**:
> `problem_bank_gap_review_r2.md`(같은 EOS 틀 기능 18~22 재대조). **이 문서는 이력 보존을
> 위해 본문을 개정하지 않는다** — D1~D9의 *설계*는 여기가 정본이고, D1~D9의 *현재 판정*과
> §0 스냅샷의 정정(코퍼스 6종 2,667→**7종 2,647** · 수용 게이트 4종→**5종** ·
> `schema/problem.py` 705행 50+필드→**723행 77필드** · `review_status`:564→**:582** ·
> §0:62-64의 `is_exposable` 직렬 서술 반증)은 R2 §0-①·§정정이 정본이다.
> R2 요지: 미해소 D 중 **4건은 미설계가 아니라 미병합**(고립 브랜치)이며, 1차가 보지 않은
> 축(선언≠배선 7건)에서 진짜 갭 7건을 새로 설계했다.
