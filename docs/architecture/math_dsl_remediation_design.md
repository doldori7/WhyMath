...
| 오개념 ID 통합(kebab 30 ↔ M-id 839) | **골격 구현**(§1) — M-id 로더·crosswalk 테이블·read-time resolver·게이트 shadow 배선(evidence·hypothesis·**scene_generation 노출 표면**·wh1_loop는 중복이라 비채택) 완료. **매핑 30건 승인·자산+CLI 커밋**(2026-07-01 Kiki 권장대로 27 + 보류/반려 후속 신규 M-id 2건 M0862·M0863 + M0118 확인 = 전 30 kebab 매핑). 잔여 = 라이브 DB 적재(ops)·canary 플립 | rekey는 resolver로 불필요화(채택) |
...
2. 🟡 **crosswalk 골격**(완료) — kebab ↔ M-id 매핑 테이블 `misconception_crosslink`
   ...
   *(완료·집행)* **매핑 채택·적재** — Kiki '권장대로 승인'(2026-07-01)으로 27건 직접매핑 승인 →
   ...
   커밋. 이어 **보류/반려 후속**: 신규 M-id 2건 발행(`period-of-scaled-sine`→**M0862**·
   `angle-sum-non-triangle`→**M0863**·kebab 카탈로그 이관·코퍼스 839→841)·`fraction-cancellation`→M0118
   확인 → **총 30건·전 30 kebab 매핑**(미매핑 0). 스키마·FK·트리플·신규 M-id 존재는
   `test_crosslink_populate.py`가 고정. **라이브 DB upsert는 ops**(실 PG·CLI 실행).
