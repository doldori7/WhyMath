...
### G3 — 정리(모듈 8) 축 스펙 선언 4건이 main에 미배선 · 그중 3건은 **미병합 고립**이다 → `OPS-33`
...
- **`ReasoningType` 이름 충돌(G3-ⓓ)을 이 회차에서 고치지 않는다.** 개명·enum 결속은
  `S4-09`(SolutionPath/SolutionStep 실체화) 소관이며, `OPS-33`는 **그 사각을 발화시키는
...
### D8. 선언≠배선 감사기에 YAML 스펙↔코드 5번째 축 (G3 → `OPS-33`)
...
| `OPS-33-yaml-spec-unwired-audit-axis` | G3 | infra-debt | S4 | 3 | 감사기 정밀도 |
| `HARN-22-id-number-suggestion-race` | 아래 번호 사고 | infra-debt | S4 | 2 | 재발방지(하네스) |

**번호 배정 경위 — 가드가 두 번 작동하고 한 번 뚫렸다**: 세 건 모두 `backlog.py add` 경유이며
ID를 수기로 확정하지 않았다.

1. **막힌 것(가드 성공 2회)**: 최초 시도한 `KG-02`·`OPS-26`은 **다른 세션의 원격 브랜치가 이미
   점유**하고 있어 거부됐다(`KG-02-concept-content-review-promotion` @ `claude/subject-problems-
   theory-check-7n9n72` · `OPS-26-wh1-llm-observability-cache-wiring` @ `claude/whymath-ai-
   integration-check-5qqcp4`). 파일 목록만 눈으로 골랐다면 인플라이트 번호를 못 봐 충돌했을
   것이다 — 2026-07-18/25·07-29에 실제로 발생한 사고 유형이며 가드가 막았다.
2. **뚫린 것(가드 실패 1회 → `HARN-22`)**: 가드가 제안한 `OPS-29`를 채택했으나, 이 PR이 대기하는
   동안 **운영 EOS r3 세션이 똑같이 `OPS-29`를 제안받아** `OPS-29-ci-enforcement-declaration-
   contract`로 먼저 머지됐다(#775). main을 병합한 순간 `validate`가 `❌ 태스크 ID 번호 충돌
   'OPS-29'`를 exit 1로 잡았고, 상대가 머지분이라 **이쪽을 `OPS-33`으로 개명**해 해소했다
   (그랜드파더 등재 아님 — 개명이 가능한 쪽이 개명한다).
   원인은 가드가 **관측만 하고 번호를 예약하지 않는** 구조적 성질(TOCTOU): 두 세션이 각각
   `add`를 돌리는 시점에 어느 쪽도 아직 푸시하지 않았으면 **같은 빈 번호가 양쪽에 제안된다.**
   `HARN-10`(가드 신설)·`HARN-15`(원격 스캔 확장)·`HARN-21`(파서 구멍)은 전부 done이나 이
   경쟁 구간은 어느 태스크도 다루지 않아 **`HARN-22`로 신규 등재**했다(시스템 실수 = 재발방지
   등재 의무). 개명 비용 실측: 문서 7곳·`MEMORY.md` 3곳·태스크 파일 1건.
   병합된 `OPS-29`(CI 강제 상태 선언 계약)와 `OPS-33`(YAML 스펙↔코드 축)은 **관심사가 상보이며
   중복이 아니다** — 전자는 "돌아감≠막음", 후자는 "선언함≠구현됨"을 본다.
