...
| 10 | **MATH 산술 1.5b 87.5% — DELTA 임계 경계** | 산술 FAST(1.5b)=87.5% vs MID(7b)=100%. 차이 12.5%p > DELTA(7%p)라 현재 규칙은 *MID 권고*(§B.2)지만, 87.5%는 ABS_MIN(60%) 한참 위라 *비용·지연(p50 1초 vs 4초) 이득*을 보면 FAST 유지도 합리적 후보다. **keep-fast vs MID** 튜닝 여지 — DELTA 보정 또는 산술 난이도별 분기(쉬운 산술=FAST, 복합=MID)로 정밀화. temperature>0 시 산술 점수 출렁임(37~62%)도 확인됐으므로 운영은 temperature=0 권장. | M1.2 |
