...
- 설계안 §42-43의 표준 인증 이벤트(성공/실패/패스키 등록/세션 생성 등)는 **사용자별 접속 시각 이력으로 영속화하지 말고** 집계/운영 이벤트 형태로 발행해야 한다.
- 현재 `privacy/audit.py` 4종 writer는 반출·동의변경·관리자접근·역할변경만 다루며, `security_privacy.md:164-174`는 학생 접속 시각 프로파일링을 피하기 위해 로그인 이력 수집을 의도적으로 거부한다. 인증 이벤트를 같은 writer에 추가하면 이 결정을 뒤집는다.
- 반영: SLO가 필요하면 사용자 식별 정보 없는 집계 메트릭스(`login_success_rate`, `passkey_adoption_rate` 등)로 설계. 사용자별 시계열이 필요한 보안 이벤트는 별도 **운영 이벤트 버스**(`ops` 이벤트 스트림)로 분리한다.
...
5. **인증/세션 이벤트 집계·운영 이벤트 발급** — §42 표준 이벤트 12종을 `privacy/audit.py`가 아닌 별도 집계/운영 이벤트 버스로 설계.
