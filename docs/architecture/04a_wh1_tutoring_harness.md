...
**후속(1단계 잔여)**: 학생 솔루션 텍스트→단계 *분해*(L5 OCR) · verify_solution/match_gate를 coach 파이프라인에 *결선*(가설·intervention) · OCR confidence 요청 필드→게이트 ② 활성 · 단원별 verify 커버리지 ≥70% 게이팅 · PRM800K 한국 분포 보정.
