...
**후속(1단계 잔여)**: 학생 솔루션 텍스트→단계 *분해*(L5 OCR·공간정보) · `match_low_quality` 소비(재확인 발화·intervention 보류·L5) · `focus_step_index` 점층 escalation(반복 막힘 시 방향→의사코드→부분풀이) · `solution_steps`↔OCR 신뢰도 연동 · 단원별 verify 커버리지 ≥70% 게이팅 · PRM800K 한국 분포 보정.
