...
**후속(1단계 잔여)**: 학생 솔루션 텍스트→단계 *분해*(L5 OCR·공간정보) · `match_low_quality` 소비(재확인 발화·intervention 보류·L5) · trigger 발화에 `first_incorrect_index` 직접 반영(`recommend_coaching` 확장·답 미루기 유지) · `solution_steps`↔OCR 신뢰도 연동 · 단원별 verify 커버리지 ≥70% 게이팅 · PRM800K 한국 분포 보정.
