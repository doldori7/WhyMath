...
- **`OPS-33`(`yaml_spec_impl` 축)** — 등재 시점엔 원격에만 있어 대조하지 못했으나(§3 등재 요약의
  CLI 거부 기록) 이 문서 커밋 직전 main에 착지해 확인됐다. **축이 다르므로 중복이 아니다** —
  `OPS-33`은 *YAML 스펙 선언 ↔ 코드 구현*을 보고, `OPS-34`는 *면제 사전의 수취인·만료*를 본다.
  다만 `SOL-03`(`scene_element_kinds` 축)까지 포함해 **세 태스크가 같은 파일
  `ops/declared_unwired_audit.py`를 건드린다.** `OPS-33` acceptance ⑦이 이미 "축을 서수가 아니라
  **이름으로 식별**하고 먼저 착지한 쪽의 등록 구조를 재사용한다"는 규약을 세웠으므로,
  `OPS-34`는 그 규약에 편입된다 — `ARCH-25` 만료 계약을 셋이 공유하며 **재구현 0**이다.
