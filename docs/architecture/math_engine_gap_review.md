# 수식 엔진(Math Engine) 모듈 — 외부 EOS 틀 대조 갭 점검·설계 (2026-08-03)
...
`visualization_module_gap_review.md`(VIZ-01/02/03 승계) · `MEMORY.md` 결정 로그(2026-08-03).
...
| 축 | 소유 태스크 | 이 문서의 행동 |
|---|---|---|
| 서버측 답 채점(클라 `is_correct` 권위) | `NLP-02-server-answer-grading-shadow` | **승계·재설계 금지.** 기능 30에서 ⏸ 표기만 |
| 풀이 단계 *분해*(1 제출→N 단계) | `NLP-03-solution-segmentation-contract` | **경계 분할**(§3 D1 박스) — `depends_on: MATH-01` 추가 |
| 시각화 공급·계약·표현력 | `VIZ-01`(done)·`VIZ-02`·`VIZ-03`·`S4-03` | **승계.** 기능 31은 ⏸ 다수 |
...
| 계산 표기 → LaTeX(역방향) | `sympy.latex` 사용 **0건**(전 저장소 확인) | 🚫 §2-⑨ |
...
| 표기 계약 교차검증 | py↔js **2자**만 + `data/` CI 트리거 부재(부록 B) | ⚠️ → **D1** / §정정 |
...
| 세부 기능 | WhyMath 현행 | 판정 |
|---|---|---|
| 2D 함수 그래프 | `Graph2dSpec`(`schema/visualization.py:56`) + 웹 계산기 2141행 + WebView | ⏸ `VIZ-01`(도달률 6.97%) |
| 슬라이더 파라미터 조작 | `Graph2dSpec.parameters` | ✅ |
| 정의역·치역 지정 | `domain`·`y_range` | ⏸ `VIZ-03` |
| 음함수·부등식·극좌표·매개변수 | 웹 `classify`는 지원 / spec 표현력 부족 | ⏸ `VIZ-03` |
| 3D 곡면 | `Surface3dSpec` 좌석·렌더 미배선 | ⏸ `S4-03` |
| spec 내 함수식 검증 | `l3/visualization.py:22` **"후속"이라 자인** — 파싱 검증 0 | ⚠️ **D1 파생**(§3 D1) |
| 생성기 허용 타입 ⊆ 렌더 가능 타입 | 하드코딩 4종 | ⏸ `VIZ-02` |
| 애니메이션(Manim) | 스키마 좌석만·구현 0 | ⏸ `VIZ-02`·`S4-03` |
...
- ④ **`ci.yml` 경로 필터 backend·mobile·web 셋에 `data/notation_contract.json` 추가** — 이걸 빼면
  D1이 만든 게이트가 D2가 지적하는 유령이 된다(부록 B)
...
**태스크**: `MATH-01-notation-normalization-authority` (stage S3 · priority 2)
...
| 위치 | 현재 기술 | 실측 |
|---|---|---|
| `notation_contract.md §4` | *"새 표기 케이스는 fixture에만 추가하면 양측이 자동 검증한다"* | **PR에서 거짓.** `ci.yml:71,79,82` 경로 필터에 `data/`가 없어 `data/notation_contract.json`만 고친 PR은 backend·mobile·web 잡을 **전부 skip**한다(main push에서만 참). 2026-07-21 `schemas/` 사각 보완과 같은 구멍이 `data/`에 남았다 → D1-④가 봉인 |
