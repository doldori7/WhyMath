...
> 시각화 양식 127건 중 실제로 표현할 렌더 타입이 있는 것은 **15건(11.8%)뿐**이다. 1차 편이
> "만들어 놓고 연결하지 않았다"였다면 2차 편은 **"연결했더니 양쪽 어휘가 다르다"**다.
...
### §7.1 G1(최우선) — 양식↔렌더 좌석 계약 부재: 태깅 127건 중 좌석 보유 15건(11.8%)
...
흐르고, 그 양식을 실제로 표현할 렌더 타입이 존재하는지는 아무도 확인하지 않는다.

**좌석 대조(양식 10종 × 렌더 타입 3종 — 각 타입의 필드로 표현 가능한지 실측)**:

| 양식 | 건수 | 표현 가능한 타입 | 판정 |
|---|---|---|---|
| 함수그래프 | 15 | `interactive_graph_2d`(`function` 필드) | ✅ 좌석 |
| 수직선 | 25 | 없음(1D 축·구간 표시 필드 0) | ❌ → `VIZ-03` 확장 대상 |
| 단위원 | 8 | 없음(`function`은 y=f(x)만 — 원은 함수가 아니다) | ❌ → `VIZ-03` 확장 대상 |
| 부등식영역 | 1 | 없음(region 색칠 필드 0) | ❌ → `VIZ-03` 확장 대상 |
| 입체도형 | 32 | `interactive_surface_3d`는 z=f(x,y) **곡면만**(다면체 아님) | ❌ → `S4-03` |
| 평면도형 | 16 | 없음 | ❌ → `S4-03` |
| 벡터도 | 12 | 없음 | ❌ → `S4-03` |
| 점화도 | 11 | 없음 | ❌ **어느 태스크 범위에도 없음**(§7.3) |
| 수형도 | 4 | 없음 | ❌ **어느 태스크 범위에도 없음**(§7.3) |
| 넓이모델 | 3 | 없음 | ❌ **어느 태스크 범위에도 없음**(§7.3) |

**피해의 성격이 D1(도달 0회)보다 나쁘다 — 폴백이 아니라 오정보다.** D1 상태에서는 시각화 블록이
아예 `return`돼 학생이 아무것도 못 받았다. 지금은 게이트를 통과한 개념이 시각화 블록을 *받는다* —
그런데 LLM은 `_SYSTEM_PROMPT`(`l3/visualization.py:82-99`) 구조상 4종 중 하나를 **반드시** 골라야
하므로, 예컨대 "입체도형" 권장 개념이 `interactive_graph_2d` 스펙을 받으면 **평범한 2D 포물선
그래프가 실제로 WebView에 렌더된다.** 빈 화면도 caption 카드도 아니라 *그럴듯하지만 개념과 무관한
그림*이다 — CLAUDE.md 『AI·신뢰』 "확실하지 않을 때 자신 있게 말함 패턴 금지"의 시각적 형태이며,
05b Part 5-1이 막으려 한 "억지 그림"이 정확히 이것이다(단, §7.6에 남기듯 이 특정 오작동 자체는
*라이브 재현이 아니라 구조적 추론*이다).
...
- `VIZ-03`(기존 2D type의 필드 확장 — **새 type 0·anti-explosion 유지**)에 **수직선 25 + 단위원 8 +
  부등식영역 1 = 34건**의 좌석을 얹을 수 있다. 1D 축 모드·implicit curve·region 색칠은 전부
...
| 태스크 | 설계 | stage | 근거 | 상태 |
|---|---|---|---|---|
| `VIZ-04-visual-style-render-seat-contract` | §7.1 G1 | S3 | 태깅 127건 중 좌석 보유 11.8% — 억지 그림 위험 | 신규 등재(todo) |
...
| `VIZ-03`(기존) | 범위 확장 — 수직선·단위원·부등식영역 34건 + `depends_on: VIZ-04` | S4 | 코퍼스 분포 근거 §7.3 | 재정렬 |
