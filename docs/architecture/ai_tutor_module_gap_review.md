...
| 학습계획(1년 사이클) | PRD FR-010 P0 — 코드 0 | ~~§4-⑤ 유보~~ → **2026-07-30 Kiki 결정으로 유보 해제**(PRD P0 정본 채택) + **국면 판정축을 달력→진도율로 재편**(Kiki 제안). `learning_path_module_gap_review.md` §3 **D7(진도율)·D8(달력=보조)·D9(파생)**로 이관 — `PATH-06`·`PATH-04`·`PATH-05`. `study_plan` **엔티티** 불채택은 유지(파생 구현) |
...
| ⑤ | ~~학습계획 엔티티·시즌 플랜(FR-010)~~ → **시즌 플랜은 발화됨** (2026-07-30 Kiki 결정+제안) | 시즌 플랜은 `PATH-06`(진도율 주축)·`PATH-04`(달력=보조)·`PATH-05`(파생)로 착수(`learning_path_module_gap_review.md` D7~D9). 여기 적었던 "진도 데이터 원천 확보" 조건은 **오판정이었다** — 고3 대입 축은 수능 범위(성취기준 과목)로 정의되어 교과서 진도와 무관하다(문항 2,647건 전건이 성취기준 코드 보유). 국면 판정도 **달력이 아니라 진도율**로 재편됐다(Kiki 제안 — 카운트다운 금지 우회 불필요·개인차 흡수·페르소나 이식). 계속 유보되는 것은 `study_plan` **엔티티**(파생 구현이므로 불필요)와 **주간·일일 단위 쪼개기**(수요 미관측)뿐 |
