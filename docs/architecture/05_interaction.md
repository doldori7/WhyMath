...
> 학생과 시스템 사이의 *유일한 접점*. 웹·패드 우선 클라이언트(+동반 모바일) + 서버 백엔드.

## 책임

### 클라이언트 (Flutter · 웹·패드 우선 + 동반 모바일)
- 대화 UI
...
> Flutter **단일 앱(크로스플랫폼)** 유지 — 웹·태블릿을 1급 타깃으로 반응형 큰 화면 우선, 모바일(폰)은 동반. 웹 스택 결정(Flutter Web vs 웹 네이티브)은 후속(MEMORY 슬라이스 85).
...
### 1. 웹·패드 우선
데이터 광범위·그래프 계산기·인터페이스 복잡성·굿노트형 손글씨(iPad+펜슬 OCR)에 *큰 화면이 필수*. 웹·태블릿을 1급 타깃으로 두고, 모바일(폰)은 *접근성/동반 도구*(경량 복습·알림·보호자뷰)로 본다. PRD §6.3·§8.2 정합 (결정: MEMORY 슬라이스 85).
