...
> 학생과 시스템 사이의 *유일한 접점*. 웹 1급(Next.js) + 패드·모바일(Flutter) 클라이언트 + 서버 백엔드.

## 책임

### 클라이언트 (웹 = Next.js · 패드·모바일 = Flutter)
- 대화 UI
- 카메라 OCR (모바일·패드) / 파일 업로드 (웹)
- 수식 입력
- 시각화 표시
- 학습 곡선
- 부모/교사 모드 (Phase 3+)

> **2개 클라이언트 코드베이스**(슬라이스 86 결정): **웹 = Next.js 15(App Router) + React 19**(진짜 1급 — 큰 화면 분석·계산기·equity 저사양/a11y·MathLive·Desmos·KaTeX web-native), **패드·모바일 = Flutter 3.x + Riverpod**(패드=Pencil 저지연 손글씨 플래그십·모바일=저사양 안드로이드 미션 하한선). 공유는 *백엔드 API 계약*(OpenAPI) + 디자인 토큰 — 클라이언트 코드는 비공유. 시각화/입력 선언적 명세(§5.2)는 기기-중립(웹 네이티브 렌더·Flutter webview).
...
### 1. 웹 1급 · 패드 손글씨 플래그십 · 모바일 미션 하한선
**웹(브라우저)**이 *진짜 1급* — 큰 화면(그래프 계산기·다중 풀이 비교·개념 그래프 탐색) + equity(크롬북·공부방/도서관 PC·저사양 브라우저 무설치 접근, 사교육 격차 해소 미션 §6.3)를 *동시에* 달성하는 유일한 축. **패드(iPad+Pencil)**는 굿노트형 저지연 손글씨 풀이의 *프리미엄 경험*(필수 아님). **모바일(폰)**은 복습·알림·카메라 OCR의 *동반*이자 **저사양 안드로이드 미션 하한선**(포기 불가). PRD §6.3·§8.2 정합 (결정: MEMORY 슬라이스 85·86).
