...
| **D8** | `A11Y-02-accessibility-coverage-drift-gate` | S3 / 3 | ✅ **착지**(같은 세션·아래 §착지 기록) |
...
### D8(`A11Y-02`) 착지 기록 — 2026-08-11 같은 세션

| acceptance | 착지 내용 |
|---|---|
| ① 드리프트 가드 | `test/accessibility_coverage_governance_test.dart` 신설(4건) — `lib/features/*/presentation/` 전수 ↔ 접근성 스위트의 `presentation/` import 집합을 대조. 미등록 위젯은 **사유가 달린 `_exemptScreens`** 에 올라야 통과. 부수 가드 3종: 죽은 허용 항목 검출(삭제·개명분) · 사유 길이 하한(근거 없는 포괄 허용 금지) · 게이트↔허용목록 **동시 등재 금지**(판정 모순 차단) |
| ② 주 학습 화면 편입 | `ChatScreen`(라이트/다크) — 활성 문제 배너·선택지·**실제로 메시지를 전송해** 학생/코치 버블·신호 카드를 만든 뒤 검사(빈 초기 상태 금지). `ProblemScreen`(라이트) — 진단 fake로 문제 **로드 완료** 상태(발문·객관식 4지·"풀이 시작")를 렌더 |
| ③ 신고 다이얼로그 축 | `DefectReportButton`을 앱바에 달고 **다이얼로그를 실제로 열어** 라디오 6종·취소/신고하기를 검사(버튼만 검사하면 다이얼로그 내부가 빠진다). 라이트/다크 × 배율 3종 |
| ④ 06 §7 갱신 | 커버리지가 **기계로 고정**됐음을 표 앞에 명시 — 남은 🟡 "미검증"은 *누락*이 아니라 **근거·트리거와 함께 등재된 제외**임을 밝혔다. 후속 목록의 chat·problem 항목은 착지로 소거하고, 나머지는 허용목록 사유로 이관 |
| ⑤ 변별력(양방향) | 아래 뮤테이션 표 참조 — 4방향 실측 |
| ⑥ 범위 밖 동결 | Semantics 전면 감사·TTS·고대비/색약 토글·`accessibility_needs` 분기·AAA 전부 미착수(확인). 목표 레벨은 `A11Y-01`의 WCAG 2.1 AA 승계 |

**커버리지 변화**: 게이트 대상 **5축 → 8축**, 접근성 테스트 **30건 → 42건**(+거버넌스 4건).
모바일 전체 361 passed(이전 342)·라인 커버리지 86.5% → **86.8%**.

**CI 배선 확인(“존재함”과 “돌아감”은 다르다)**: `--reporter json`으로 **bare `flutter test`**
(CI가 쓰는 그 명령)의 실행 목록을 파싱해 거버넌스 파일이 **4건 실행됨**을 확인했다(54 스위트·
361 테스트). expanded 리포터 출력에는 이 파일 경로가 안 찍혀 처음엔 미실행처럼 보였는데,
**추정하지 않고 기계 판독으로 판별**했다 — 리포터 출력 습관의 문제였고 배선은 정상이었다.
...
| A11Y-02 ⓐ: 새 `*_screen.dart` 추가(허용목록 미등재) | **red** — 가드가 파일명을 지목(`['mutation_probe_screen.dart']`) |
| A11Y-02 ⓑ: 그 파일을 허용목록에 등재 | **green**(exit 0) — 양방향 성립 |
| A11Y-02 ⓒ: 파일 삭제 후 허용목록 항목만 남김 | **red** — 죽은 허용 항목 가드가 지목(사유 없는 제외가 쌓이지 않는다) |
| A11Y-02 ⓓ: 편입된 `problem_screen`의 액션 버튼을 24dp로 축소 | **red** — `Size(760.0, 24.0)` 실측 지목. 즉 게이트가 새 화면을 *실제로* 검사한다(편입이 형식이 아님) |
