...
## §8. 사후 정정 — 자체 재검증 결과 (2026-08-11, 같은 날 후행)

> `COLLAB-04` 착지(#792) 직후 이 문서의 판정 자체를 **적대적으로 재검증**한 결과다. 위 §0~§7의
> 본문은 **수정하지 않는다**(작성 시점 판정의 좌표를 보존) — 정정은 이 절에만 적는다.
> **판정 1건이 뒤집혔고, 그 방향이 학생 안전 축이다.**

### ①【뒤집힘·높음】D7의 "학생 안전 축은 문제 없다"는 **틀렸다**

§3 D7은 *"학생 안전 축에는 문제가 없다. 오히려 코드가 계약보다 엄격하다(계약 `summary` vs 실태
`none`)"* 라고 적었다. **이건 D7이 본 엔드포인트 1개에서만 참이다.**

`GET /v1/me/export`(`api/me.py:3017` · `ConsentedUser` — 봉인된 `/v1/me/assessments`와 **동일한
학생 토큰 클래스**)가 `_EXPORT_PLAN`(`privacy/export.py:93`)에 **내부 정본 `Assessment`**를 그대로
싣고, `_row_to_json`(`:190`)은 `to_schema().model_dump()` — **필터가 0**이다. `Assessment.to_schema()`
(`db/models/assessment.py:147`)는 매핑된 전 컬럼을 실어 `SchemaAssessment`로 복원하므로
`STUDENT_HIDDEN_PREDICTION_FIELDS` 5필드가 페이로드에 그대로 나간다. `state_snapshots`
(`UserStateSnapshot.estimated_percentile`)까지 합쳐 **또래 비교 데이터의 학생 도달 경로가 2개**다.

**즉 #8의 실태 해상도는 `none`이 아니라 `full`이다** — 계약(`summary`)보다 **느슨한** 쪽이고,
D7 본문이 *"있으면 심각도가 다르다"*고 지목한 바로 그 방향이다.

모순 사슬: `pipa_data_matrix.md` §2.2 #8(학생 본인조차 **◐**, §2.4가 제품 차원 차단까지 명시)
→ `access_matrix.json` #8(`student=summary`) → `ASM-02`(c)·`ASM-07` 3층 봉인 → **`/v1/me/export`가
원시 `full` 반환**으로 앞의 셋을 모두 우회.

**왜 안 잡혔나 — D7의 서사가 코드 축에서 그대로 반복됐다.** export가 `Assessment`를 실은 것이
2026-08-06(#668), `ASM-07` 봉인이 2026-08-09(#732)다. *먼저 있던 배선을 나중 결정이 무효화했는데
아무도 red로 받지 못했다* — D7이 **계약 파일**에 대해 쓴 문장이 **코드**에도 그대로 적용된다.
가드는 양방향 모두 0이다: `test_prediction_field_sealing.py`의 `export` 참조 **0건**,
`test_export.py`의 봉인 필드 참조 **0건**.

**공정 기재(과장 방지)**: 현재 실제 유출값은 **`null`**이다(`estimated_percentile` writer 0건).
**잠재 결함**이지 라이브 피해가 아니다. 또 export의 추론치 포함은 **의도적**이다 — 모듈 주석이
GDPR Art.15 본인 열람·이동권을 근거로 명시한다. 따라서 이건 단순 버그가 아니라 **법적 열람권 대
교수학적 비노출의 미조정 충돌**이며, 저장소에 그 충돌을 조정한 흔적이 **0건**이다(이 문서에도
`열람권`·`GDPR` 0건). 다만 봉인 설계의 취지가 *"writer가 붙는 순간"*을 막는 것이므로 **값 부재를
안전 근거로 쓸 수 없다.**

**→ `ASM-12-export-prediction-seal-bypass` 등재(priority 1).**

### ②【선후 정정·높음】`COLLAB-05`를 D7 설계 ② 그대로 집행하면 안 된다

§3 D7 설계 ②는 *"#8 `student`·`teacher`를 `summary`→`none`으로 좁힌다"*였다. ①이 드러난 지금
그대로 집행하면 **계약은 `none`, 실태는 `full`** 이 되어 어긋남이 *좁혀지는 게 아니라 위험한
방향으로 벌어진다*. **`ASM-12` acceptance ②의 판정이 `COLLAB-05`보다 먼저 와야 한다.**

### ③【보강·D5 미완】`COLLAB-04`가 같은 논리를 `data/corpus/`·`schemas/`에 적용하지 않았다

`COLLAB-04`의 변경 자체는 옳으나(개별 나열 → 위치 기반·fail-safe), 그 태스크가 세운 논리
*"계약 파일이 트리거 축에 있는가는 테스트 파일 축과 다르다"*를 두 축에 적용하지 않았다.

- **`data/corpus/`** — backend 잡의 **상시 게이트 스텝**(`ci.yml:319-320` 표기 커버리지
  `notation_coverage`)이 `data/corpus/problem_bank_*/problems.jsonl`을 읽는데, 코퍼스 변경은
  backend 잡을 **깨우지 않는다**. 코퍼스에 신규 표기를 넣으면 게이트가 `exit 1`을 내지만 그 PR은
  게이트를 실행조차 하지 않는다. `data/corpus`를 참조하는 `tests/backend` 파일은 **63개** —
  `COLLAB-04`가 봉합한 최상위 계약 소비 테스트 10개의 **6배**다.
- **`schemas/`** — `web`(`:110`)·`mobile`(`:102`) 필터에는 있는데 **`backend`(`:88`)에만 없다**.
  그런데 `tests/backend/test_failure_prevention_manifest.py:96-98`이 `schemas/v1.1/*.yaml` 존재를
  **하드 단언**한다.
- **`COLLAB-04` 주석의 정당화가 사실이 아니다** — *"하위 `data/corpus/`는 별도 축(`corpus` 플래그)이
  담당"*이라 적었으나, `corpus` 플래그의 유일한 소비처(`:209`)는 **`data_pipeline` 필터로 게이트된
  잡 내부**라 코퍼스 단독 PR에서는 잡 통째 skip → 그 스텝도 미실행이고, `continue-on-error`까지
  걸려 있다. **플래그가 죽어 있다.**

즉 §5가 등재한 **반복 실수 10회차**(트리거 축 부분 배선 — *연결이 조건부라 정상 상태에서는 완전히
정상으로 보인다*)가 **6배 큰 표면에 그대로 남아 있다.**

**→ `COLLAB-07-trigger-axis-completion-corpus-schemas` 등재(priority 2).**

### ④【보강】신규 가드의 사각 2건 (실증)

`tests/infra/test_ci_contract_fixture_trigger_wiring.py`의 `_backend_test_consumers`가 **파일 전체
substring 매칭**이라 "소비"가 아니라 **"언급"만으로 충족**된다 — docstring에 이름만 적은 테스트로
가드가 9 passed·`exit 0`. 또 대상이 `.json` suffix로 한정돼 **최상위 비-json 계약**
(`data/policy_contract.yaml` 등)은 계약으로 인식조차 못 해 **가드 green·CI skip**이 성립한다.
둘 다 합성 사례로 실증했다. `COLLAB-07` acceptance ④가 상환한다.

### ⑤【잠재】정규식 방언이 아니라 **git `core.quotePath`** 에서 발산한다

§3 D5·§7이 "CI는 POSIX ERE, 가드는 Python `re`"만 전제로 명기했는데, 17케이스 차등 테스트 결과
**두 엔진은 동일 입력에 대해 동치**다(전건 same). 진짜 발산은 입력 표현에 있다 —
`git diff --name-only`는 비ASCII 경로를 `"data/\355\221\234..."`처럼 **인용 + 8진 이스케이프**로
내보내므로 CI의 `grep -qE`는 선행 `"` 때문에 `^data/`가, 후행 `"` 때문에 `$`가 깨져 **SKIP**인데,
가드는 `Path.iterdir()` 원문자열로 판정해 **RUN**이다. 한국어 파일명 계약이 하나라도 생기면
**가드 green / CI skip**의 false-green이 성립한다. 현재 비ASCII 계약 파일 **0건**이라 잠재로 두고
`COLLAB-07` acceptance ⑥이 전제를 명시한다.

### ⑥【정본 정합】`by-design` 유예의 사유문이 실재하지 않는 크론을 근거로 든다

D6의 "롤업 호출 주체 0건"은 **미발견 갭이 아니라 이미 등재된 유예**였다 —
`ops/declared_unwired_audit.py:995`가 `harness.learning_metrics_rollup_cli`를 `_OPERATIONS_BATCH`로
분류하며 *"일 1회 **크론**/수동 실행이 설계 확정값"*이라 적는다. 그런데 **크론은 0건**이다(수동만
가능). 그리고 `_classify`(`:1131-1135`)는 `by-design:`에 대해 **사유가 비어있지 않은지만** 검사하고
**만료도 없다**(만료 계약은 `pending-task:`에만 적용).

**이건 D7과 정확히 같은 결함 클래스다** — D7의 `_produced_by_violations`는 "참조가 빈 문자열이
아닌가"만 보고 *참조 대상의 실재*를 안 보고, 여기 `_classify`는 "사유가 빈 문자열이 아닌가"만 보고
*사유의 진실성*을 안 본다. 두 레지스트리가 같은 방식으로 각자의 갭을 green으로 세탁한다.
CLAUDE.md **"만료 없는 유예·제외 금지"**(2026-08-03)에도 걸린다.

덧붙여 `_OPERATIONS_BATCH`는 **회원 1명짜리 범주**다(AXIS_CLI 42건 중 1건). 나머지 41건은 오프라인
리포트·코퍼스 생성기·라이브 의존이라 미실행이 학생 응답에 영향을 주지 않는데, 롤업 CLI만이
**학생 대면 서빙 엔드포인트의 유일한 공급원**이다. 같은 파일 `:978-985`의 `OPS-24`가 백필 CLI 2종에
대해 *"백필 누락은 **서빙 영향을 갖는 회귀**"*라며 유예를 뒤집고 **신규 잡 없이 `--check` 드리프트
가드를 기존 잡 스텝으로** 얹은 선례가 있다 — `COLLAB-06`이 그대로 쓸 수 있다.
**태스크 신설 없이 `COLLAB-06` 착수 시 반영**한다(중복 등재 회피).

### ⑦【정정】D6·D7 판정 자체는 **유효**하다

위 ①은 D7의 *부수 결론*(안전 축 무해)을 뒤집은 것이지 D7의 **사실관계**를 뒤집은 것이 아니다.
독립 재확인 결과: `AssessmentSchema` 클래스 부재·`StudentAssessment`에 필드 부재·거버넌스 테스트
green(29 passed) 전건 유효. `produced_by` 14좌표 AST 전수 해석 결과 **유령 참조는 #8 하나뿐**이다.
D6의 3개 주장도 전건 실측 확인됐다 — 특히 "미집계와 무활동이 구분되지 않는다"는 **반증을 시도했으나
실패**했다(두 상태의 적재 행이 모두 `[]`로 동일).

### ⑧【정정】부록 기대값 2건이 스테일 — 판정 불변

아래 부록은 **2026-08-11 01:54 시점 실측**이다. 이후 변화:

| 부록 | 당시 기대 | 현재 | 사유 |
|---|---|---|---|
| ① `__tablename__` | 63 | **64** | 신규 = `solution_paths`(`SOL-01` 착지). **협업 테이블 아님 → "협업 전 스택 0" 판정 불변** |
| ② `grep access_matrix .github/workflows/` | 0건(exit 1) | **1건** | `COLLAB-04`가 `ci.yml:71` 주석에 계약명을 나열. **이 문서의 조치가 이 문서의 기대값을 무효화한 것** |

부록 ②의 필터 프로브 자체는 정규식 **리터럴을 명령 안에 담고 있어** 지금 실행해도 당시 결함
상태를 그대로 재현한다(현행 `ci.yml`을 읽지 않는다) — 의도된 동작이다.

---

## 부록 — 재현 명령 (2026-08-11 01:54 확인 · 이후 변화는 §8-⑧)
