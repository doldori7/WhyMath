...
| SEC-10(세션 가시성·전체 로그아웃)이 서버 엔드포인트만 배선, 모바일 화면·인터셉터 0건 | HTTP | `MOB-12-auth-session-lifecycle-client-wiring` |
| PED-03(학습 공급 루프)의 study/outcome 엔드포인트를 부르는 쪽이 모바일에도 통합테스트에도 없음(단위 함수만 직접 테스트) | HTTP | `MOB-13-content-supply-loop-client-wiring` |
