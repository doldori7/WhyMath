...
>
> **모듈 자동 등록 설계**(2026-08-02): Kiki 요청 "관리자 작업화면에 모든 가능 메뉴가 자동으로 등록되도록" — [04 §2 원칙7](04_admin_console_architecture.md)에 선언적 모듈 레지스트리(`AdminModule`+`_MODULE_REGISTRY`) + `GET /v1/admin/menu` BFF 엔드포인트 설계 신설(코드는 아직 없음 — 설계 단계 결정, Kiki가 "설계 문서만 갱신"으로 범위 확정). 겸사겸사 03/04/00의 RBAC 서술을 `SEC-07`(v0 2값 착지) 반영해 현행화.
...
| **[04_admin_console_architecture.md](04_admin_console_architecture.md)** | 관리 UI 아키텍처 | Next.js + **Admin BFF** + **RBAC**(v0 착지·콘솔 확장 선결) + **모듈 자동 등록**(선언적 레지스트리). 8 core engines ↔ L1~L7 매핑·감사 로그·이중 회계. |
...
| 6 | **RBAC v0 착지·관리 콘솔 확장 선결** | role 2값(`STUDENT`·`CONTENT_ADMIN`)·콘텐츠 CRUD 인가 완료(`SEC-07`). 관리 콘솔의 Admin BFF·모듈 레지스트리 권한 필터·2차원 항목별 인가는 아직 선결. | `schema/enums.py`(`Role`)·`db/models/user.py`(`role` 컬럼), `04 §2 원칙3` |
| 7 | **모든 관리 메뉴는 레지스트리에서 자동 파생** | 관리 콘솔 좌측 내비를 손으로 하드코딩하지 않는다 — 선언적 모듈 레지스트리(`AdminModule`)에 엔트리 하나를 추가하면 권한이 되는 한 자동으로 메뉴에 등록된다. 메뉴 필터는 UX일 뿐, 실제 접근 통제는 라우트별 `require_role`이 이중 방어. | `04 §2 원칙7`(2026-08-02 신설, Kiki "모든 가능 메뉴 자동 등록") |
| 8 | **거부 우회 금지·측정치 이중 회계** | 게이트 판정을 UI에서 손편집 금지. 핵심 판정치는 SaaS(Langfuse)뿐 아니라 인프로세스에서도 산출(죽으면 "측정 실패"를 보여야). | `CLAUDE.md` 프로세스·AI·신뢰 금기 |
...
| 관리(admin) UI·교사/학부모 대시보드 | 🔴 | 코드 0. 모듈 자동 등록 설계는 완료(`04 §2 원칙7`, L7 Phase3) |
| RBAC(role 컬럼·`require_role`) | 🟡 | v0 2값(`STUDENT`·`CONTENT_ADMIN`) 착지·콘텐츠 CRUD 인가 완료(`SEC-07`). 관리 콘솔 소비·2차원 매트릭스는 미착수 |

---

**버전**: 1.1 | **작성**: 2026-07-24 | **최종 수정**: 2026-08-02(불변식 #6 RBAC 현행화·#7 모듈 자동 등록 신설·스냅샷 갱신) | **다음 검토**: 관리 콘솔 모듈 레지스트리 착수 시점
