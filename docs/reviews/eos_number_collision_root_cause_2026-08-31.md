...
## 7-B. 런북 형식 결함 2회차 — 증거 인용이 복사-실행됐다 (`CLAUDE.md` v0.2.6)
...
**상환** = `CLAUDE.md` v0.2.6 — 실행용 블록에는 **순수 명령만**(프롬프트 접두·출력 줄·설명
...
## 7-C. 3회차 — 같은 병목이 *버전 번호*에서 재현됐다 (`CLAUDE.md` v0.2.4)
...
**해소**: 본 세션이 양보해 밀어냈다 — main의 v0.2.4를 이력에 보존하고, 본 세션 분을
**v0.2.5**(대장 신규 항목) · **v0.2.6**(실행용↔증거 블록 분리)으로 재배정했다. §6-1a의
태스크 번호 양보와 **같은 원칙**(원격에 먼저 착지한 쪽이 기준)이다.

**의의 — 이것이 세 번째 동종 사례다.** 태스크 ID(§6-1a) → 게이트 ID(§8) → **문서 버전 번호**.
