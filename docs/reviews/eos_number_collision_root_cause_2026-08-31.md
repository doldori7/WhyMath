...
## 7-B. 런북 형식 결함 2회차 — 증거 인용이 복사-실행됐다 (`CLAUDE.md` v0.2.7)

§8의 결함(미머지 게이트 clear 안내)에 이어, **같은 세션에서 두 번째 런북 형식 결함**이 났다.

**경위**: 세션이 §2-3의 claim 대장 실측을 Kiki에게 보고하며 다음을 코드 펜스에 담았다 —
`$ git ls-tree -r --name-only origin/harness-claims` (프롬프트 접두 `$` 포함) 와 그 **출력 줄**
`claims/EOS-71-….json   ← 여기`. Kiki가 **그대로 붙여넣어** PowerShell이 둘 다 명령으로
해석했고 `CommandNotFoundException` 2건이 났다.

**피해 0** — 둘 다 조회였고 실행조차 되지 않았다. 다만 *파괴적 명령의 출력*을 같은 방식으로
인용했다면 결과가 달랐다.

**원인**: 코드 펜스는 Kiki에게 **복사-실행 대상으로 읽힌다.** 세션은 그것을 "증거 인용"으로
의도했지만 **의도는 형식에 나타나지 않았다.** 시크릿 규칙("생략 문자를 복사-실행되는 위치에
두지 않는다")이 이미 같은 원리를 담고 있었으나 *증거 인용* 축으로 일반화돼 있지 않았다.

**상환** = `CLAUDE.md` v0.2.7 — 실행용 블록에는 **순수 명령만**(프롬프트 접두·출력 줄·설명
화살표 금지), 증거 인용은 실행용과 **눈으로 구별되게** 표시하고 실행이 필요하면 **동작하는
형태의 블록을 따로** 준다.

**§8·§7-B의 공통 교훈**: 두 결함 다 *내용*은 옳았고 **형식이 틀렸다.** §8은 맞는 명령을 틀린
체크아웃에서 돌리게 했고, §7-B는 명령이 아닌 것을 명령 자리에 놓았다. Kiki에게 나가는 블록은
**"이걸 그대로 붙여넣으면 무슨 일이 일어나는가"** 를 매번 자문해야 한다.

## 7-C. 3·4회차 — 같은 병목이 *문서 버전 번호*에서 두 번 재현됐다

이 PR의 base를 최신화하는 중, main의 **PR #928(EOS-60)** 이 `CLAUDE.md`를 **v0.2.4**로 올린
것이 발견됐다 — 본 세션도 같은 번호를 썼다(대장 신규 항목 편입). **git이 add/add가 아닌
content 충돌로 잡아** 자동 병합을 막았다.

| | v0.2.4로 주장한 변경 |
|---|---|
| main (#928) | 골든 벤치마크 계약 인덱스 등재 |
| 본 PR | Kiki 머신 안내 규칙에 *대장 신규 항목* 편입 |

**해소**: 본 세션이 양보해 밀어냈다 — main의 v0.2.4를 이력에 보존하고 본 세션 분을 재배정했다
(아래 2차 충돌로 최종 **v0.2.6**·**v0.2.7**). §6-1a의
태스크 번호 양보와 **같은 원칙**(원격에 먼저 착지한 쪽이 기준)이다.

### 그리고 곧바로 한 번 더 (v0.2.5)

base를 다시 최신화하자 **PR #924**가 `CLAUDE.md`를 **v0.2.5**로 올린 것이 나왔다 — 본 세션이
방금 v0.2.5로 밀어낸 바로 그 번호다. **같은 해소를 두 번 반복**해 본 세션 분을
**v0.2.6**·**v0.2.7**로 다시 밀었다(main의 v0.2.4·v0.2.5를 이력에 보존).

특히 주목할 점: #924가 올린 규칙은 *"다단계 명령 블록에서 자리표시자 대신 셸 변수로 잇는다"*
— **§7-B와 같은 계열**(Kiki 명령 블록 위생)이다. 두 세션이 서로를 모른 채 **같은 부류의
결함을 각자 겪고 각자 규칙화**했다. 규칙 자체는 상호 보완적이라 둘 다 살렸다.

**의의 — 이것이 세 번째·네 번째 동종 사례다.** 태스크 ID(§6-1a) → 게이트 ID(§8) → **문서 버전 번호 ×2**.
셋 다 *서로의 미머지 변경을 볼 수 없는 병렬 세션이 같은 식별자를 각자 배정*한 것이고,
`HARN-43`이 겨냥하는 병목이 **`backlog.py` 번호 가드보다 넓다**는 증거다 — 가드가 있는
축(태스크 ID)에서는 CLI가 5번 실거부했지만, 가드가 **없는** 축(게이트 ID·문서 버전)에서는
매번 사람 또는 git 충돌이 최후 방어선이었다. 이번엔 git이 잡아 피해 0이나, `CLAUDE.md`
버전 줄이 두 브랜치에서 *같은 자리*가 아니었다면 조용히 둘 다 v0.2.4로 남았을 것이다.
