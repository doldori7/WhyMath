...
> **성격**: 조사 전용 — 본 세션은 대장(태스크·게이트)을 변경하지 않았다. §5 등재 후보는 제안이며 번호를 배정하지 않는다(HARN-10: 번호 추론 배정 금지).
>
...
> `backlog.py add`는 본 세션이 실행하지 않는다. 아래는 Kiki 판정용 후보이며, 각 후보에 **왜 12월 검증에 필요한가**(선언 §0-5 신규 기능 게이트)를 병기했다.

| # | 후보 | 제안 계열 | 12월 검증 관여 근거 | 선결 |
|---|---|---|---|---|
| 1 | **EOS Core/Adapter 경계 목록 1장** — 현행 `l1`~`l6`·`schema`·`db` 패키지를 Core/Adapter/Infra로 표기 | ARCH | G1 차단 조건("Core→Math 정적 의존 0")의 판정 기준 자체 | — |
| 2 | **import-linter forbidden 계약 추가** — Core 지정 패키지 → Adapter 지정 패키지 import 금지 + CI lint 잡 배선 확인(존재≠돌아감) | ARCH | G1 차단 조건의 **측정 도구** | 1 |
| 3 | **SubjectAdapter Protocol 정의** — `evaluate_answer`/`detect_misconception`/`explain`/`validate_problem` 4메서드 + Math 구현 1건(기존 함수 위임) | ARCH | Gate 0-B · "Physics를 붙일 때 Core를 뜯지 않는가"의 유일한 증명 수단 | 1 |
| 4 | **S1-16 착수**(기등재 todo) | S1-16 | A2 스키마 축 · 선언이 W1 몫으로 지목했으나 미착수 | — |
| 5 | **기능 인벤토리 + Migration Difficulty Matrix** — ②§3.3 12필드 · ②§3.14 6축 18점 | ARCH/HARN | Gate 0-D 전 문항 · 대시보드 5숫자 2개 | — |
| 6 | **열린 PR 16건 4분류** — MERGE/REWORK/POSTPONE/CLOSE + 라벨 생성 | HARN | ①§24 단독 경고 · #885·#856/#858 중복 실측 | — |
| 7 | **P0~P3 등급을 태스크 스키마 필드로** + `backlog.py`에 신규 기능 게이트 질의 | HARN | B1·B8·②Rule 3·4 — **PR #916 A1과 동일 축이므로 그 PR과 통합 판정 필요** | #916 |
| 8 | **Feature Flag 최소 체계** — 환경변수 기반 3~5플래그(E축·연구성 기능 default OFF) | OPS | ①§12 · 12월 판정 시 미완 기능 격리 | — |
| 9 | **README 로컬 부트스트랩 절** | 문서 | ①§23 · 신규 세션·기기 재현성 | — |
| 10 | **Golden Dataset 디렉터리 분리** | QA | ①§40 · EOS-64와 통합 검토 | EOS-64 |
| 11 | **선언 정본 §3~§5 갱신** — G0 확정치(앵커 6·25h·F-Ⅳ 3/6·CU 450/185) 반영 | 문서 | **PR #916 A7과 동일 — 중복 등재 금지** | #916 |

**의도적 비제안**: 에러코드 체계(B3)·Freeze 라벨(B7)·PR 템플릿 3블록(B6)은 각각 12월 판정 직전(11월)에 하는 편이 낫다 — B6는 분류 체계(후보 5·7)가 선행돼야 빈 칸이 되지 않고, B7은 11/30 시점 작업이다. 지금 등재하면 "만료 없는 유예" 없이 떠 있는 todo만 늘린다.
