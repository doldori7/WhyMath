...
| 대상 | 8/31 | 9/1 | 실측 근거 |
|---|---|---|---|
| **P0-02** 기존 기능 인벤토리 | 갭 | **충족** | `backlog/inventory/feature_inventory.yaml` 23행 — `action:` 판정 전건 보유(KEEP 4 · REFACTOR 17 · HEAVY_REFACTOR 1 · REPLACE_CANDIDATE 1). 생성기 `scripts/analysis/eos_feature_inventory.py`가 임계 정본(EOS-68 done) |
...
| **P0-09** Migration Map | 갭 | **충족** | 위 인벤토리 장부가 ②§3.14 6축 18점(A 과목결합·B DB·C 모듈·D 테스트·E 상태·F 데이터)을 그대로 담아 판정 |
...
| **Gate 0-D** (Migration) | 미충족 | **충족** | P0-02·P0-09 착지로 4문항 판정 가능 |
| **Rule 6** Core에 subject-specific 금지 | 미강제 | **기계 강제** | 위와 동일. **진성 위반 baseline이 0줄**이다 — 유예 목록에 CORE 진성 위반이 한 건도 없고, `unmatched_ignore_imports_alerting`(기본 ERROR)이 "빚을 갚으면 줄을 지우라"고 CI로 강제한다("만료 없는 유예 금지"의 코드 집행) |

②가 §3.7에서 **단일 문장으로 요구한 것**("Core가 이차방정식을 알게 만들면 안 된다")은
이로써 저장소가 첨부 문서보다 앞선다 — ②는 규칙을 산문으로 적었고, 저장소는 그것을
**깨지면 CI가 적색이 되는 계약**으로 옮겼다.

### §7.2 유지된 갭 (5건 — 전부 "분류·집행" 축)

§0 결론 2("②는 분류 축이 빠졌다")는 **여전히 유효하다**. 착지한 것은 *경계·계약* 축이고,
남은 것은 전부 *등급·교환·표시* 축이다.
...
**등재하지 않은 이유**(정직한 공백): 위 4건은 전부 Kiki 판정 사항이거나 기등재 후보(B급 1)의
집행이라, 본 재측정 세션은 대장을 변경하지 않았다. 2는 이미 게이트로 떠 있다.
