...
## 근본 설계 결정 — 반복 표본 감사 → 전수 감사 전환 (S3-15 · 2026-07-29)

### 근본원인 분석

rotation-0/1/2 3라운드가 왜 수렴하지 않았는지 두 축으로 분해했다.

1. **표본 크기의 통계적 한계**: n=200에서는 결함 **2건**만으로도 Wilson 95% 상한이 2.98%로
   임계 2%를 넘는다(점추정 1%인데도). 반대로 결함이 우연히 표본에 안 걸리면 진짜 결함율이
   더 높아도 PASS가 나온다 — 200-표본 반복은 "운이 좋으면 통과하는" 복권에 가깝다.
2. **패턴 매칭의 구조적 한계**: `rephrase_hygiene.py`(S3-12)는 자기 docstring에 이미
   정직하게 기록돼 있다 — "**어형 자체가 붕괴된 비문은 이 결정론 게이트가 못 잡는다** —
   형태소 분석·맞춤법 검사기 없이는 일반화된 탐지가 불가능하다(과공학 방지). **이런 결함은
   사람/AI 감사 표본 폴백으로 남는다**." 즉 설계 스스로 "패턴이 못 잡는 결함은 감사가
   받는다"고 전제했는데, 그 감사가 매번 **코퍼스의 일부(200/429)만** 보고 있었다 — 못 보는
   부분에 남은 결함이 계속 새 라운드에서 "새로운 유형"으로 재발하는 것처럼 보인 것이다.

### 커버리지 실측

rotation-0·1·2 표본(슬러그 교집합, 재생성으로 사라진 슬러그 제외)을 합집합하면 **349/429
(81.4%)** 만 어느 라운드에서든 검토됐다 — **80문(18.6%)은 3라운드 내내 단 한 번도 AI
검수를 받은 적이 없었다**. 반복 표본 추출이 "다른 200개"가 아니라 "겹치는 200개"를 자꾸
다시 보고 있었던 셈이다(`--rotation N`은 disjoint를 보장하지 않는 설계·docstring 명시).

### 결정: 전수 감사로 전환

4번째 패턴 축을 `rephrase_hygiene.py`에 추가하고 rotation-3을 도는 대신(수렴 안 됨이
실증됐으므로 무의미), **코퍼스 전체가 최소 1회는 검토되도록** 미검토 80문을 전수 검수하고,
발견된 결함을 **패턴이 아니라 항목 단위로 직접 교정**했다(재생성이 아니라 텍스트·태그
직접 수정 — Tier1 재검산으로 안전 확인). 완료 후 **429/429 전건**을 감사 라벨로 기록해
`corpus_audit_eval --min-n 200`(전수이므로 자동으로 충족)에 넣으면, 표본이 아니라 모집단
전체이므로 Wilson 상한이 통계적으로 훨씬 타이트해진다 — 반복 표본의 "운" 요소가 사라진다.

### 실행 결과

- **rotation-2의 잔여 결함 2건** 직접 교정: `wm-skel-1cc6d23230b8`(이질문자+어형붕괴+개념
  오치환 복합) · `wm-log-495ada0f7043`(목적어 불명확) — 둘 다 수치·정답 불변, 발문만
  자연스럽게 재기술.
- **미검토 80문 전수 검수**(pedagogy 서브에이전트 1인) → ok 46 · defect 34
  (`standard_tag_error` 29 · `grammar_break` 4 · `other` 1).
- **텍스트 결함 5건 직접 교정**: `wm-log-bab16bf460c4`(목적어 불명확)·
  `wm-skel-05c623267e6a`(영문 접미사 잔재+비표준 조어+모호 의문형)·
  `wm-skel-4004cf190f38`("두 차수식" 비표준어)·`wm-skel-9f01f300ac29`("극값" 개념
  오치환 — "근" 대신)·`wm-skel-e8a981925018`(어형 중복 비문). 전건 정답·근값 재계산으로
  수정 후 문구가 원 수학 내용과 여전히 일치함을 확인.
- **계통 결함 발견·근원 교정(부수 발견 — 감사 확정)**: census-gap 80문 중 QUAD-EQ 도메인
  (`wm-skel-*`) 29건이 `[10공수1-02-02]`(판별식으로 근을 "판별")로 태깅됐으나 문항은
  판별식을 쓰지 않고 인수분해·완전제곱꼴로 근을 직접 "계산"한다 — 정본 statement 대조로
  **[9수02-20]**("이차방정식을 풀 수 있고, 이를 활용하여...")이 정위치임을 확정. 코퍼스
  전수 조회 결과 이 오귀속은 census-gap 29건에 국한되지 않고 **`wm-skel-*` 전량 146문**에
  적용돼 있었다. 나아가 **소스 코퍼스 `problem_bank_generated_v0`(이미 rotation-0 240표본
  1.11% PASS로 "통과"했던 코퍼스)에도 매치되는 185문 전량**이 같은 오귀속을 갖고 있었다 —
  그 조기 통과 감사도 이 계통 결함을 못 잡았다는 뜻이다(표본 감사의 한계를 다시 한 번
  실증). 근본원인은 `harness/problem_corpus_batch.py`의 밴드 공통 스펙
  `_STANDARD_CODE = "[10공수1-02-02]"` 하드코딩(quad 밴드 전체가 단일 상수 공유) —
  **S3-12의 Vieta 밴드(`root_aggregate_batch.py`) 오귀속과 동일 계통**, 다른 밴드에서
  이미 한 번 실증된 실수 패턴이 이 밴드에도 있었다. 생성기 상수를 `[9수02-20]`으로
  교정(재발 방지 — 다음 재생성부터 정상)하고, 기존 두 코퍼스(rephrased_v0 146건·
  generated_v0 185건)는 슬러그·problem_id 불변으로 `achievement_standard_codes` 필드만
  직접 재태깅(전면 재생성은 슬러그가 `standard_codes`를 해시 입력으로 쓰므로 불필요한
  ID 연쇄 변경을 피하려 선택 — Tier1 재검산으로 안전 확인). 연동 오개념 카탈로그
  `M0862`(opposite-root-selected)·`M0863`(factor-sign-flip)의 `standard_code`도 동시
  교정(S3-12 "M0017·M0018·M0051 동시 교정" 선례 동형 — 표시 태그만 고치고 그래프 앵커를
  놓치는 회귀 방지). `concept_src_id=HK06`(이차방정식의 근)은 근 계산·비교 스킬을 포함하는
  broad 개념이라(behavior_skills에 quadratic-equation-solving 포함) 무관한 구 개념으로
  드리프트한 것은 아니라고 판단해 유지했다.

### 최종 판정

```
python -m whymath_backend.harness.corpus_audit_eval corpus_audit_rephrased_v0_census.jsonl \
  --max-defect-upper 0.02 --min-n 200 --require-as-found
표본 n=429  결함=0  점추정=0.0000  95% 상한=0.0063  → exit 0 (PASS)
```

**`problem_bank_rephrased_v0`는 전수 감사 PASS로 노출 적격 재평가 대상 전환**(법적 게이트
`is_exposable()`은 원래 통과 상태 — 실 노출 전환은 별도 운영 결정). `problem_bank_generated_v0`
185문도 같은 오귀속을 교정했으므로 기존 rotation-0 PASS 판정이 유지된다(교정이 판정을
바꾸지 않음 — 오귀속은 태그 정합 문제였지 수학적 정확성 문제가 아니었다. Tier1 재검산
620/620 통과로 확인).

**설계 결정 요약**: 이후 이 코퍼스(또는 유사하게 반복 FAIL하는 코퍼스)에 결함이 추가로
발견되면, **반복 rotation 표본 대신 미검토 잔여분을 채우는 census-gap 감사**를 우선
고려한다 — 표본 크기가 작을수록 통계적 우연의 영향이 크고, 전수 커버리지가 그 우연을
구조적으로 제거한다.
