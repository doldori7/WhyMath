...
> **⚠️ ARCH-07 사후 주석(2026-07-14)**: 이 문서는 *동결된 검수-이벤트 스냅샷*이다(재작성 금지·S2-08 선례).
> 표본 추출 시점(2026-07-13·PR #515)의 코퍼스를 가리키며, 이후 **S2-02(PR #517)가 verify 블록에
> solution_steps를 추가**해 현행 코퍼스로는 바이트 재현되지 않는다(렌더러 결정론·slug/발문/판정
> 기준 필드는 전부 불변 — ARCH-07 적대 검증이 코퍼스@#515 기준 재현 확인). 검수 판정의 유효성은
> 영향 없음(steps는 검수 대상 필드 아님).
