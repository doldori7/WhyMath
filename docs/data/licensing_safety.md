...
| 와이매스 문제은행 코퍼스 6종 v0/v1 (자체 저작) | 자체 저작 | ✅ | ✅ | ✅ | 연습·평가 문항 2,667건(성취기준+구조 시그니처 결정론 생성 5종·손저작 시드 1종). rephrased_v0(483)만 발문 표현에 LLM 사용(수치·정답·distractor_map은 코드 소유). 표본 검수는 generated_v0(620)만 완료(Wilson 95% 상한 1.11% PASS)·잔여 4종 진행 중(S3-09) (데이터 카드: `problem_bank_corpus_v1.md`) |
