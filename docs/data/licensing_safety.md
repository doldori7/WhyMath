...
| NCIC 정보과 성취기준 (코딩/정보 후보용) | 공공누리 1유형 | ✅ | ✅ | ✅ 필수 | **미착수**(E7 후보, 순서 미확정 — `subject_expansion_e_axis_v1.md` §2). 수학 NCIC 행과 동일 라이선스 성격 예상 — 원문 조회 후 확정 |
| **정보올림피아드(KOI) 기출 문항** (코딩/정보 후보용) | 저작물 | ❌ | ❌ | — | **미착수**. 평가원 기출과 동일 취급 — 구조 메타(영역·난이도)만 + 자체 동등문제 대체 |
| **백준(BOJ)·프로그래머스 문제 본문** (코딩/정보 후보용) | 사기업/저작물 | ❌ | ❌ | — | **미착수·절대 금지 후보**(검정교과서·학원 자료와 동일 취급) — 본문 재사용 시 이용약관·저작권 위반. 자체 알고리즘 문제로 완전 대체 |
