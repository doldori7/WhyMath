...
| 2026-06-17 | `684cfc13…eacaf0` | 437* | 541 | **개정본 수령·통합 대기(미적재)**. 변경 ⑴ 고1 공통 **기본수학1·2 개념 34 추가**(`10기수1-*`·`10기수2-*`·전부 2022개정·성취기준 참조 무결성 OK) — 단 *개념명+성취기준만 채운 스텁*(선수엣지 0·암기카드 0·오개념·은유·허용표현·설명·정식정의 전부 공란). ⑵ 기존 403 코퍼스 보존 필드 텍스트 변경 **0**, `std_codes` 표기만 47건(선택과목 하이픈 `[12미적Ⅰ01-01]→[12미적Ⅰ-01-01]` 43·멀티코드 구분자 4 — 코드 실체 동일·백엔드 `STANDARD_CODE_PATTERN` 멱등 수용). ⑶ 선수엣지·암기카드·국제트랙·성취기준 마스터(895=435+460) **불변**. ⑷ 사용자 보고 수학식·맞춤법 수정은 `설명`·`정식정의`(§3 redact·학생비노출)·`성취기준 문장` 칸 소재로 추정 → **커밋 코퍼스 미보존이라 대조 불가**(정본 xlsx에만 존재·재생성해도 redact). **결정**: 스텁 34개의 교수학 본문(선수엣지·오개념·은유) 충전 후 적재 — 코퍼스는 **403 유지**(품질 우선·CLAUDE.md #3). `*` = 정본 xlsx 기준 수치, 코퍼스 미반영. |
