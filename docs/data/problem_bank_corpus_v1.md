...
> **⚠️ 정정 (2026-08-11 · 문제은행 R3 §정정-1)**: 아래 요약의 **"6종 · 전 2,613건"은 stale**이다.
> 실측은 **7종 2,647건**이며, 누락분은 `S4-13`이 신설한 7번째 코퍼스
> `problem_bank_probability_finite_v0` **34건**이다(2,613 + 34 = 2,647). 그 34문은 `S4-16`
> 강등전 통과 전까지 **노출 부적격**(`review_status=pending`) 상태다. 본문은 이력 보존을 위해
> 개정하지 않는다 — 정본 수치는 이 배너다. 근거: `docs/architecture/problem_bank_gap_review_r3.md` §부록.
