...
>
> **상태(2026-06-21)**: Phase 0(데이터카드) 작성. 코퍼스(`data/corpus/atom_graph_v1/`)·정형화·
> 적재는 Phase 1에서 신규 파이프라인 `data_pipeline/atom_graph/`로 생성한다(아래 §6).
...
| 항목 | 값 |
|---|---|
| 형태 | 사용자 업로드 xlsx(4 시트: 안내·선수엣지_통합·원자_통합마스터·요약) |
| 업로드일 | 2026-06-21 (K-12 3축 확장·정렬·코드 정비본·표기 정비) |
| 원본 sha256 | `759f163a2a271f88c4eaf3d26f30e4cfad96cab1a5df79304b8fdbefcde41ed4` (917,455 bytes) |
| 저작 | 와이매스 자체작성(교수학 주석·원자화·대학 신규축) + 공공 표준 *코드* 참조(NCIC) |
| 산출 코퍼스(Phase 1) | `data/corpus/atom_graph_v1/graph.json` + `_provenance.json` |

> 원본 xlsx는 **커밋하지 않는다**(§3 redaction 대상 자유텍스트 포함). 진실 원천은 redaction을
> 적용한 코퍼스(graph.json)다. 재추출 시 동일 sha256 원본을 사용자에게 재요청한다.

### 표기 정비 이력(전수 검증 완료)
- 비표준 사이시옷 점검: **초기값→초깃값 15곳은 원본에서 수정 완료**. **자리값→자릿값 13 · 경계값→
  경곗값 4(총 17)는 Phase 1 transform 사이시옷 정규화로 흡수**(회색지대 고유값 등은 유지·결정론).
- 코드 정합: 원자 `연결성취기준`의 **미적Ⅰ/Ⅱ 43개가 무하이픈**(공수·기수는 하이픈으로 NCIC와 일치).
  → **Phase 1 transform에서 원자측 정규화**(`12미적Ⅰ01-01`→`12미적Ⅰ-01-01`)로 공식 NCIC에 맞춤
  (코퍼스 `standards_v1` 무수정). 그 외 392 연결성취기준 코드는 코퍼스와 조인 OK.
