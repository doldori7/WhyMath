...
> **⚠️ SUPERSEDED (2026-08-11)**: 이 리포트와 짝 JSON은 **6종 2,667문** 기준이며,
> `problem_bank_coverage_2026-08.md`(7종 **2,638문** · 유형 축 · 과목 축 포함)로 **대체됐다**.
> 측정 이력으로 보존하며 본문은 수정하지 않는다 — **현재 수치로 인용 금지**.
> (직전 배너 "stale — 재생성 대상"[2026-08-11 · 문제은행 R3 §정정-2]이 요구한 재생성이
> 이것으로 이행됐다. 재생성의 **CI 배선**은 여전히 `PB-02`(열린 PR #739) 소관이다.)
>
> **승계되는 주의(신본에도 그대로 적용)**: 이 수치는 **main 기준**이다 — `PB-06`이 대장을
> 고정한 미머지 저작분(코퍼스 30종 11,446문 · 고유 성취기준 코드 55개 · main과 교집합 0)은
> 반영돼 있지 않다. 커버리지 리포트를 **저작 우선순위의 유일한 입력으로 쓰기 전에 `PB-06`의
> 회수 판정을 확인**할 것.
