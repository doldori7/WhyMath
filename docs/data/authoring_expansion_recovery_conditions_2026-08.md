...
### ⑷ 부수 조건 — `S4-09` 회수(`claude/whymath-solution-review-40xspg` `707c5665`)
...
브랜치 전용 산출 12파일(`l3/multi_solution.py` · `l3/solution_path*.py` · `db/models/solution_path.py` ·
`whs/path_promotion.py` · **alembic 리비전 2건** · 테스트 5). 단 **소비처 `S4-10`은 그 브랜치에서도
`in_progress`**라, S4-09만 회수하면 `multi_solution.py`가 미소비 상태로 착지한다. alembic head
분기 재봉합도 필요하다.
...
| ② | `PB-06` ② | `ci.yml:301-302` | **`ci.yml:306-307`** | 라인 드리프트(303-305는 주석) |
...
`provenance_audit`는 `ci.yml:306-307`(backend 잡)에 배선돼 있는데 backend 필터(`ci.yml:74`)에
