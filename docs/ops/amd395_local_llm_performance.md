...
| # | 대책 | 근거 | 되돌리기 |
|---|---|---|---|
| **1** | **페이지파일 확대(시스템 관리 또는 32~64GB)** | 🔴 **1순위로 승격(08:07 사다리 결과)** — 물리 여유 34.9 GiB에서 **1.0 GiB CPU 버퍼**가 실패했고 `free_swap`이 2.8 GiB→891 MiB로 고갈 중이다. 커밋 한도가 막히면 크기와 무관하게 전부 실패한다 | 시스템 설정 원복 + 재부팅 |
