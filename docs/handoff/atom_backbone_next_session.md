# 원자 백본 마이그레이션 — 다음 세션 인계 (Phase 2b ~ 5)

> **목적**: 이 세션에서 Phase 0·1·2a를 완료·푸시했다. 다음 세션이 *재유도 없이* 이어가도록 상태·
> 환경·규율·다음 단계를 못 박는다. **진실 원천은 `MEMORY.md`의 "원자 백본 전면 교체" 결정 로그**다.
...
- data-pipeline venv:
  ...
  cd src/data-pipeline && /tmp/wmvenv/bin/pip install -e '.[dev]'
  ```
- **원자 소스 xlsx·대학 참조 CSV는 휘발**. 단, **Phase 2~5는 *커밋된 코퍼스*(graph.json)로 작업**하므로 xlsx 불요. (xlsx 재추출이 필요하면 사용자에게 재요청.)
...
1. **Phase 2b — 원자 임베딩(pgvector)**: `concept_embedding`을 원자 code로 재키잉(또는 `atom_embedding` 신규). 임베딩 입력=안전 필드만(name + metaphor 등·아직 콘텐츠 4종 전이라 name 중심). Fake provider 단위테스트 + 실 임베딩 통합(게이트). **미러**: `l1/concept_graph/embedding.py`·`populate.py`.
2. **Phase 2c — Neo4j 원자 그래프 재적재**(MERGE 멱등). **미러**: `data_pipeline/concept_graph/load.py`(NEO4J_* env·`.[neo4j]` extra).
...
## 7. 핵심 경로
- 코퍼스: `data/corpus/atom_graph_v1/{graph.json,_provenance.json}`
- 파이프라인: `src/data-pipeline/data_pipeline/atom_graph/`
- backend 적재: `src/backend/whymath_backend/l1/atom_graph/`
