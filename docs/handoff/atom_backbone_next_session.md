...
4. **Phase 4 — 크로스워크 (통합 정의, 2026-07-03 S0 계획 확정·사용자 승인)** — 구(舊) 이원 정의(본 문서 "문제 크로스워크" vs `docs/strategy/status_roadmap_2026-07.md:79` "437↔원자 concept_src_id 연결")를 다음 3요소로 통합. **본 절이 정본**:
   ...
   - **ⓑ problem_concept 재연결** (~~S0-3~~ → **S2 연기**, 2026-07-04 사용자 결정): 원계획은 기존 문제의 *소단원/단원 매칭* + *성취기준 코드* 2중 다리로 `problem_concept`를 원자 code로 재연결. **실측 결과 문제 코퍼스→DB 적재 파이프라인 부재**(문제는 테스트 픽스처만·어떤 파이프라인도 문제를 시드하지 않음) → 재연결할 실데이터 0건 → 지금 파이프라인 구현 시 소비처 없는 dead code(CLAUDE.md 금기). **문제은행이 실제 적재되는 S2(콘텐츠 공장)로 연기.** 그 사이 구 437 폐기(S0-4)의 안전은 **problem_concept FK 0-참조 게이트**(fresh DB 단언 + prod는 S0-5 진단 스크립트의 `problem_refs` fanout 집계)로 커버.
