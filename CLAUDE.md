...
- ❌ **삭제된 브랜치의 커밋을 "소실"로 단정 금지 — git 실패는 접근 불가의 증거가 아니다 (2026-08-09 등재 → 2026-08-10 전제 정정)** — 회수 태스크 착수 시 원 브랜치 생존은 `git ls-remote --heads origin | grep <브랜치명>`으로 **가장 먼저** 확인한다("규모가 크다·hot file 충돌"만으로 반복 보류하지 말 것 — 진짜 차단 사유가 다른 데 있을 수 있다). **다만 브랜치가 삭제됐어도 커밋은 대개 살아 있다**: GitHub API(`repos/<owner>/<repo>/commits/<sha>`, `detail=full_patch`로 유니파이드 diff까지 · `contents?ref=<sha>`)는 브랜치에서 떨어진 SHA도 그대로 돌려준다. 따라서 `git fetch`/`cherry-pick` 실패는 **"git 프로토콜 경로가 막혔다"**는 뜻이지 "원본이 사라졌다"가 아니다 — 회수 좌표(SHA)를 알고 있다면 API 경로를 반드시 시험한 뒤에야 "재설계"로 성격을 바꾼다. 태스크 notes의 원 커밋 SHA는 이 때문에 **삭제 후 유일한 회수 좌표**이므로 지우지 않는다. (사고 경위 ①: S3-24/S3-25를 `/drive` 3회 이상 "규모가 크다"는 이유로만 보류하다 뒤늦게 브랜치 삭제를 발견 — `git ls-remote` 5초면 될 확인을 여러 세션이 놓쳤다. ②: 그 발견 직후 이번엔 반대 방향으로 과잉 단정 — `git fetch <sha>` 실패만 보고 "원본 접근 전면 불가"로 결론내 16개 태스크 acceptance를 전부 "포팅"에서 "재설계"로 뒤집고 이 규칙까지 그 전제로 등재했다. 실제로는 API로 완전히 읽혔고(2026-08-10 `224043f` 실측), 병렬 세션은 같은 시기에 API로 원본을 회수해 S3-35를 완료했다(#748이 "회수 경로는 GitHub API 고정"으로 정본화). **한 접근 경로의 실패를 전 경로의 실패로 일반화하지 말 것** — 기존 "환경 사실의 추론 등재 금지"가 *부분 성공을 전체 정상으로* 읽는 것을 막는다면, 이 항목은 그 거울상인 *부분 실패를 전체 불가로* 읽는 것을 막는다)
