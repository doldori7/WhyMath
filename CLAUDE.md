...
- **Kiki 머신(Phaiakes9) 행동 요청 시 실행 명령 동봉 필수** — git pull·env 설정·CLI 실행 등 Kiki가 직접 실행할 작업을 안내할 때는 항상 복사-붙여넣기 가능한 명령 블록(PowerShell 기준·작업 디렉터리 명시)을 함께 제공한다. 특히 **미머지 브랜치의 신규 파일을 쓰는 명령이면 해당 브랜치 fetch/checkout(또는 pull) 명령을 반드시 선행 포함**한다(2026-07-14 `fill_live_cost_table.py` 부재 오류 재발 방지 합의).
