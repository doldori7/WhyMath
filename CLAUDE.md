...
- ❌ **미병합 브랜치 "회수" 태스크 착수 전 소스 브랜치 생존 확인 생략 금지 (2026-08-09 등재)** — 다른(고립) 브랜치에서 코드를 포팅·재적용하는 태스크는 착수 전 **가장 먼저** `git ls-remote --heads origin | grep <브랜치명>`(또는 동등 확인)으로 원 소스 브랜치가 아직 origin에 존재하는지 실측한다. "규모가 크다·hot file 충돌이 있다"는 판단만으로 반복 보류하지 않는다 — 진짜 차단 사유가 "소스 자체의 소실"일 수 있고, 이 경우 규모와 무관하게 그 태스크(와 거기서 파생된 모든 하위 태스크)는 애초에 "포팅"이 불가능하다("재설계"로 성격이 바뀐다). (사고 경위: S3-24/S3-25(미병합 브랜치 회수 Stage 2, 16개 항목)를 `/drive` 반복 호출 3회 이상 "규모가 크다"는 이유로만 보류하다가, S3-24를 9개 개별 태스크로 분리 등록까지 마친 뒤에야 원 소스 브랜치 `claude/shadow-data-s3-pilot-nh5kbz`가 origin에서 완전히 삭제됐음을 발견 — `git fetch`·원 커밋 해시 직접 fetch 전부 실패. 더 일찍 `git ls-remote`로 5초 만에 확인 가능했던 사실을 여러 세션에 걸쳐 놓쳤다)
