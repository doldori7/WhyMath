...
class TestGlobToRegex:
    ...
    def test_directory_pattern_expands_to_all_descendants(self):
        """test_디렉토리성_패턴은_하위_전체로_확장"""
        # 끝 '/'인 패턴은 'p/**' 취급
...
class TestExpandAndScope:
    ...
    def test_single_path_scope_determination(self):
        """test_단건_스코프_판정"""
...
class TestOverlap:
    def test_detect_overlap_with_real_files(self):
        """test_실파일_교집합_검출"""
    ...
    def test_detect_prefix_containment_for_new_file(self):
        """test_프리픽스_포함_검출_신규파일_대비"""
    ...
    def test_separate_domains_have_no_overlap(self):
        """test_도메인_분리는_무겹침"""
        assert overlap("A", ["src/backend/**"], "B", ["src/data-pipeline/**"], FILES) is None
        assert overlap("A", ["src/mobile/**"], "B", ["docs/**"], FILES) is None

    def test_undetermined_when_one_side_has_no_paths(self):
        """test_한쪽_paths_미선언이면_판정_불가"""
        assert overlap("A", [], "B", ["src/backend/**"], FILES) is None

    def test_identical_literal_file_caught_by_tier1(self):
        """test_리터럴_동일_파일은_1단이_잡는다"""
        result = overlap(
            "A",
            ["docs/standards/build_harness.md"],
            "B",
    ...
    def test_describe_summarizes_evidence(self):
        """test_describe는_근거를_요약한다"""
