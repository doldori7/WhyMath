"""조사 받침 헬퍼 테스트 — 수 읽기(한자어) 기준 받침 판별·조사 선택 봉인(S2-08).

봉인: ① 일의 자리 읽기 표 전수 ② 다자리·0으로 끝나는 수(십·백·천·영 지배) ③ 음수 부호 불변
④ 한글 단어 종성 ⑤ 무리수·분수 문자열 None 폴백 ⑥ AI 검수 리뷰 실오류 정확 예시
(0→과·4→와·13→을·6→을·3→과) ⑦ (으)로 ㄹ 받침 예외. 순수 함수라 LLM·DB·IO 0.
"""

from __future__ import annotations

import pytest
from whymath_backend.l3.equivalent.josa import (
    eul_reul,
    eun_neun,
    euro_ro,
    has_batchim_hangul,
...
class TestHasBatchimText:
    ...
    @pytest.mark.parametrize("token", ["sqrt(11)", "-3±2√2", "1/2", "√3/2", "2*sqrt(2)"])
    def test_irrational_or_fraction_none(self, token: str) -> None:
        # 분수(분모 먼저)·무리수(루트) 독법이 마지막 음절을 지배해 문자 기반 판별 불가 → None.
...
class TestJosaSelectors:
    ...
    def test_none_fallback_default_batchim(self) -> None:
        # 판별 불가 문자열은 안전 기본(받침 有) — 을/과/은/이 선택.
        assert eul_reul("sqrt(11)") == "을"
        assert wa_gwa("-3±2√2") == "과"

    def test_josa_default_override(self) -> None:
        # default_batchim=False면 판별 불가 시 받침 無 계열 선택.
        assert josa("sqrt(11)", "을", "를", default_batchim=False) == "를"
