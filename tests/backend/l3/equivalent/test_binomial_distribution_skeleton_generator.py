"""고등 이항분포 평균·분산 스켈레톤 생성기 단위테스트(hermetic·게이트 재사용).

`BinomialDistributionSkeletonGenerator`가 ① 전건 S2-a 4종 게이트 통과(분수 답 검증 재실증
— discrete_expected_value_skeleton_generator와 동형 설계) ② answer가 독립 재계산(E(X)=np·
V(X)=np(1-p))과 일치 ③ 풀 유일·소진 시 None ④ `kind` 필터로 밴드가 실제로 분리됨(2026-08-05
elementary_addsub 사고 재발 방지 회귀 봉인) ⑤ 개념 태깅·위생 게이트 무오탐을 못 박는다.
LLM·DB·PG 0(순수 결정론).
"""
...
class TestPool:
    def test_pool_nonempty_and_unique(self) -> None:
        pool = _build_pool()
        assert len(pool) >= 300
        keys = {(s.kind, s.n, s.k, s.m) for s in pool}
        assert len(keys) == len(pool)

    def test_probability_fractions_are_irreducible(self) -> None:
        from math import gcd

        for skeleton in _build_pool():
            assert gcd(skeleton.k, skeleton.m) == 1
            assert 1 <= skeleton.k < skeleton.m

    def test_answer_matches_independent_recompute(self) -> None:
        for skeleton in _build_pool()[:150]:
            p = Fraction(skeleton.k, skeleton.m)
            expected = skeleton.n * p if skeleton.kind == "mean" else skeleton.n * p * (1 - p)
    ...
    def test_kind_filter_restricts_output_and_ids_stay_unique_across_bands(self) -> None:
        ...
        for kind in ("mean", "variance"):
            candidates = _drain(BinomialDistributionSkeletonGenerator(kind=kind), _spec(), 30)
    ...
    def test_generation_exhausts_to_none_per_kind(self) -> None:
        for kind in ("mean", "variance"):
            ...
            gen = BinomialDistributionSkeletonGenerator(kind=kind)
...
class TestAcceptance:
    def test_all_pass_acceptance_gate(self) -> None:
        for kind in ("mean", "variance"):
            pool_size = len(_build_pool(kind))
            candidates = _drain(
                BinomialDistributionSkeletonGenerator(kind=kind), _spec(), pool_size
            )
...
class TestShape:
    def test_question_format_and_no_choices(self) -> None:
        gen = BinomialDistributionSkeletonGenerator()
    ...
    def test_concept_tags_resolve_via_legacy_437_space(self) -> None:
        candidate = BinomialDistributionSkeletonGenerator().generate(_spec())
        assert candidate is not None
        assert len(candidate.concept_tags) == 1
        assert candidate.concept_tags[0].concept_src_id == "H:12확통03-03"
