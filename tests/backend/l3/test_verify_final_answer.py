"""순수 `verify_final_answer` 3상태 검증 단위테스트 — 서버 권위 채점 primitive(S3-09·hermetic).

correct(학생 최종답 ≡ 기대정답·등식/자연표기/객관식)·incorrect(불일치 확정)·unverifiable(기대정답
미보유·빈 입력·판정 불가) + 데모 4문제 정답 + 서버 정직성(unverifiable을 correct로 위장 안 함) 가드.

DB·네트워크 0(순수 함수). 서버 권위 *배선*(submit_attempt에서 클라 보고를 이김)은 api/test_me.py가
...
class TestDemoFourProblems:
    """데모 4문제 정답 — 실기기 시연 코퍼스(problems.jsonl)의 기대정답과 자연 학생 입력."""

    def test_larger_root_equation_form(self) -> None:
        # wm-quad-eq-larger-root: 기대정답 "3", 학생 "x=3"(등식 형태) → correct.
...
class TestUnverifiable:
    """서버 검증 불가 → unverifiable(정직 — 절대 correct 위장 안 함)."""
