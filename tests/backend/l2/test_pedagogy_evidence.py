"""L2 교수법 증거 writer — `evidence_event` 기록 좌석 단위테스트 (PED-03·SEC-16·hermetic).
...
`evidence_event`의 최초 writer다. 관심사:
  ...
  - **SEC-16**(`docs/reviews/functional_security_audit_2026-08-08.md` M2): 처치 행 meta의 키드
...
class TestTreatment:
    ...
    async def test_records_user_binding_hexdigest_in_meta(self) -> None:
        """SEC-16 — 처치 행 meta에 (session ↔ user) 키드 해시 바인딩이 올바른 값으로 실린다."""
        session = _FakeSession()
        settings = _settings()
        sid = uuid.UUID(int=7)
        row = await record_pedagogy_treatment(
...
class TestSessionUserBinding:
    """SEC-16 바인딩 함수 — 순수·결정론·입력 민감(세 축 전부 변별)·hex 형태."""
...
class TestOutcomeOwnership:
    """SEC-16 결함주입 — 위조·도용·구형·중복 기입이 행을 남기지 못하고 거부되는지."""
...
class TestB1PlaintextProhibition:
    def test_signatures_have_no_free_text_slot(self) -> None:
        ...
        막아 두면 실수로도 흘릴 수 없다. SEC-16이 추가한 `user_id`(UUID)·`settings`(Settings)는
