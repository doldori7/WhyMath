...
class TestCoverageScopeUnavailable:
    """`school_type`이 None이면 커버리지 3필드 전부 None(스코프 계산 불가·정직 폴백)."""
...
class TestCoverageComputed:
    ...
    async def test_percent_computed_from_scope_then_observed(self) -> None:
        ...
        assert result.standard_coverage_observed == 2
        assert result.standard_coverage_percent == 50.0
