"""선수 traversal 예산(깊이·노드 수) 단일 출처 가드 — Q10-⑧·④ (동작 불변 동결).

math_dsl_risk_register.md Q10-⑧(traversal 예산). 선수 traversal은 두 축으로 bound된다:
① *깊이* — `MAX_PREREQUISITE_DEPTH` 단일 출처, API 경계(`api/me.py` MaxDepth)가 상한을 *공유*
   (매직 넘버 중복 제거). (a) 값 동결 + (b) API가 단일 출처 참조를 함께 못 박아 drift 차단.
② *노드 수(breadth)* — `MAX_PREREQUISITE_NODES` 단일 출처 안전밸브(④ hub 오염). 재귀 CTE 결과가
   규모에서 병리적으로 폭증하는 것만 차단하고 현재 규모에선 미발동(행동 불변). 순수 헬퍼
   `_cap_by_node_budget`이 절단 시 *조용히 넘기지 않고* 로깅한다(silent 금지·CLAUDE.md #3).

⚠️ 둘 다 *그래프 traversal 예산*이다 — "LLM 컨텍스트 예산"(max_tokens)이 아니다(소비처 부재·
premature·spec만 register §5 ⑨에 동결). hermetic(introspection + 순수 헬퍼·DB 불요).
"""
...
# ── 노드 수(breadth) 예산 — `MAX_PREREQUISITE_NODES` 안전밸브(④ hub 오염·순수 헬퍼) ──────────
