"""problem.identity_id 마이그레이션 *왕복 통합테스트* — 실 PG (WHYMATH_RUN_INTEGRATION).

마이그레이션 head가 적용된 **실 PostgreSQL**에서 S4-18 리비전(090d254a5d43)을 *revision-local
왕복*(downgrade -1 → upgrade head)으로 검증한다. CI `backend — 마이그레이션·통합 (실 PG)` 잡이
이미 `alembic downgrade base → upgrade head` 전구간 왕복을 돌리므로, 이 테스트는 그 위에서
*S4-18 리비전이 identity_id 1컬럼+색인을 정확히 더하고/되돌리며 UUID PK·기존 행을 보존*함을
못 박는다(`test_problem_p3a_migration_integration.py` 구조 미러). PG 미도달 시 graceful skip.

검증:
  ① head에서 problem.identity_id 존재 + ix_problem_identity_id 색인 존재.
  ② sentinel problem 행(자체생성·UUID 확보) → downgrade -1 → identity_id·색인 사라지되
     **sentinel 행·UUID PK는 그대로**(PK churn 0) → upgrade head → 컬럼·색인 복귀·UUID 동일.
...
_S4_18_REVISION = "090d254a5d43"  # 이 슬라이스가 추가한 리비전(head)
_PREV_REVISION = "b4c5d6e7f0a2"  # 직전 리비전(downgrade -1 도착점)
...
class TestProblemIdentityIdMigrationRoundtrip:
    """S4-18 리비전 revision-local 왕복 — identity_id 컬럼+색인 추가/제거·UUID PK 보존."""

    def test_head_has_column_and_index(self) -> None:
        _skip_if_unreachable()
        # ① head 상태(CI가 upgrade head 완료): identity_id 컬럼 + 색인 존재.
        assert _column_exists("identity_id")
        assert _index_exists("ix_problem_identity_id")
    ...
    def test_downgrade_then_upgrade_preserves_uuid_pk(self) -> None:
        ...
        try:
            ...
            # ② downgrade -1(S4-18 역적용) — 컬럼·색인 사라지되 행·UUID 보존.
            command.downgrade(cfg, _PREV_REVISION)  # type: ignore[arg-type]
            assert not _column_exists("identity_id")
            assert not _index_exists("ix_problem_identity_id")
            # **핵심**: sentinel 행·UUID PK는 그대로(PK churn 0).
            assert _sentinel_uuid(seeded_uuid) == seeded_uuid

            # ③ upgrade head(재적용) — 컬럼·색인 복귀·UUID 여전히 동일.
