"""problem P3a 메타 필드 마이그레이션 *왕복 통합테스트* — 실 PG (WHYMATH_RUN_INTEGRATION).

마이그레이션 head가 적용된 **실 PostgreSQL**에서 P3a 리비전(c8d9e0f1a2b3)을 *revision-local 왕복*
(downgrade -1 → upgrade head)으로 검증한다. CI `backend — 마이그레이션·통합 (실 PG)` 잡이 이미
`alembic downgrade base → upgrade head` 전구간 왕복을 돌리므로, 이 테스트는 그 위에서 *P3a
리비전이 8 컬럼·2 신규 enum 타입을 정확히 더하고/되돌리며 UUID PK·기존 행을 보존*함을 못 박는다.
PG 미도달 시 graceful skip(`test_concept_source_id_migration_integration.py` 미러).

검증(P3a 핵심 — 가산적·UUID PK 보존):
  ① head에서 problem.{8 신규 컬럼} 존재 + bloom_level_enum·scoring_type_enum PG 타입 존재 +
     question_format_enum에 신규 6 라벨 존재(기존 4 라벨도 보존).
  ② sentinel problem 행(자체생성·UUID 확보) → downgrade -1 → 8 컬럼·2 enum 타입 사라지되
     **sentinel 행·UUID PK는 그대로**(PK churn 0) → upgrade head → 8 컬럼 복귀·UUID 동일.
...
_P3A_REVISION = "c8d9e0f1a2b3"  # 이 슬라이스가 추가한 리비전(head)
_PREV_REVISION = "b7c8d9e0f1a2"  # 직전 리비전(downgrade -1 도착점)
...
def _seed_sentinel() -> uuid.UUID:
    ...
    자체생성 출처·최소 NOT NULL 컬럼만 채운다(source_type·curriculum_version·valid_from_year·
    subject·unit_codes·signature_patterns 등은 server_default 있거나 여기서 명시).
    """
    from sqlalchemy import text
...
class TestProblemP3aMigrationRoundtrip:
    """P3a 리비전 revision-local 왕복 — 8 컬럼·2 enum 추가/제거·UUID PK 보존."""

    def test_head_has_columns_enums_and_new_labels(self) -> None:
        _skip_if_unreachable()
        # ① head 상태(CI가 upgrade head 완료): 8 컬럼 + 2 신규 enum 타입 존재.
        for col in _NEW_COLUMNS:
            assert _column_exists(col), f"{col} 미존재"
        assert _enum_type_exists("bloom_level_enum")
        assert _enum_type_exists("scoring_type_enum")
        # question_format_enum: 기존 4 + 신규 6 라벨 모두 존재(하위호환 보존).
        labels = _enum_labels("question_format_enum")
        assert {"객관식", "단답형", "합답형", "서술형"} <= labels  # 기존 보존
        assert {
            "객관식진단",
            "빈칸형",
            "짝짓기형",
    ...
    def test_downgrade_then_upgrade_preserves_uuid_pk(self) -> None:
        ...
        try:
            ...
            # ② downgrade -1(P3a 역적용) — 8 컬럼·2 enum 타입 사라지되 행·UUID 보존.
            command.downgrade(cfg, _PREV_REVISION)  # type: ignore[arg-type]
            for col in _NEW_COLUMNS:
                assert not _column_exists(col), f"{col} downgrade 후 잔존"
            assert not _enum_type_exists("bloom_level_enum")
            assert not _enum_type_exists("scoring_type_enum")
            # **핵심**: sentinel 행·UUID PK는 그대로(PK churn 0).
            assert _sentinel_uuid(seeded_uuid) == seeded_uuid

            # ③ upgrade head(재적용) — 8 컬럼·2 enum 복귀·UUID 여전히 동일.
            command.upgrade(cfg, "head")  # type: ignore[arg-type]
            for col in _NEW_COLUMNS:
                assert _column_exists(col), f"{col} 재적용 후 미복귀"
            assert _enum_type_exists("bloom_level_enum")
            assert _enum_type_exists("scoring_type_enum")
