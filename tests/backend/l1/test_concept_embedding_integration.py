        ...
        try:
            ...
            try:
                with engine.begin() as conn:  # type: ignore[attr-defined]
                    conn.execute(
                        text(
                            "INSERT INTO concept_embedding "
                            "(concept_id, embedding, provider, model, dim, text_hash) "
                ...
                with engine.connect() as conn:  # type: ignore[attr-defined]
                    subject = conn.execute(
                        text("SELECT subject FROM concept_embedding WHERE concept_id = :k"),
                        {"k": key},
                    ).scalar_one()
                assert subject == "수학"
            finally:
