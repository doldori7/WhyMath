...
_EXPECTED_SCENE_KINDS: frozenset[str] = frozenset(
    {
        ...
        "tutoring_prompt",  # 9블록 Tutoring(LTHC 학생적응·mastery 분기·S5l)
    }
)
...
def test_scene_kind_taxonomy_matches_crosswalk() -> None:
    ...
    focus)**, **Tutoring=`step_panel`+`socratic_prompt`+`tutoring_prompt`(S5l·LTHC 학생적응)**,
    Assessment=schema/assessment+gating, AIExplanation=같은 장면의 렌더 타깃 — 이 중
