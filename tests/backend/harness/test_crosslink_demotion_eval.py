...
class TestCliEndToEnd:
    ...
    def test_report_shows_honest_boundary(self) -> None:
        ...
        # same-standard는 못 잡음(false accept·인간 존치)·커버 15/34
        # (오개념 수치평가 MC 코퍼스가 8 kebab을 machine-decidable로 승격 — 5→8→13→15).
        assert report.same_detected == 0
        assert report.kebabs_decidable == 15 and report.kebabs_total == 34
