...
class TestSummarize:
    ...
    def test_coverage_accounting_uses_real_catalog(self) -> None:
        ...
        assert report.kebabs_total == 52  # 실 카탈로그 52종(843 트랜치1·2·3 포함)
...
class TestCliEndToEnd:
    ...
    def test_report_shows_honest_boundary(self) -> None:
        ...
        # same-standard는 못 잡음(false accept·인간 존치)·커버 52/52
        # (843 트랜치1·2·3 18종 신규 탐지 kebab 포함·전수 machine-decidable → human-only 0).
        assert report.same_detected == 0
        assert report.kebabs_decidable == 52 and report.kebabs_total == 52
