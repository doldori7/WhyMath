"""고등 이차부등식 경계값 파일럿 코퍼스 배치(W2 Phase1 #1) — 결정론·LLM 0.

검증 축(elementary_addsub_batch·matrix_ops_batch·university_calc1_batch·middle_function_
batch·discrete_ev_batch 대칭):
  ① 2밴드(outer/inner) 전건이 수용 게이트(Tier1 산술 검산)를 통과해 적재된다.
  ② 산출 JSONL의 모든 레코드가 `verify.verification_tier=machine_sampled`를 갖는다.
  ③ 재실행 바이트 결정론 — 같은 입력이면 같은 파일.
  ④ **밴드 간 problem_id 중복 0** — 2026-08-05 elementary_addsub 실측 사고(direction/
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    for record in records:
        ...
        assert record["achievement_standard_codes"] == ["[10공수1-02-11]"]
        assert record["concepts"] == [
            {"concept_src_id": "HK16", "role": "PRIMARY", "relevance": 0.95}
        ]
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_quad_ineq_batch(n_per_band=40, out_path=first)
    run_quad_ineq_batch(n_per_band=40, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = QuadraticInequalityBoundarySkeletonGenerator(direction="outer")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(
            _SPEC, QuadraticInequalityBoundarySkeletonGenerator(direction="inner")
        )
        for _ in range(11)
    ]
