"""고등 2×2 행렬 연산 파일럿 코퍼스 배치(W2) — 결정론·LLM 0.

검증 축(elementary_addsub_batch 대칭):
  ① 4밴드(덧셈·뺄셈·스칼라배·곱셈) 전건이 수용 게이트(Tier1 성분 검산)를 통과해 적재된다.
  ② 산출 JSONL의 모든 레코드가 `verify.verification_tier=machine_sampled`를 갖는다.
  ③ 재실행 바이트 결정론 — 같은 입력이면 같은 파일.
  ④ **밴드 간 problem_id 중복 0** — 2026-08-05 elementary_addsub 실측 사고(operation 필터
     누락으로 밴드가 같은 뼈대를 중복 방출)의 재발 방지 회귀 봉인.
...
from whymath_backend.harness.matrix_ops_batch import run_matrix_ops_batch
from whymath_backend.l3.equivalent.acceptance import EquivalenceSpec
from whymath_backend.l3.equivalent.generator import CandidateProblem
from whymath_backend.l3.equivalent.matrix_ops_skeleton_generator import MatrixOpsSkeletonGenerator
from whymath_backend.l3.equivalent.orchestrator import run_equivalent_generation
from whymath_backend.l3.verification_tier import VerificationTier

_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[10공수1-04-02]"}),
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    for record in records:
        ...
        assert record["achievement_standard_codes"] == ["[10공수1-04-02]"]


def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    first = tmp_path / "a.jsonl"
    second = tmp_path / "b.jsonl"
    run_matrix_ops_batch(n_per_band=30, out_path=first)
    run_matrix_ops_batch(n_per_band=30, out_path=second)
    assert first.read_bytes() == second.read_bytes()


def test_gate_rejects_authoring_defect() -> None:
    """**변별력 실측** — 정답을 1 틀리게 만든 생성기는 Tier1 성분 검산에서 거부된다."""

    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = MatrixOpsSkeletonGenerator(operation="add")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_SPEC, MatrixOpsSkeletonGenerator(operation="multiply"))
        for _ in range(11)
    ]
