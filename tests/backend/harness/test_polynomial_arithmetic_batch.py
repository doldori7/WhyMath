...
검증 축(measurement_unit_conversion_batch 대칭):
  ① 5밴드(gongsu_add/gongsu_sub/kisu_add/kisu_sub/kisu_mul) 전건이 수용 게이트(Tier1 정수
     산술 검산)를 통과해 적재된다.
...
_KISU_MUL_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({KISU_MUL_STANDARD_CODE}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.0,
    answer_format=None,
)
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    assert {b.name for b in report.bands} == {
        "arithmetic-gongsu_add",
        "arithmetic-gongsu_sub",
        "arithmetic-kisu_add",
        "arithmetic-kisu_sub",
        "arithmetic-kisu_mul",
    }
    ...
    assert report.total_stored == 165  # 40+40+30+30+25.
    ...
    assert codes_seen == {GONGSU_STANDARD_CODE, KISU_ADDSUB_STANDARD_CODE, KISU_MUL_STANDARD_CODE}
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = PolynomialArithmeticSkeletonGenerator(kind="gongsu_add")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(
            _KISU_MUL_SPEC, PolynomialArithmeticSkeletonGenerator(kind="kisu_mul")
        )
        for _ in range(11)
    ]
