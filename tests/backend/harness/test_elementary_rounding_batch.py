"""초등 어림(올림·버림·반올림) 파일럿 코퍼스 배치(W2 Phase2 #2) — 결정론·LLM 0.

검증 축(elementary_division_remainder_batch·quad_ineq_batch 대칭):
  ① 3밴드(ceil/floor/round) 전건이 수용 게이트(Tier1 산술 검산)를 통과해 적재된다.
...
from whymath_backend.harness.elementary_rounding_batch import run_elementary_rounding_batch
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_elementary_rounding_batch(n_per_band=120, out_path=first)
    run_elementary_rounding_batch(n_per_band=120, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = RoundingSkeletonGenerator(kind="round")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_SPEC, RoundingSkeletonGenerator(kind="ceil")) for _ in range(11)
    ]
