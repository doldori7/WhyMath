"""초등 길이·들이·무게 단위 관계 파일럿 코퍼스 배치(W2 Phase3 #5) — 결정론·LLM 0.

검증 축(elementary_area_measure_batch 대칭):
  ① 4밴드(mm_to_cm/km_to_m/L_to_mL/kg_to_g) 전건이 수용 게이트(Tier1 산술 검산)를 통과해
     적재된다.
...
from whymath_backend.harness.measurement_unit_conversion_batch import (
    run_measurement_unit_conversion_batch,
)
...
_LENGTH_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({LENGTH_STANDARD_CODE}),
    target_misconception_ids=frozenset(),
    difficulty_overall=1.9,
    answer_format=None,
)
_MASS_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({MASS_STANDARD_CODE}),
    target_misconception_ids=frozenset(),
    difficulty_overall=1.9,
    answer_format=None,
)
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_measurement_unit_conversion_batch(
        mm_cm_n=40, km_m_n=15, l_ml_n=20, kg_g_n=20, out_path=out
    )
    ...
    assert report.total_stored == 95  # 40+15+20+20.
    ...
    assert codes_seen == {LENGTH_STANDARD_CODE, VOLUME_STANDARD_CODE, MASS_STANDARD_CODE}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_measurement_unit_conversion_batch(
        mm_cm_n=30, km_m_n=10, l_ml_n=15, kg_g_n=15, out_path=first
    )
    run_measurement_unit_conversion_batch(
        mm_cm_n=30, km_m_n=10, l_ml_n=15, kg_g_n=15, out_path=second
    )
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = MeasurementUnitConversionSkeletonGenerator(pair="mm_to_cm")
    ...
    outcomes = [run_equivalent_generation(_LENGTH_SPEC, BrokenGenerator()) for _ in range(8)]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(
            _MASS_SPEC, MeasurementUnitConversionSkeletonGenerator(pair="kg_to_g")
        )
        for _ in range(11)
    ]
