"""초등 최대공약수·최소공배수 파일럿 코퍼스 배치(W2 Phase2 #3) — 결정론·LLM 0.

검증 축(elementary_rounding_batch·quad_ineq_batch 대칭):
  ① 2밴드(gcd/lcm) 전건이 수용 게이트(Tier1 산술 검산)를 통과해 적재된다.
  ② 산출 JSONL의 모든 레코드가 `verify.verification_tier=machine_sampled`를 갖는다.
  ③ 재실행 바이트 결정론 — 같은 입력이면 같은 파일.
  ④ **밴드 간 problem_id 중복 0**.
  ⑤ **밴드별 표준코드가 정확히 갈린다** — 이 도메인 고유(gcd→[6수01-04]·lcm→[6수01-05]).
...
from whymath_backend.harness.elementary_gcd_lcm_batch import run_elementary_gcd_lcm_batch
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_elementary_gcd_lcm_batch(n_per_band=150, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"gcd", "lcm"}
    ...
    assert codes_seen == {GCD_STANDARD_CODE, LCM_STANDARD_CODE}  # 두 표준코드 모두 실제로 등장.
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_elementary_gcd_lcm_batch(n_per_band=120, out_path=first)
    run_elementary_gcd_lcm_batch(n_per_band=120, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = GcdLcmSkeletonGenerator(kind="gcd")
    ...
    outcomes = [run_equivalent_generation(_GCD_SPEC, BrokenGenerator()) for _ in range(8)]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_LCM_SPEC, GcdLcmSkeletonGenerator(kind="lcm")) for _ in range(11)
    ]
