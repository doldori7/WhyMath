"""초등 넓이 단위 환산·직육면체 겉넓이 파일럿 코퍼스 배치(W2 Phase1 #4) — 결정론·LLM 0.

검증 축(quad_ineq_batch·sequence_sigma_batch·coordinate_geometry_batch 대칭):
  ① 3밴드(conversion-m_to_cm/conversion-km_to_m/surface-area) 전건이 수용 게이트(Tier1
     산술 검산)를 통과해 적재된다.
...
from whymath_backend.harness.elementary_area_measure_batch import (
    run_elementary_area_measure_batch,
)
...
_CONVERSION_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[6수03-12]"}),
    target_misconception_ids=frozenset(),
    difficulty_overall=1.6,
    answer_format=None,
)
_SURFACE_AREA_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[6수03-17]"}),
    target_misconception_ids=frozenset(),
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_elementary_area_measure_batch(conversion_n=15, surface_area_n=40, out_path=out)
    ...
    assert report.total_stored == 70  # 15+15(환산 2밴드) + 40(겉넓이).
    ...
    assert codes_seen == {"[6수03-12]", "[6수03-17]"}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_elementary_area_measure_batch(conversion_n=10, surface_area_n=30, out_path=first)
    run_elementary_area_measure_batch(conversion_n=10, surface_area_n=30, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = RectangularPrismSurfaceAreaSkeletonGenerator()
    ...
    outcomes = [run_equivalent_generation(_SURFACE_AREA_SPEC, BrokenGenerator()) for _ in range(8)]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_CONVERSION_SPEC, AreaUnitConversionSkeletonGenerator())
        for _ in range(11)
    ]
