"""초등 두 자리 나눗셈(몫·나머지) 파일럿 코퍼스 배치(W2 Phase2 #1) — 결정론·LLM 0.

검증 축(quad_ineq_batch·elementary_area_measure_batch 대칭):
  ① 2밴드(quotient/remainder) 전건이 수용 게이트(Tier1 산술 검산)를 통과해 적재된다.
...
from whymath_backend.harness.elementary_division_remainder_batch import (
    run_elementary_division_remainder_batch,
)
...
_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[4수01-07]"}),
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_elementary_division_remainder_batch(n_per_band=150, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"quotient", "remainder"}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_elementary_division_remainder_batch(n_per_band=120, out_path=first)
    run_elementary_division_remainder_batch(n_per_band=120, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = TwoDigitDivisionSkeletonGenerator(kind="quotient")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_SPEC, TwoDigitDivisionSkeletonGenerator(kind="remainder"))
        for _ in range(11)
    ]
