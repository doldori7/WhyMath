"""수열의 합 — Σ 성질·여러 가지 수열의 합 파일럿 코퍼스 배치(W2 Phase1 #2) — 결정론·LLM 0.

검증 축(quad_ineq_batch·elementary_addsub_batch·matrix_ops_batch 대칭):
  ① 3밴드(sigma-linear/power-square/power-cube) 전건이 수용 게이트(Tier1 산술 검산)를
     통과해 적재된다.
  ② 산출 JSONL의 모든 레코드가 `verify.verification_tier=machine_sampled`를 갖는다.
  ③ 재실행 바이트 결정론 — 같은 입력이면 같은 파일.
  ④ **밴드 간 problem_id 중복 0** — 2026-08-05 elementary_addsub 실측 사고(kind/direction/
     operation 필터 누락으로 밴드가 같은 뼈대를 중복 방출)의 재발 방지 회귀 봉인.
...
from whymath_backend.harness.sequence_sigma_batch import run_sequence_sigma_batch
...
_SIGMA_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[12대수03-04]"}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.0,
    answer_format=None,
)
_POWER_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[12대수03-05]"}),
    target_misconception_ids=frozenset(),
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_sequence_sigma_batch(sigma_n=40, power_n=10, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"sigma-linear", "power-square", "power-cube"}
    assert all(not b.failure_reasons for b in report.bands), [
        b.failure_reasons for b in report.bands
    ]
    assert report.total_stored == 60  # 40(sigma) + 10(square) + 10(cube).
    ...
    assert codes_seen == {"[12대수03-04]", "[12대수03-05]"}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_sequence_sigma_batch(sigma_n=30, power_n=8, out_path=first)
    run_sequence_sigma_batch(sigma_n=30, power_n=8, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = SigmaLinearitySkeletonGenerator()
    ...
    outcomes = [run_equivalent_generation(_SIGMA_SPEC, BrokenGenerator()) for _ in range(8)]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_POWER_SPEC, PowerSumSkeletonGenerator(kind="square"))
        for _ in range(11)
    ]
