"""고등 중복조합·이항정리 계수 파일럿 코퍼스 배치(W2 Phase3 #2) — 결정론·LLM 0.

검증 축(elementary_volume_measure_batch 대칭):
  ① 2밴드(repeated-combination/binomial-coefficient) 전건이 수용 게이트(Tier1 산술 검산)를
     통과해 적재된다.
...
_BINOMIAL_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[12확통01-03]"}),
    target_misconception_ids=frozenset(),
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_combination_binomial_batch(repeated_combination_n=56, binomial_n=65, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"repeated-combination", "binomial-coefficient"}
    assert all(not b.failure_reasons for b in report.bands), [
        b.failure_reasons for b in report.bands
    ]
    assert report.total_stored == 121  # 56(중복조합) + 65(이항정리).
    ...
    assert codes_seen == {"[12확통01-02]", "[12확통01-03]"}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_combination_binomial_batch(repeated_combination_n=30, binomial_n=30, out_path=first)
    run_combination_binomial_batch(repeated_combination_n=30, binomial_n=30, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = RepeatedCombinationSkeletonGenerator()
    ...
    outcomes = [
        run_equivalent_generation(_REPEATED_COMBINATION_SPEC, BrokenGenerator()) for _ in range(8)
    ]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_BINOMIAL_SPEC, BinomialCoefficientSkeletonGenerator())
        for _ in range(11)
    ]
