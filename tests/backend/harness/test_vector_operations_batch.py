"""고등 벡터(선형결합·위치벡터 성분) 파일럿 코퍼스 배치(W2 Phase1 #5) — 결정론·LLM 0.

검증 축(quad_ineq_batch·sequence_sigma_batch·coordinate_geometry_batch 대칭):
  ① 2밴드(linear-combo/position-vector) 전건이 수용 게이트(Tier1 산술 검산)를 통과해
     적재된다.
...
from whymath_backend.harness.vector_operations_batch import run_vector_operations_batch
...
_LINEAR_COMBO_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[12기하03-01]"}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.1,
    answer_format=None,
)
_POSITION_VECTOR_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[12기하03-02]"}),
    target_misconception_ids=frozenset(),
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_vector_operations_batch(n_per_band=150, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"linear-combo", "position-vector"}
    ...
    assert codes_seen == {"[12기하03-01]", "[12기하03-02]"}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_vector_operations_batch(n_per_band=120, out_path=first)
    run_vector_operations_batch(n_per_band=120, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = VectorLinearCombinationSkeletonGenerator()
    ...
    outcomes = [run_equivalent_generation(_LINEAR_COMBO_SPEC, BrokenGenerator()) for _ in range(8)]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_POSITION_VECTOR_SPEC, PositionVectorComponentSkeletonGenerator())
        for _ in range(11)
    ]
