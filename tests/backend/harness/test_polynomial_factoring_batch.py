...
검증 축(polynomial_arithmetic_batch 대칭):
  ① 2밴드(gongsu_factor/kisu_factor) 전건이 수용 게이트(Tier1 정수 산술 검산)를 통과해
     적재된다.
...
from whymath_backend.harness.polynomial_factoring_batch import (
    run_polynomial_factoring_batch,
)
...
_GONGSU_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({GONGSU_FACTOR_STANDARD_CODE}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.2,
    answer_format=None,
)
_KISU_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({KISU_FACTOR_STANDARD_CODE}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.2,
    answer_format=None,
)
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_polynomial_factoring_batch(gongsu_factor_n=60, kisu_factor_n=50, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {
        "factoring-gongsu_factor",
        "factoring-kisu_factor",
    }
    ...
    assert codes_seen == {GONGSU_FACTOR_STANDARD_CODE, KISU_FACTOR_STANDARD_CODE}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_polynomial_factoring_batch(gongsu_factor_n=30, kisu_factor_n=25, out_path=first)
    run_polynomial_factoring_batch(gongsu_factor_n=30, kisu_factor_n=25, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = PolynomialFactoringSkeletonGenerator(kind="gongsu_factor")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(
            _KISU_SPEC, PolynomialFactoringSkeletonGenerator(kind="kisu_factor")
        )
        for _ in range(11)
    ]
