...
검증 축(polynomial_factoring_batch 대칭):
  ① 2밴드(gongsu_sysineq/kisu_sysineq) 전건이 수용 게이트(Tier1 정수 산술 검산)를
     통과해 적재된다.
...
from whymath_backend.harness.linear_inequality_system_batch import (
    run_linear_inequality_system_batch,
)
...
_GONGSU_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({GONGSU_SYSINEQ_STANDARD_CODE}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.3,
    answer_format=None,
)
_KISU_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({KISU_SYSINEQ_STANDARD_CODE}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.3,
    answer_format=None,
)
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_linear_inequality_system_batch(gongsu_n=60, kisu_n=50, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"sysineq-gongsu", "sysineq-kisu"}
    assert all(not b.failure_reasons for b in report.bands), [
        b.failure_reasons for b in report.bands
    ]
    assert report.total_stored == 110  # 60+50.
    ...
    assert codes_seen == {GONGSU_SYSINEQ_STANDARD_CODE, KISU_SYSINEQ_STANDARD_CODE}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_linear_inequality_system_batch(gongsu_n=30, kisu_n=25, out_path=first)
    run_linear_inequality_system_batch(gongsu_n=30, kisu_n=25, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = LinearInequalitySystemSkeletonGenerator(kind="gongsu_sysineq")
    ...
    outcomes = [run_equivalent_generation(_GONGSU_SPEC, BrokenGenerator()) for _ in range(8)]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(
            _KISU_SPEC, LinearInequalitySystemSkeletonGenerator(kind="kisu_sysineq")
        )
        for _ in range(11)
    ]
