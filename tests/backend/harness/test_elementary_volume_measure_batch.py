"""초등 직육면체 부피·부피 단위·원주 파일럿 코퍼스 배치(W2 Phase3 #1) — 결정론·LLM 0.

검증 축(elementary_area_measure_batch 대칭):
  ① 3밴드(volume/volume-unit/circumference) 전건이 수용 게이트(Tier1 산술 검산)를 통과해
     적재된다.
...
from whymath_backend.harness.elementary_volume_measure_batch import (
    run_elementary_volume_measure_batch,
)
...
_VOLUME_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[6수03-19]"}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.0,
    answer_format=None,
)
_VOLUME_UNIT_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[6수03-18]"}),
    target_misconception_ids=frozenset(),
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_elementary_volume_measure_batch(
        volume_n=40, volume_unit_n=15, circumference_n=15, out_path=out
    )

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"volume", "volume-unit", "circumference"}
    assert all(not b.failure_reasons for b in report.bands), [
        b.failure_reasons for b in report.bands
    ]
    assert report.total_stored == 70  # 40(부피) + 15(부피단위) + 15(원주).
    ...
    assert codes_seen == {"[6수03-19]", "[6수03-18]", "[6수03-15]"}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_elementary_volume_measure_batch(
        volume_n=30, volume_unit_n=10, circumference_n=10, out_path=first
    )
    run_elementary_volume_measure_batch(
        volume_n=30, volume_unit_n=10, circumference_n=10, out_path=second
    )
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = RectangularPrismVolumeSkeletonGenerator()
    ...
    outcomes = [run_equivalent_generation(_VOLUME_SPEC, BrokenGenerator()) for _ in range(8)]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_VOLUME_UNIT_SPEC, VolumeUnitConversionSkeletonGenerator())
        for _ in range(11)
    ]
