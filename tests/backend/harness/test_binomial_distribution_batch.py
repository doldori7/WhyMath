"""이항분포 평균·분산 파일럿 코퍼스 배치(W2 Phase1 #8) — 결정론·LLM 0.

검증 축(discrete_ev_batch·quad_ineq_batch 대칭):
...
from whymath_backend.harness.binomial_distribution_batch import run_binomial_distribution_batch
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_binomial_distribution_batch(n_per_band=100, out_path=out)
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_binomial_distribution_batch(n_per_band=80, out_path=first)
    run_binomial_distribution_batch(n_per_band=80, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = BinomialDistributionSkeletonGenerator(kind="mean")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_SPEC, BinomialDistributionSkeletonGenerator(kind="variance"))
        for _ in range(11)
    ]
