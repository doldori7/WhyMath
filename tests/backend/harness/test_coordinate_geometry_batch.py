"""좌표평면(두 점 거리·직선의 방정식) 파일럿 코퍼스 배치(W2 Phase1 #3) — 결정론·LLM 0.

검증 축(quad_ineq_batch·sequence_sigma_batch 대칭):
  ① 2밴드(distance/line) 전건이 수용 게이트(Tier1 산술 검산)를 통과해 적재된다.
...
from whymath_backend.harness.coordinate_geometry_batch import run_coordinate_geometry_batch
...
_DISTANCE_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[10공수2-01-01]"}),
    target_misconception_ids=frozenset(),
    difficulty_overall=2.0,
    answer_format=None,
)
_LINE_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[10공수2-01-02]"}),
    target_misconception_ids=frozenset(),
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_coordinate_geometry_batch(n_per_band=150, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"distance", "line"}
    ...
    assert codes_seen == {"[10공수2-01-01]", "[10공수2-01-02]"}
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_coordinate_geometry_batch(n_per_band=120, out_path=first)
    run_coordinate_geometry_batch(n_per_band=120, out_path=second)
...
def test_gate_rejects_authoring_defect() -> None:
    ...
    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = PointDistanceSkeletonGenerator()
    ...
    outcomes = [run_equivalent_generation(_DISTANCE_SPEC, BrokenGenerator()) for _ in range(8)]
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_LINE_SPEC, LineEquationSkeletonGenerator()) for _ in range(11)
    ]
