"""표본평균의 평균·분산 파일럿 코퍼스 배치(W2 Phase3 #4) — 결정론·LLM 0.

검증 축(binomial_distribution_batch 대칭):
  ① 2밴드(mean/variance) 전건이 수용 게이트(Tier1 유리수 산술 검산)를 통과해 적재된다.
...
from whymath_backend.harness.sample_mean_distribution_batch import (
    run_sample_mean_distribution_batch,
)
...
_SPEC = EquivalenceSpec(
    achievement_standard_codes=frozenset({"[12확통03-06]"}),
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_sample_mean_distribution_batch(n_per_band=80, out_path=first)
    run_sample_mean_distribution_batch(n_per_band=80, out_path=second)
    assert first.read_bytes() == second.read_bytes()


def test_gate_rejects_authoring_defect() -> None:
    """**변별력 실측** — 정답을 1 틀리게 만든 생성기는 Tier1 유리수 검산에서 거부된다."""

    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = SampleMeanDistributionSkeletonGenerator(kind="mean")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_SPEC, SampleMeanDistributionSkeletonGenerator(kind="variance"))
        for _ in range(11)
    ]
