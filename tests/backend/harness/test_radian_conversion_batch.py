...
검증 축(polynomial_arithmetic_batch·polynomial_factoring_batch 대칭):
  ① 2밴드(deg_to_rad/rad_to_deg) 전건이 수용 게이트(Tier1 수치 검산 — pi 상수 직접 평가)를
     통과해 적재된다.
  ...
  ⑥ 산출물이 항상 `is_published=False`이고 두 밴드가 같은 성취기준 코드를 공유한다.
"""
...
from whymath_backend.harness.radian_conversion_batch import run_radian_conversion_batch
...
def test_batch_stores_all_bands_and_stamps_tier(tmp_path: Path) -> None:
    ...
    report = run_radian_conversion_batch(deg_to_rad_n=40, rad_to_deg_n=35, out_path=out)

    assert report.total_stored == report.written and report.written is not None
    assert {b.name for b in report.bands} == {"radian-deg_to_rad", "radian-rad_to_deg"}
    assert all(not b.failure_reasons for b in report.bands), [
        b.failure_reasons for b in report.bands
    ]
    assert report.total_stored == 75  # 40+35.
    ...
    assert codes_seen == {RADIAN_STANDARD_CODE}  # 두 밴드가 코드 하나 공유.
...
def test_batch_is_byte_deterministic(tmp_path: Path) -> None:
    ...
    run_radian_conversion_batch(deg_to_rad_n=20, rad_to_deg_n=18, out_path=first)
    run_radian_conversion_batch(deg_to_rad_n=20, rad_to_deg_n=18, out_path=second)
    assert first.read_bytes() == second.read_bytes()


def test_gate_rejects_authoring_defect() -> None:
    """**변별력 실측** — 정답을 조작한 생성기는 Tier1 검산에서 거부된다."""

    class BrokenGenerator:
        def __init__(self) -> None:
            self._inner = RadianConversionSkeletonGenerator(kind="rad_to_deg")
...
def test_healthy_generator_is_accepted_for_contrast() -> None:
    ...
    outcomes = [
        run_equivalent_generation(_SPEC, RadianConversionSkeletonGenerator(kind="deg_to_rad"))
        for _ in range(11)
    ]
