"""PED-18 교수전략 카탈로그 스키마 동결 + 불변식 — 필드셋·폐쇄 어휘·제거 필드 3종 부재 lock.

정본: `docs/architecture/04f_pedagogy_strategy_catalog.md` §3.2(필드 소비처 지정표)·§3.3(제거
...
세션에서 재제안되어 스키마에 스며드는 것을 기계로 막는다(04f §8 영구 기록과 이중 방어).
...
# ── 동결 리터럴 (PED-18 시점 — 변경은 "소비처 실재 증명" 전제·04f §3.2) ──
_CARD_FIELDS = {
    "strategy",
    "name_ko",
    "description",
...
# 제거 필드 3계열(04f §3.3) — model_fields 어느 이름에도 이 어간이 나타나면 안 된다.
...
class TestSchemaFieldFreeze:
    """필드셋 리터럴 동결 + 제거 필드 3계열 부재 동결(04f §3.3)."""
    ...
    def test_removed_field_stems_absent(self) -> None:
        # 부재의 동결 — 제거 3계열 어간이 어떤 필드명에도 나타나지 않는다(04f §3.3).
