"""학습 공급 엔드포인트 관통 — study → outcome → 집계 + SEC-24(원 SEC-16) 소유권 (실 PG·기본 SKIP).
...
받치면 부족하다(단위는 writer가 올바른 행을 *만든다*까지만 보인다). 여기서는 실 PG에 실제로 왕복해:
  ...
  ④ **SEC-24(원 SEC-16)**: 위조·타인 세션의 outcome이 실 스키마·실 HTTP 경로에서 403으로, 중복이 409로
...
def _settings() -> Settings:
    """SEC-24(원 SEC-16) 바인딩 HMAC 키(jwt_secret)를 채운 설정 — writer·앱(오버라이드) 양쪽이 공유."""
...
def test_pedagogy_evidence_round_trip_and_aggregation_on_live_pg() -> None:
    ...
    (JSONB meta 왕복·복합 PK·이벤트 유형 필터·SEC-24(원 SEC-16) 소유권 SELECT), 엔드포인트 계약은
    hermetic 단위가 담당한다. outcome은 같은 트랜잭션의 처치를 autoflush로 되읽어 소유권을
    통과한다(정당 경로가 SEC-24(원 SEC-16) 이후에도 계속 성립함의 실측).
    """
...
def test_orphan_outcome_is_rejected_and_excluded_on_live_pg() -> None:
    """**변별력** — 처치 없는 outcome은 ⓐ writer가 기입 자체를 거부하고(SEC-24(원 SEC-16) 승격),
...
def test_outcome_ownership_http_on_live_pg() -> None:
    """SEC-24(원 SEC-16) HTTP 관통 — 위조 403 · 타 학생 403(동일 detail) · 정당 201 · 중복 409 (실 PG).
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            # ③ 정당 소유자 → 201(기존 study→outcome 흐름이 SEC-24(원 SEC-16) 이후에도 성립).
