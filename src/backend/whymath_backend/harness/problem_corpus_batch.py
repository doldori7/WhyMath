...
def _attach_similar_relations(
    ...
    new_tags_by_index: dict[int, tuple[RelationTag, ...]] = {}
    for members in groups.values():
        ...
        for pos in range(1, len(ordered)):
            ...
            new_tags_by_index[idx] = record.relation_tags + back_edges

    return [
        replace(record, relation_tags=new_tags_by_index.get(idx, record.relation_tags))
        for idx, record in enumerate(records)
    ]
