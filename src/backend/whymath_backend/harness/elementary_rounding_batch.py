"""초등 어림(올림·버림·반올림) 파일럿 코퍼스 배치 — W2 Phase 2 #2(순수 결정론·LLM 0).

`elementary_division_remainder_batch` 형제. `RoundingSkeletonGenerator`로 3밴드
(ceil/floor/round·`[6수01-03]`)를 생성 → 수용 게이트(`l3/verify_answer` Tier1 경유)·
...
CLI: `python -m whymath_backend.harness.elementary_rounding_batch [--n N] [--out PATH]
[--dry-run]`
"""
...
__all__ = ["CORPUS_DIR_NAME", "run_elementary_rounding_batch"]

CORPUS_DIR_NAME = "problem_bank_elementary_rounding_v0"
_STANDARD_CODE = "[6수01-03]"
_SPEC_DIFFICULTY = 1.5
_DEFAULT_N = 200  # 밴드당 요청 수(풀 목표 300보다 작게 — 풀 소진 없이 안정적으로 채움).
...
def run_elementary_rounding_batch(
    *, n_per_band: int = _DEFAULT_N, out_path: Path | None = None, write: bool = True
) -> CorpusBatchReport:
    """3밴드(ceil/floor/round) 배치 실행 — 생성→게이트(Tier1)→적재."""
    ...
    kinds: tuple[RoundingKind, ...] = ("ceil", "floor", "round")
    bands = [
        _run_band(
            name=kind,
            spec=spec,
            generator=RoundingSkeletonGenerator(kind=kind),
            n=n_per_band,
            sink=sink,
        )
        for kind in kinds
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m whymath_backend.harness.elementary_rounding_batch",
        description="초등 어림(올림·버림·반올림) 파일럿 코퍼스 배치(3밴드·결정론·Tier1).",
    )
    parser.add_argument("--n", type=int, default=_DEFAULT_N, help="밴드당 요청 수(기본 200).")
    parser.add_argument("--out", default=None, help="출력 코퍼스 경로(기본 rounding_v0).")
    parser.add_argument("--dry-run", action="store_true", help="파일 기록 없이 밴드별 수율만 출력.")
    args = parser.parse_args(argv)

    report = run_elementary_rounding_batch(
        n_per_band=args.n,
