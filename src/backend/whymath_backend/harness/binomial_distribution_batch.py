"""고등 이항분포 평균·분산 파일럿 코퍼스 배치 — W2 Phase 1 #8(순수 결정론·LLM 0).

`discrete_ev_batch`(이산확률변수 기댓값) 형제. `BinomialDistributionSkeletonGenerator`로
2밴드(mean/variance·`[12확통03-03]`)를 생성 → 수용 게이트(`l3/verify_answer` Tier1 경유)·
`JsonlCorpusSink`를 **재사용**해 파일럿 코퍼스를 적재한다. 이 성취기준의 첫 코퍼스이자
W2 Phase 1 목록의 마지막 항목이다.

`kind` 필터 필수(`quad_ineq_batch`의 `direction` 관례 계승) — 미지정이면 두 밴드가 고정
시드 풀을 처음부터 재생해 같은 뼈대를 중복 방출한다. `run_equivalent_generation`을
`signature_index=None`으로 호출한다 — 형제 배치와 동일 이유(구조 dedup이 조건식을 해값으로
정규화하므로 서로 다른 (n,p) 조합이 같은 평균/분산을 내면 오탐).
...
CLI: `python -m whymath_backend.harness.binomial_distribution_batch [--n N] [--out PATH]
[--dry-run]`
"""
...
__all__ = ["CORPUS_DIR_NAME", "run_binomial_distribution_batch"]

CORPUS_DIR_NAME = "problem_bank_binomial_distribution_v0"
_STANDARD_CODE = "[12확통03-03]"
_SPEC_DIFFICULTY = 3.0  # mean(3.0)/variance(3.3) 평균과 정합(허용격차 0.5 이내).
_DEFAULT_N = 150  # 밴드당 요청 수(풀 목표 198보다 작게 — 풀 소진 없이 안정적으로 채움).
...
def run_binomial_distribution_batch(
    *, n_per_band: int = _DEFAULT_N, out_path: Path | None = None, write: bool = True
    ...
    kinds: tuple[BinomialStatKind, ...] = ("mean", "variance")
    bands = [
        _run_band(
            name=kind,
            spec=spec,
            generator=BinomialDistributionSkeletonGenerator(kind=kind),
            n=n_per_band,
            sink=sink,
        )
        for kind in kinds
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m whymath_backend.harness.binomial_distribution_batch",
        description="이항분포 평균·분산 파일럿 코퍼스 배치(2밴드·결정론·Tier1 유리수 검산).",
    )
    parser.add_argument("--n", type=int, default=_DEFAULT_N, help="밴드당 요청 수(기본 150).")
    parser.add_argument("--out", default=None, help="출력 코퍼스 경로(기본 binomial_v0).")
    parser.add_argument("--dry-run", action="store_true", help="파일 기록 없이 밴드별 수율만 출력.")
    args = parser.parse_args(argv)

    report = run_binomial_distribution_batch(
        n_per_band=args.n,
