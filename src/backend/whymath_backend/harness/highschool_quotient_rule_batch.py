"""고등 미적분Ⅱ 몫의 미분법 파일럿 코퍼스 배치 — W2 Phase 2 #5·최종(순수 결정론·LLM 0).

`university_calc1_chain_quotient_batch`(대학 CALC1 축) 형제. `HighschoolQuotientRuleSkeleton
Generator`로 단일 변형(`[12미적Ⅱ-02-04]`)을 생성 → 수용 게이트(`l3/verify_answer` Tier1
경유)·`JsonlCorpusSink`를 **재사용**해 파일럿 코퍼스를 적재한다. 이 성취기준의 첫 코퍼스다.

`run_equivalent_generation`을 `signature_index=None`으로 직접 호출한다 — 형제 배치와 동일
이유(구조 dedup이 조건식을 해값으로 정규화하므로 서로 다른 다항식 조합이 같은 f'(k) 값을
내면 오탐). 단일 변형이라 밴드 분리도 불요.
...
CLI: `python -m whymath_backend.harness.highschool_quotient_rule_batch [--n N] [--out PATH]
[--dry-run]`
"""
...
__all__ = ["CORPUS_DIR_NAME", "run_highschool_quotient_rule_batch"]

CORPUS_DIR_NAME = "problem_bank_highschool_quotient_rule_v0"
_STANDARD_CODE = "[12미적Ⅱ-02-04]"
_SPEC_DIFFICULTY = 3.7  # 생성기가 못 박는 난이도(모든 후보 동일)와 정합(허용격차 0.5 이내).
_BAND_NAME = "quotient-rule"
...
def run_highschool_quotient_rule_batch(
    *, n: int = _DEFAULT_N, out_path: Path | None = None, write: bool = True
) -> CorpusBatchReport:
    """단일 밴드(몫의 미분법) 배치 실행 — 생성(시드 풀)→게이트(Tier1 미분 평가 검산)→적재."""
    ...
    seen: set[str] = set()
    generator = HighschoolQuotientRuleSkeletonGenerator(skip_conditions=seen)
    outcomes = [
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m whymath_backend.harness.highschool_quotient_rule_batch",
        description="고등 미적분Ⅱ 몫의 미분법 파일럿 코퍼스 배치(결정론·Tier1 미분 검산).",
    )
    parser.add_argument("--n", type=int, default=_DEFAULT_N, help="요청 수(기본 200).")
    parser.add_argument("--out", default=None, help="출력 코퍼스 경로(기본 quotient_rule_v0).")
    parser.add_argument("--dry-run", action="store_true", help="파일 기록 없이 수율만 출력.")
    args = parser.parse_args(argv)

    report = run_highschool_quotient_rule_batch(
        n=args.n,
