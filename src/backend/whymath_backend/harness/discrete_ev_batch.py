"""고등 이산확률변수 기댓값 파일럿 코퍼스 배치 — W2 통계 영역 착점(순수 결정론·LLM 0).

`elementary_addsub_batch`·`matrix_ops_batch`·`university_calc1_batch`·`middle_function_batch`
형제. `DiscreteExpectedValueSkeletonGenerator`로 단일 변형(기댓값·`[12확통03-02]`)을
생성 → 수용 게이트(`l3/verify_answer` Tier1 경유)·`JsonlCorpusSink`를 **재사용**해 파일럿
코퍼스를 적재한다. 고등 "통계" 영역(7코드 전량 0%)의 첫 코퍼스다.

`run_equivalent_generation`을 `signature_index=None`으로 직접 호출한다 — 선행 네 생성기와
동일 이유(구조 dedup이 조건식을 해값으로 정규화하므로 서로 다른 확률분포가 같은 E(X)를
내면 판박이로 오탐한다). 단일 변형이라 밴드 분리도 불요.

**검증 등급 각인**: `machine_sampled`(Tier1 유리수 산술 검산 — `discrete_expected_value_
...
CLI: `python -m whymath_backend.harness.discrete_ev_batch [--n N] [--out PATH] [--dry-run]`
"""
...
__all__ = ["CORPUS_DIR_NAME", "run_discrete_ev_batch"]

CORPUS_DIR_NAME = "problem_bank_discrete_ev_v0"
_STANDARD_CODE = "[12확통03-02]"
_SPEC_DIFFICULTY = 2.8  # 생성기가 못 박는 난이도(모든 후보 동일)와 정합(허용격차 0.5 이내).
_BAND_NAME = "expected-value"
...
def run_discrete_ev_batch(
    *, n: int = _DEFAULT_N, out_path: Path | None = None, write: bool = True
) -> CorpusBatchReport:
    """단일 밴드(기댓값) 배치 실행 — 생성(시드 풀)→게이트(Tier1 유리수 검산)→적재."""
    ...
    seen: set[str] = set()
    generator = DiscreteExpectedValueSkeletonGenerator(skip_conditions=seen)
    outcomes = [
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m whymath_backend.harness.discrete_ev_batch",
        description="고등 이산확률변수 기댓값 파일럿 코퍼스 배치(결정론·Tier1 유리수 검산).",
    )
    parser.add_argument("--n", type=int, default=_DEFAULT_N, help="요청 수(기본 200).")
    parser.add_argument("--out", default=None, help="출력 코퍼스 경로(기본 discrete_ev_v0).")
    parser.add_argument("--dry-run", action="store_true", help="파일 기록 없이 수율만 출력.")
    args = parser.parse_args(argv)

    report = run_discrete_ev_batch(
        n=args.n,
