...
SEC-16이 처치 행 `meta`에 추가한 것은 *키드 해시 바인딩*(`META_KEY_USER_BINDING`)뿐이다 —
...
# SEC-16: 처치 행에 저장되는 세션↔사용자 키드 해시 바인딩. 집계(effectiveness)는 이 키를
...
def session_user_binding(session_id: uuid.UUID, user_id: uuid.UUID, *, settings: Settings) -> str:
    """세션↔사용자 바인딩 키드 해시(HMAC-SHA256 hexdigest·64자) — SEC-16 소유권 대조 축.
...
async def record_pedagogy_treatment(
    ...
    **실제 렌더된 전략**(강등 결과)을 `strategy`로 기록한다(학생이 본 것이 처치다).
    ...
    SEC-16: `meta`에 세션↔사용자 키드 해시 바인딩(`META_KEY_USER_BINDING`)을 함께 남긴다 —
    `/outcome` 소유권 대조의 앵커다. raw `user_id`가 아니라 HMAC 해시라 가명화가 유지된다.
    """
    meta: dict[str, Any] = {
...
async def record_pedagogy_outcome(
    ...
    *,
    ...
    SEC-16: stage 전에 소유권·중복을 검증한다. **이 함수가 outcome의 유일한 writer**이므로
    검증을 여기(안쪽)에 두어야 우회가 구조적으로 불가하다(PED-06 "정본화 ≠ 집행" 교훈 —
    계약을 API 층에만 두면 새 호출자가 생길 때 조용히 뚫린다). 검증 실패 시:
