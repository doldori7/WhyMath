"""학생 *최종답* ↔ 기대정답(`Problem.answer`) 동치 검증 — 서버 권위 3상태 (S3-09).

배경(학습 루프 닫힘의 서버 권위 축·G2): 실기기에서 "정답을 내도 다음 문항으로 안 넘어감"의
근본원인은 `ProblemAttempt`가 한 번도 생성되지 않아 `next-problem`의 미시도 필터가 무발동한 것
이었고, 부수로 `submit_attempt`가 정답 여부를 *클라이언트 보고*로 신뢰했다. 후자는 CLAUDE.md
절대 원칙 "수학 로직을 클라에 넣지 않는다"(독립 수학 코어가 권위) 위반이다. 이 모듈은 그 판정을
*서버(L3 코어)*로 끌어온다 — 학생의 최종답이 문항 기대정답과 동치인지 **3상태**로 판정한다:
...
class FinalAnswerState(str, Enum):
    ...
    클라이언트는 이 값을 응답에서 읽어 "correct일 때만 다음 문항 진행"을 판단한다.
    """

    correct = "correct"
    """학생 최종답이 기대정답과 동치임을 서버가 결정론으로 확인 — is_correct=True 승격 근거."""

    incorrect = "incorrect"
    """학생 최종답이 기대정답과 *불일치*함을 서버가 확인 — is_correct=False(클라 보고가 못 이김)."""
...
def verify_final_answer(
    ...
    서버 권위 채점의 L3 primitive(S3-09). 입력은 학생 최종답 문자열과 문항(기대정답·선택지·형식
    보유)이다. 판정 흐름:
