...
PED-09(04e §6)로 LLM 생성 경로가 열렸다: `analogy_generator`(개념-grain 비유 —
`concept_content.metaphor` 채움)·`example_generator`(목표-grain 실생활 예시 — 슬롯 채움)·
`analogy_checker`(결함 4축 규칙 검출)·`analogy_demand`(발주 수요 실측 CLI). 검수 상태기계
(prescreen→review)는 두 생성기가 재사용한다(새 상태기계 없음).
