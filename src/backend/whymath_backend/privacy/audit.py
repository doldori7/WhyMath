"""개인정보 감사(SEC-09) — 반출·동의변경·관리자접근 3종 writer + IP 해싱.
...
프로파일링 자산화하고 볼륨도 소음이라 의도적으로 제외한다. 감사 대상은 "시스템 밖으로 나가는
사건"과 "본인 아닌 주체의 접근" 3종뿐:
...
**commit은 호출자**(엔드포인트 트랜잭션과 합류) — 감사 행과 주행위(반출/동의기록)가 같은
트랜잭션으로 원자적이어야 하기 때문이다(하나만 성공하는 부분 실패 방지). `deletion_audit`는
