"""임베딩 제공자 *구현체·팩토리* — 레이어-중립 L1 인프라(감사 I4 layer 단방향).

`EmbeddingProvider` Protocol(추상)은 `l1/embedding_primitives.py`가 소유하고, 이 모듈은 그
구현체(`FakeEmbeddingProvider`·`LocalEmbeddingProvider`·`OpenAIEmbeddingProvider`)와 팩토리
(`build_provider`)·`cosine_similarity`를 담는다. **이전에는 이 심볼들이 L4
(`l4/misconception/semantic/provider.py`)에 살아서 L1 적재기(`l1/concept_graph/populate.py`·
`l1/misconception/crosslink_candidates.py`)가 L4를 *위로* import하는 역방향 의존이 있었다** —
본 모듈이 그 인프라를 L1으로 내려 위반을 없앤다(의존성 역전: 추상·구현체 모두 하위 중립 레이어,
소비는 상위). `config`+stdlib+지연 import에만 의존하므로 어떤 상위 L계층도 import하지 않는다
(순환 불가). 기존 `...semantic.provider` import 경로는 그 자리의 얇은 re-export shim이 유지한다.
...
제공자는 recall 향상만 제공하고 방향 판별을 해결하지 않는다.
...
# (의존성 역전 — 추상은 하위, 구현체는 이 모듈). 여기서 re-export해 기존 import 경로
# (`...provider import EmbeddingProvider`·matcher/combined)를 유지한다.
