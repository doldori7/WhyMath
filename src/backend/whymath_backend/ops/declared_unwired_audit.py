...
_MANIFEST: dict[str, dict[str, str]] = {
    ...
    AXIS_HTTP: {
        ...
        "GET /v1/auth/sessions": "pending-task:MOB-10-auth-session-lifecycle-client-wiring",
        "DELETE /v1/auth/sessions": "pending-task:MOB-10-auth-session-lifecycle-client-wiring",
        "DELETE /v1/auth/sessions/{param}": (
            "pending-task:MOB-10-auth-session-lifecycle-client-wiring"
        ),
        "POST /v1/auth/refresh": "pending-task:MOB-10-auth-session-lifecycle-client-wiring",
        "POST /v1/auth/logout": "pending-task:MOB-10-auth-session-lifecycle-client-wiring",
        ...
        "POST /v1/me/objectives/{param}/study": (
            "pending-task:MOB-11-content-supply-loop-client-wiring"
        ),
        "POST /v1/me/objectives/{param}/outcome": (
            "pending-task:MOB-11-content-supply-loop-client-wiring"
        ),
        # OCR 다중 페이지 변형 — 단일 페이지 POST /v1/ocr는 이미 모바일이 호출(reached).
        # /pages는 NLP-01(OCR 도달 관측, todo)이 정확히 이 축을 추적한다.
