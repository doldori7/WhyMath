...
def _build_response_payload(
    ...
    *,
    ...
    `server_mastery`(slice 70)는 서버 L2 저장소의 *실제* 현재 숙달도(있을 때만). 주어지면 클라
    `bkt_mastery`(수치)를 *대체*해 hint 보수화·LTHC·solution 코칭 결정에 쓴다(신뢰 경계 제거).
    클라 명시 `mastery_level`(라벨)은 여전히 최우선. 숙달도 자체는 *결정에만* 쓰고 반환 payload엔
    싣지 않는다(`expected_answer`와 같은 비노출 계약·slice 69 경계).
    """
    ...
    # slice 70: 서버 L2 숙달도(server_mastery)가 있으면 클라 bkt_mastery(수치)를 *대체*한다
    # (신뢰 경계 제거 — 클라값은 위조·노후 가능). 단 클라 명시 mastery_level(라벨)은 최우선 유지.
...
async def _server_mastery_for(
    ...
    """학생의 *실제* 현재 숙달도를 서버 L2 저장소에서 조회 — 클라 `bkt_mastery` 대체(slice 70).

    `_expected_answer_for`(slice 64)와 동형의 서버 진실원천 조회: 문항의 대표 개념(PRIMARY·없으면
    TESTED)의 현재 숙달도(`ConceptMasteryHistory` 최신 측정)를 읽어 코칭 결정에 쓴다. 게이트
    (`l4_server_mastery_enabled`)가 off거나 `problem_id`가 None이면 조회를 *건너뛴다*(None —
    소비처가 클라값으로 폴백). 문항↔개념 매핑이 없거나(개념 None) 측정이 없으면(숙달 None)도
    None(graceful — 신규 문항·첫 학습도 안전). 반환값은 *결정에만* 쓰고 HTTP 응답엔 싣지 않는다
    (slice 69 노출 경계·우열 매기기 금지).
    """
...
async def create_session(
    ...
    # slice 70: 학생의 *실제* 숙달도를 서버 L2에서 조회해 클라 bkt_mastery 대신 코칭 결정에 쓴다
    # (신뢰 경계 제거). 게이트 off·문항/개념/측정 부재면 None → 클라값 폴백(하위호환).
...
async def append_turns(
    ...
    # slice 70: 멀티턴도 서버 L2 숙달도로 코칭(create_session과 동형·dialogue.problem_id 출처).
