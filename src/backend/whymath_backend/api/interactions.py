"""L5 상호작용 이벤트 수집 — 학생의 시각화 조작(슬라이더·3D 식) → AttemptEvent 영속. 슬라이스 96-F.

E(웹→Flutter JS 채널)로 흐른 학생 조작 신호를 백엔드에 적재해 L2 행동 분석(능동 탐구·메타인지
신호)의 입력으로 삼는다. 시각화 조작은 `EventType.그래프그리기`(그래프 그리기 활동)로 매핑하고,
세부(어떤 조작·파라미터·값)는 `event_data` JSONB에 담는다 — 전용 enum/마이그레이션 없이 기존
`attempt_event` 하이퍼테이블 재사용(7계층: L5만 세션 보유·하위 조합).

경계: 이 엔드포인트는 *수집 게이트*다 — 검증(스키마)·인증(동의)만 하고 그대로 적재한다.
attempt_id/problem_id(문제 컨텍스트) 연결은 후속(계산기 임베드가 특정 attempt에 아직 안 묶임).
"""
...
async def post_interaction(
    ...
    """인증 학생의 시각화 조작 이벤트를 `attempt_event`에 적재한다(그래프그리기 활동으로 분류).

    세부는 `event_data`(JSONB)에 담는다: `{interaction, payload, client_at}`. `get_session`은
    자동 commit하지 않으므로 핸들러가 명시적으로 commit한다.
    """
    session.add(
        AttemptEvent(
            ...
            event_data={"interaction": body.type, "payload": body.payload, "client_at": body.at},
        )
    )
