...
- SEC-16: `/outcome`은 `/study`가 그 학생에게 발급한 `session_id`만 받는다 — 소유권 검증은
  유일 outcome writer(`record_pedagogy_outcome`) 안쪽에서 일어나고, 이 라우터는 검증 실패를
  HTTP(403·409)로 번역만 한다.
...
# 설정 의존성 — SEC-16 세션↔사용자 바인딩(HMAC 키 = jwt_secret)에 필요(`_auth.py` 관례).
...
class OutcomeRequest(BaseModel):
    ...
    session_id: uuid.UUID = Field(
        description=(
            ...
            "검증한다(SEC-16) — 타인·위조 세션 id는 403으로 거부된다."
        )
    )
...
async def post_study_unit(
    ...
    # SEC-16: 처치 행 meta에 (session_id ↔ user) 키드 해시 바인딩이 함께 남는다 —
...
async def post_study_outcome(
    ...
    `session_id`는 `/study`가 *요청 학생 본인에게* 발급한 값이어야 한다(SEC-16 —
    ...
    try:
        ...
        # "그 session_id가 존재하는가"를 알려주는 존재 오라클이 된다(SEC-16 오라클 차단).
        raise HTTPException(
