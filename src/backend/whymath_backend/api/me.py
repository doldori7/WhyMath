...
async def recommend_next_problem(
    ...
    mode 무관 — 일관성 우선). `prioritize_weak_concepts`와 동시 지정 시 가중은 곱으로 합성.
    """
    attempt_stmt = (
        select(
    ...
    if mode == "suneung":
        ...
        suneung_weak_weights: list[float] | None = None
        if prioritize_weak_concepts and candidates:
            suneung_weak_weights = await _load_weak_concept_weights(
                session, user.user_id, [p.problem_id for p in candidates]
            )
        suneung_sib_weights: list[float] | None = None
        ...
        extra_weights = _combine_weights(suneung_weak_weights, suneung_sib_weights)
    ...
    weak_weights: list[float] | None = None
    if prioritize_weak_concepts and candidate_rows:
        weak_weights = await _load_weak_concept_weights(
            session, user.user_id, [pid for pid, _d, _b in candidate_rows]
        )
    sib_weights: list[float] | None = None
    ...
    weights = _combine_weights(weak_weights, sib_weights)
