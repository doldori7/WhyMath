...
# ops/스크립트가 직접 호출.
#
# SEC-13(2026-08-08): 이 엔드포인트는 도입 당시부터 "내부·집계 전용 원시 계측 표면"이라고
...
async def get_my_harness_metrics(
    ...
    **인가(SEC-13)**: `RequireContentAdmin` — `Role.CONTENT_ADMIN`이 아니면 403. 학생
    ...
    **PED-06 도달 관측**: 이 호출 자체를 `GrowthEvidenceReachCounters`가 센다(`GET /health/ready`
    ...
    검증 가능하게 만든다. 응답 필드 자체는 이번 태스크로 변경하지 않는다 — SEC-13은 *누가*
    호출할 수 있는지만 좁힌다.
    """
