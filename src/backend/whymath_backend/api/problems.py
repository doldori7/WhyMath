...
정답류 비노출(SEC-15 — `functional_security_audit_2026-08-08.md` M1): 무인증 GET의
...
ETag(SEC-15): 모든 ETag는 **공개 투영 기준**으로 계산한다 — 전체 스키마 해시를 무인증
...
async def create_problem(
    ...
    응답에 ETag(공개 투영 기준 — 모듈 docstring SEC-15 오라클 항목)를 실어 이후 조건부
...
async def read_problem(
    ...
    """UUID로 문제 단건 조회 — 없으면 404. **무인증 공개 라우트 → 공개 투영**(SEC-15).
...
async def list_problems(
    ...
    **무인증 공개 라우트 → 공개 투영**(SEC-15 — 정답류 키 부재). `subject`를 주면 해당
...
async def list_problem_steps(problem_id: uuid.UUID, session: SessionDep) -> list[PublicProblemStep]:
    ...
    **무인증 공개 라우트 → 공개 투영**(SEC-15): `expected_answer`(단계 정답)·
    `common_mistakes`(힌트류)는 키 자체가 없다 — 코치(L4)가 서버 내부에서만 쓴다.
...
async def patch_problem(
    ...
    새 ETag를 싣는다. ETag는 공개 투영 기준(모듈 docstring SEC-15 — GET이 공개 투영
...
async def delete_problem(
    ...
    비교 기준은 공개 투영 ETag·모듈 docstring SEC-15).
    """
    existing = await session.get(Problem, problem_id)
    if existing is None:
        raise HTTPException(
