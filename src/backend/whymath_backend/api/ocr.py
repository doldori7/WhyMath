...
# SEC-17: 인증 DoS(메모리 고갈) 방어. ConsentedUser 게이트라 무인증은 아니나, 인증된(탈취) 계정이
# 대용량 파일을 반복 업로드하거나 /pages로 페이지 수 상한(20장)까지 동시 업로드해 메모리 고갈을
# 일으킬 수 있다(정본: docs/reviews/functional_security_audit_2026-08-08.md M4). 페이지 수 캡만으론
# 단건 대용량·합계 증폭을 막지 못하므로 바이트 상한·MIME 허용목록을 추가한다(verify.py의
# _MAX_EXPR_LEN 선례 — 캡 상수 중앙화 + 초과 시 명확한 4xx).
...
async def _read_and_validate_image(image: UploadFile, *, max_bytes: int) -> bytes:
    """업로드 이미지 1장을 읽어 MIME·크기를 검증한 뒤 바이트를 반환한다(SEC-17).
...
async def post_ocr(
    ...
    SEC-17: 업로드는 `_MAX_IMAGE_BYTES`(10MB) 이하·MIME이 `_ALLOWED_IMAGE_CONTENT_TYPES`
...
async def post_ocr_pages(
    ...
    SEC-17: 각 페이지는 `_MAX_IMAGE_BYTES`(10MB)·MIME 허용목록을 개별 통과해야 한다(위반 시
    413/415). 페이지 수 캡(20장)만으로는 20 × 10MB = 200MB까지 열려 증폭 방어가 안 되므로,
    합계가 `_MAX_PAGES_TOTAL_BYTES`(50MB)를 넘는 순간(마지막 페이지까지 기다리지 않고) 즉시
    413으로 조기 중단한다(인증 DoS 방어).
