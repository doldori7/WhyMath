...
현 좌석(S0 슬라이스 1): Tier1+2 판정 combiner(`verdict.py`)만. Tier1 수치 답 검산은
`l3/verify_answer.py`, Tier2 기호 단계 동치는 `l3/verify_solution.py`(둘 다 기존/신규 좌석)이고,
본 패키지는 그 둘의 결과를 §4 판정 규칙으로 결합해 최종 등급(verified/unverified/failed)을 낸다.

범위 밖(후속): 솔버 루프·도구 8종·`solution_nodes`/저장소 스키마·자기 진화·PRM·베이스라인
풀이율 측정(S1+·§9 로드맵).
"""
