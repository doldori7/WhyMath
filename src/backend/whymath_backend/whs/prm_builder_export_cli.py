...
(비루트 step만·VERIFIED→good·FAILED→bad·PENDING/UNVERIFIED 배제·정직 회계) → `iter_prm_jsonl`
(레코드/줄). 출력 분리:
...
사용법:
    python -m whymath_backend.whs.prm_builder_export_cli > prm.jsonl
    python -m whymath_backend.whs.prm_builder_export_cli --stream > prm.jsonl
...
# export 좌석 — () → PrmDataset. 기본은 실 DB(전 문제 노드 평탄 조회·순수 빌더 호출), 테스트는
# 합성 데이터셋을 주입해 DB 없이 CLI 배선(인자 파싱·JSONL stdout·요약 stderr·종료 코드)을 검증한다.
ExportFn = Callable[[], Coroutine[Any, Any, PrmDataset]]
...
async def _default_export_fn() -> PrmDataset:  # pragma: no cover — 실 DB(integration)
    """기본 export — 세션 1개로 전 문제 노드를 평탄 조회해 PRM 데이터셋으로 변환한다(읽기 전용).

    `get_all_nodes`(problem_id별 인접·결정적)→`build_prm_dataset`(순수). `parent_id` 체인은 같은
    문제 트리 내부에서만 연결되어 전 문제 노드를 한 리스트로 넘겨도 크로스-문제 오염이 0이다.
    읽기뿐이라 commit 없음.
    """
...
async def _run_stream(stream_fn: StreamFn, node_loader: NodeLoader) -> PrmStreamAccounting:
    """스트리밍 경로 본체 — problem_id를 흘리며 문제별 JSONL을 stdout에 print, 회계를 누적해 반환.

    `stream_fn`이 흘린 problem_id마다 `node_loader`로 그 문제 트리를 로드해 `build_prm_dataset`로
    빌드하고 `iter_prm_jsonl`을 한 줄씩 print한다 — 전 노드 적재 없음(한 문제 트리씩). 회계는
    `PrmStreamAccounting.merge`로 누적. 소진 후 누산기를 돌려주면 호출자가 `summary()`로 stderr
    요약을 낸다.
    """
...
def main(
    ...
    *,
    ...
    가드). `export_fn`/`stream_fn`/`node_loader`는 테스트 주입 좌석(기본 실 DB). 종료 코드 0(노드
    ...
    if args.stream:
        accounting = asyncio.run(_run_stream(stream_fn, node_loader))
