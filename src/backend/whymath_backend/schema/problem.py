...
컨벤션(`l3/models.py` 답습):
  ...
  - 공유 베이스 클래스 없음(각 모델 독립) — 단 하나의 예외가 SEC-15 공개 투영이다:
    `PublicProblem`(공개·학생 대면 기반) ⊂ `Problem`(내부 정본·정답류 6필드 추가),
    `PublicProblemStep` ⊂ `ProblemStep`(기대답·흔한실수 2필드 추가). 방향이 허용목록을
    만든다(`schema/assessment.py` `StudentAssessment` ⊂ `Assessment`·ASM-07 선례 동형).
...
# ──────────────────────────────────────────────────────────────────────────
# SEC-15 — 공개(무인증·학생 대면) 응답에서 **키 자체가 없어야 하는** 정답류 필드
...
#: `expected_answer`는 단계별 정답이고, `common_mistakes`는 [{"error","hint"}] 힌트류라
#: 사전 일괄 노출이 Polya 단계별 제공 원칙·오개념 reactive retrieval(preload 금지)과
#: 상충한다 — 코치(L4)가 서버 내부에서 반응적으로만 쓴다.
...
# ──────────────────────────────────────────────────────────────────────────
# 공개 투영: PublicProblem (SEC-15 — 무인증·학생 대면 응답 정본)
# ──────────────────────────────────────────────────────────────────────────
class PublicProblem(BaseModel):
    """문제의 **공개(무인증·학생 대면) 응답 정본** — 정답류 필드에 자리가 없다(SEC-15).
    ...
    **사고 경위**(`docs/reviews/functional_security_audit_2026-08-08.md` M1): SEC-07
    ...
    #  SEC-15 `PUBLIC_HIDDEN_ANSWER_FIELDS` 참조)
    ...
    #  Problem 서브클래스로 이동, SEC-15 `PUBLIC_HIDDEN_ANSWER_FIELDS` 참조)
...
class Problem(PublicProblem):
    ...
    **내부·영속 정본**이다. 무인증·학생 대면 응답에는 이 모델을 쓰지 않는다 —
    `PublicProblem`을 쓴다(SEC-15 · 정답류 6필드는 이 서브클래스만 가진다).
    ...
    법적 교정 불변식(MEMORY 2026-05-28, `@model_validator(mode="after")`로 강제):
      ...
      *왜*: 검정 교과서·평가원·EBS 본문의 복제·영리이용은 저작권법 §32 단서·§136·
      ...
      Problem 본문 규칙만 강제한다. SEC-15 정답류 비노출은 이 저작권 축과 *독립* —
      저작권 축은 "무엇을 저장하면 안 되는가", SEC-15는 "저장한 것 중 무엇을 공개
      응답에 싣지 않는가"다.)
    ...
    # ===== 정답·풀이(내부 정본 전용 — SEC-15 PUBLIC_HIDDEN_ANSWER_FIELDS) =====
    ...
    # 오답 선지의 *여집합*이 정답 선지를 드러내므로 정답류로 분류(SEC-15 감사 M1).
...
# ──────────────────────────────────────────────────────────────────────────
# 공개 투영: PublicProblemStep (SEC-15) + 보조: ProblemStep (§3.2 problem_step)
# ──────────────────────────────────────────────────────────────────────────
class PublicProblemStep(BaseModel):
    """풀이 단계의 **공개(무인증·학생 대면) 응답 정본** — 기대답·힌트류에 자리가 없다.

    `ProblemStep`(내부 정본)이 이 기반에 `PUBLIC_HIDDEN_STEP_ANSWER_FIELDS` 2필드
    (`expected_answer`·`common_mistakes`)를 *더한다*(`PublicProblem` ⊂ `Problem`과 동일
    방향·SEC-15). `socratic_prompt`는 답을 제공하지 않는 교수학 발문이라 공개 유지.
    """
    ...
    def from_step(cls, step: ProblemStep) -> PublicProblemStep:
        """내부 정본 → 공개 투영. 기대답·흔한실수는 애초에 대상 모델에 자리가 없다."""
...
class ProblemStep(PublicProblemStep):
    ...
    않는다 — `PublicProblemStep`을 쓴다(SEC-15).
    """
