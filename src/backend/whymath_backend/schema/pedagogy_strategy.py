...
설계 정본: `docs/architecture/04f_pedagogy_strategy_catalog.md` §3(전략 카탈로그 — 필드 소비처
...
⚠️ 용어 3축 주의(04f §1): 이 카탈로그의 "전략"은 **교수전략**(`PedagogyStrategy` — 교사가 어떻게
가르칠까)이다. 학생의 문제 공략 전략(`StrategyNode`·`strategy.analogy`=유추)과 같은 영단어·다른
개념이므로 혼동 금지.

────────────────────────────────────────────────────────────────────────────
필드 원칙 — 소비처를 지정 못 하는 필드는 착시·금지 (04f §3.2)
────────────────────────────────────────────────────────────────────────────
...
────────────────────────────────────────────────────────────────────────────
제거 필드 3종 — 정본 이중화 방지 (04f §3.3 — 부재를 테스트로 동결)
────────────────────────────────────────────────────────────────────────────
외부 프레임워크의 다음 3계열은 *의도적으로 없다*(재제안 방지 — 04f §8 영구 기록과 이중 방어):
...
# ──────────────────────────────────────────────────────────────────────────
# 학교급 4종 — `target_grade_bands` 항목의 통제 어휘. 소비처는 select() 후보 필터(PED-18 §4·
...
# 와 동일 어휘 재사용(04f §3.2 — 새 어휘 축 신설 금지).
...
class PedagogyStrategyCard(BaseModel):
    ...
    9필드 전부에 소비처가 지정되어 있다(04f §3.2 소비처 지정표 — 각 Field description 병기).
    ...
    제거 필드 3종(AI 추천 점수·기계 판정용 금지조건·선행/후속·페이딩)의 부재 근거는 04f §3.3
    ...
    target_grade_bands: list[str] = Field(
        ...,
        description="적용 학교급(GRADE_BAND_VOCAB 폐쇄 4종) — 소비처: select() 후보 필터(PED-18)",
    )
    difficulty_range: list[str] = Field(
        ...,
        description=(
            "적합 문항 난이도(DIFFICULTY_VOCAB 폐쇄 3종) — 소비처: select() 후보 필터(PED-18)"
        ),
    )
    ...
    usage_notes: str = Field(
        ...
        description=(
            ...
            "(기계 판정 정본은 팩 forbidden_modes+gate() 2축·04f §3.3)"
        ),
    )
...
# ──────────────────────────────────────────────────────────────────────────
# 스키마 동결 (PED-18 — 필드 추가는 "소비처 실재 증명" 전제)
# ──────────────────────────────────────────────────────────────────────────
# `PedagogyStrategyCard`의 9필드와 폐쇄 어휘 3종(GRADE_BAND_VOCAB·DIFFICULTY_VOCAB·
# ERROR_TYPE_VOCAB)·제거 필드 3계열의 *부재*는 PED-18에서 동결됐다. 변경 시
