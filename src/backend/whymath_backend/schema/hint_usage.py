...
설계 판단(근거 병기):
  - `hint_id` — **느슨참조**(FK 아님). 실측(2026-08-30): 힌트 정본 테이블이 레포에 없다
    ...
    **의미 확정(ARCH-39 · 2026-09-06 · 판정 기준 main 3f2b39c1)**: 이 컬럼은 *미래의 `hints`
    테이블을 향한 전방참조가 아니다*. Hint 엔티티는 **영구 부재로 판정**됐다
    (`canonical_entity_model_v1.md` §3-B) — 좌석이 언젠가 생기는 것이 아니라 만들지 않기로
    했으므로, 여기 담길 수 있는 것은 위에 적은 *실재하는 식별자 경로*뿐이다. 컬럼은 유지한다
    (nullable·비용 0·그 경로가 열리면 즉시 쓸 수 있다). 다만 **현재 writer는 0건**이다
    (서빙 코드 전수 — 정의·주석 외 대입 0). 즉 이 필드는 현재 *구조만 있고 흐르지 않는다* —
    "작동 신호 없는 알고리즘 부착 금지"(CLAUDE.md)에 따라 그 사실을 여기 명시해 둔다.
