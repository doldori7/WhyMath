...
늘리지 않았다"까지만 말한다. 의미적 위반은 프로브(`EOS-92`)가 필드별 채움 표로 사람이
판정하며, Core가 불투명 페이로드를 해석하는 코드는 `eos_core_adapter_boundary_scan.py`
(정적 import 축)로도 잡히지 않는다 — 그 축의 기계 집행은 **현재 없다**(있는 척 금지).
