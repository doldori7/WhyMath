...
`eos_core_adapter_boundary_scan.py`(정적 import 축)로도 잡히지 않는다 — 그 축의 기계 집행은
**현재 없다**(있는 척 금지).

**이 축의 검증 책임은 `EOS-92`에 있다.** 기계가 못 보는 갭에 소유자가 없으면 "알려진 결함·
소유자 없음" 상태가 된다. `EOS-92`는 이미 "채우지 못하거나 **의미가 뒤틀리는** 필드를 가려내는"
태스크로 정의돼 있으므로, 의미적 확장의 검출은 그 프로브의 필드별 채움 표가 **보상 통제**
(compensating control)로 담당한다. 새 장치를 만들지 않는다 — 지목만 한다.
