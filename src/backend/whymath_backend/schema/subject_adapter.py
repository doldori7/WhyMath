...
### 프로브 결과 처리 규칙 2조 (Kiki 지시 2026-09-05 · 협상 불가)
...
두 조항의 귀결: **이 계약은 프로브를 거치며 줄어들 수만 있다(단조 축소·monotonic shrink).**
확장 경로가 없으므로 "Physics를 지원하려다 Core가 비대해지는" 실패 방식이 구조적으로 닫힌다.
...
집행: `tests/backend/schema/test_subject_adapter_two_tier_contract.py` — 상태 라벨·규칙 2조·
필드 집합을 기계가 잡는다. **필드 확장은 무조건 RED**, 축소는 강등 대장(`DEMOTED_FIELDS`)
경유로만 통과한다. 프로브 없이 이 상태를 되돌리면 RED.
