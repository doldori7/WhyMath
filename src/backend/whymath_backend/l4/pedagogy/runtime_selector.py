...
────────────────────────────────────────────────────────────────────────────
카탈로그 후보 필터 (PED-18 — 정본 `docs/architecture/04f_pedagogy_strategy_catalog.md` §4)
────────────────────────────────────────────────────────────────────────────
교수전략 카탈로그(PED-18·`strategy_registry`)의 적합성 필드를 `select()`가 **후보 필터**로
...
# reason_code — 카탈로그 필터 폴백 사유(04f §4 불변식 ① "조용한 실패 아님" — 구조화 로그 필드).
...
def grade_to_band(grade: int | None) -> str | None:
    ...
    정본: 04f §4(카탈로그 후보 필터의 grade 신호 — "생산자 먼저" 원칙). 생산자는
...
def narrow_candidates(
    ...
    *,
    ...
    """카탈로그 적합성으로 후보 집합을 **좁히기만** 하는 순수 함수(04f §4의 `narrow`).
...
def _rule_table(
    ...
    전략이 이긴다 — 우선순위 순서 자체는 그대로다(04f §4 "R1~R5 우선순위 그대로").

    R2 정밀화(04f §4 v2)는 **정직한 공백으로 보류**한다: 설계는 오개념 가설의 `error_type`을
...
def select(
    ...
    *,
    ...
    카탈로그 후보 필터(PED-18 — 04f §4·모듈 docstring): `pedagogy_catalog_filter_enabled`
...
def gate(
    ...
    *시퀀스* 성질(페이딩·표현 전환 균형)이라 이 시점 판정이 불가능하다 — `mode_guard`가 같은 이유로
    ...
    ⚠️ **카탈로그 입력 금지(04f §4 불변식 ② — PED-18 동결)**: 이 함수는 교수전략 카탈로그
    ...
    동결한다 — 카탈로그 축을 추가하려면 그 테스트와 04f §4 개정이 선행이다.
    """
...
def decide(
    ...
    *,
    ...
    PED-18)과 ② 해당 교수법 팩 조회(게이트 축①)에 함께 쓰인다. 팩을 못 찾으면 축①은 생략되고
    축②만 적용된다(`gate()` fail-safe 참조). `difficulty`(상/중/하)는 select 필터 전용이며
    게이트에는 들어가지 않는다 — 카탈로그·난이도는 *효과* 축이지 *허용* 축이 아니다(04f §4
