...
_ALGEBRA: tuple[Misconception, ...] = (
    ...
    Misconception(
        ...
        # 변별의 핵심은 문자열 앞머리 부정 전방탐색 `\A(?!.*x=0)`이다: 올바른 풀이는 어디선가
        # 반드시 `x=0`을 적으므로(그게 손실되지 않은 근이다) 전체 문자열에 `x=0`이 있으면
        # 매치되지 않는다 — 즉 이 정규식은 "x=2를 얻었다"가 아니라 **"x=0을 끝내
        # 적지 않았다"**를 본다. 기호형 `ax2=bx`는 `\d`가 글자에 미매치라 disjoint(기존
        # substring 경로 보존)이고, `(?!\d)`가 `x=20`의 부분 일치를 막는다.
        regex_signals=(r"\A(?!.*x=0).*x2=(?P<b>\d+)x.*x=(?P=b)(?!\d)",),
    ),
    Misconception(
    ...
    Misconception(
        ...
        regex_signals=(r"\(x-(?P<a>\d+)\)=0.*x=-(?P=a)(?!\d)",),
    ),
