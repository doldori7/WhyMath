...
class Settings(BaseSettings):
    ...
    pedagogy_pack_prompt_enabled: bool = Field(
        ...
        description=(
            ...
            "주의(2026-08-10 정정): forbidden_modes *문면* 가드(`mode_guard`)는 이 플래그와 "
            "무관하게 코치 응답 경로에 배선돼 있지 않다 — 오프라인 결함주입 측정 전용이며 집행 "
            "지점 배선 여부는 PED-16 판정(dsl_integration_gap_review §2-⑤). 사전 가드는 "
            "`runtime_selector` supply 경로에 별도 배선. "
