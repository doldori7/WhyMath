...
class Settings(BaseSettings):
    ...
    pedagogy_catalog_filter_enabled: bool = Field(
        default=False,
        description=(
            "교수전략 카탈로그(PED-18)의 적합성 필드(target_grade_bands·difficulty_range·"
            "target_k_types)를 `runtime_selector.select()`의 **후보 필터**로 소비할지"
            "(정본 docs/architecture/04f_pedagogy_strategy_catalog.md §4). **False(기본·옵트인)** "
            ...
            "카탈로그를 읽지 않는다(효과≤허용 우회 차단·04f §4 불변식 ②·테스트 동결). "
    ...
    pedagogy_strategy_card_enabled: bool = Field(
        ...
        description=(
            ...
            "(name_ko·description·research_basis 요약 1줄 — attention 절약)를 얹을지(PED-18 — "
            "정본 04f §3.2 소비처 지정표). **False(기본·옵트인)** — OFF면 카탈로그 미조회·"
