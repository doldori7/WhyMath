...
아직 main에 없다(S3-32는 PR #738/#739 두 구현이 경합 중이라 착륙 미정). 이 마이그레이션은
`misconception_catalog`만 FK로 참조하는 신규 테이블 생성이라 `dialogue` 컬럼 추가와 순서
의존이 전혀 없으므로, main의 실제 head(`090d254a5d43`)로 재지정해 독립 착륙시킨다.
"""
