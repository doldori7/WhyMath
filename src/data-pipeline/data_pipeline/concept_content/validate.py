"""K-12 개념 콘텐츠 코퍼스 검증 — `concept_content_university/validate.py` 리포트 패턴 미러.

집합 invariant:
  - **code_unique**(error) — 개념ID 유일.
  - **name_present**(error) — 개념명 비공란.
  - **content_present**(warning) — 핵심 콘텐츠(은유·오개념·정식정의·허용표현) 누락 보고(검수 큐).
  - **subject_count**(warning) — 과목 수(기대 17).
...
class ContentValidationReport:
    ...
    def summary(self) -> str:
        ...
        lines = [
            f"K-12 콘텐츠 검증: content={self.content_count} flashcards={self.flashcard_count} "
...
def validate_content(
    items: Sequence[ConceptContent],
) -> ContentValidationReport:
    """K-12 개념 콘텐츠 집합 검증. 반환=리포트(error 0이면 유효)."""
    ...
    for code, cnt in Counter(c.code for c in items).items():
        if cnt > 1:
            report.issues.append((_ERROR, "code_unique", f"개념ID 중복 {cnt}회: {code}"))
    for c in items:
        if not c.name:
            report.issues.append((_ERROR, "name_present", f"개념명 공란: {c.code}"))
        missing = [
            label
            for label, val in (
                ("은유", c.metaphor),
