"""문제유형 그래프 CLI — transform-v1(problem_types.jsonl → graph.json).

사용:
    python -m data_pipeline.problem_type_graph transform-v1 \
        --source data/corpus/problem_type_graph_v1/problem_types.jsonl \
        --output-dir data/corpus/problem_type_graph_v1

transform-v1: 자체작성 `problem_types.jsonl`을 정형화→검증한 뒤 `graph.json`(문제유형 노드)과
`_provenance.json`(source sha256·카운트·결정성)을 저장한다. skill_graph transform-v1 출력 미러
(문제유형은 PG 프로젝션만·Neo4j·엣지 없음 — 연결은 참조 키 behavior_skills로 노드 내장).
...
from data_pipeline.problem_type_graph.models import SOURCE_CITATION
...
app = typer.Typer(
    name="whymath-problem-type-graph",
    help="문제유형 그래프 — problem_types.jsonl(자체작성 택소노미) 정형화·검증.",
...
@app.callback()
def _main() -> None:
    """문제유형 그래프 파이프라인 CLI(서브커맨드: transform-v1).
...
def _read_jsonl(path: Path) -> list[dict[str, object]]:
    """jsonl → dict 목록(빈 줄 무시). 각 줄은 한 문제유형 레코드."""
...
def transform_v1(
    source: Annotated[
        Path,
        typer.Option("--source", "-s", help="자체작성 problem_types.jsonl 경로."),
    ] = Path("data/corpus/problem_type_graph_v1/problem_types.jsonl"),
    ...
    """problem_types.jsonl → 정형화 + 검증 + (선택) 코퍼스 저장."""
    _setup_logging(verbose)
    print(SOURCE_CITATION)
    print()

    if not source.exists():
        typer.echo(f"[!] problem_types.jsonl 없음: {source}", err=True)
...
def _write_graph_json(result: TransformResult, path: Path) -> None:
    """정형화 산출 → graph.json(문제유형 노드 배열·엣지 없음 — 연결은 참조 키로 노드에 내장)."""
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "source_citation": SOURCE_CITATION,
        "problem_types": [p.model_dump() for p in result.problem_types],
    }
...
def _write_provenance(
    ...
    report: ProblemTypeValidationReport,
    source: Path,
    source_sha: str,
    path: Path,
) -> None:
