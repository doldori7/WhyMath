"""스킬 그래프 CLI — transform-v1(skills.jsonl → graph.json).

사용:
    python -m data_pipeline.skill_graph transform-v1 \
        --source data/corpus/skill_graph_v1/skills.jsonl \
        --output-dir data/corpus/skill_graph_v1

transform-v1: 자체작성 `skills.jsonl`을 정형화→검증한 뒤 `graph.json`(스킬 노드)과
`_provenance.json`(source sha256·카운트·결정성)을 저장한다. atom_graph transform-v1 출력을
미러하되, 입력이 xlsx가 아닌 자체 jsonl이라 extract·Neo4j load 단계가 없다(스킬은 PG 프로젝션만).
...
from data_pipeline.skill_graph.models import SOURCE_CITATION
from data_pipeline.skill_graph.transform import TransformResult, transform_skills
from data_pipeline.skill_graph.validate import SkillValidationReport, validate_skills

app = typer.Typer(
    name="whymath-skill-graph",
    help="스킬 그래프 — skills.jsonl(자체작성 택소노미) 정형화·검증.",
...
@app.callback()
def _main() -> None:
    """스킬 그래프 파이프라인 CLI(서브커맨드: transform-v1).

    Typer가 단일 커맨드를 콜백으로 접지 않도록 명시 콜백을 둔다(atom_graph CLI 미러).
    """
...
def _read_jsonl(path: Path) -> list[dict[str, object]]:
    """jsonl → dict 목록(빈 줄 무시). 각 줄은 한 스킬 레코드."""
...
def transform_v1(
    source: Annotated[
        Path,
        typer.Option("--source", "-s", help="자체작성 skills.jsonl 경로."),
    ] = Path("data/corpus/skill_graph_v1/skills.jsonl"),
    ...
    """skills.jsonl → 정형화 + 검증 + (선택) 코퍼스 저장."""
    _setup_logging(verbose)
    print(SOURCE_CITATION)
    print()

    if not source.exists():
        typer.echo(f"[!] skills.jsonl 없음: {source}", err=True)
...
def _write_graph_json(result: TransformResult, path: Path) -> None:
    """정형화 산출 → graph.json(스킬 노드 배열·엣지 없음 — 선수는 참조 키로 노드에 내장)."""
