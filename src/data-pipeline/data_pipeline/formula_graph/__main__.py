"""수식 그래프 CLI — transform-v1(formulas.jsonl → graph.json).

사용:
    python -m data_pipeline.formula_graph transform-v1 \
        --source data/corpus/formula_graph_v1/formulas.jsonl \
        --output-dir data/corpus/formula_graph_v1

transform-v1: 자체작성 `formulas.jsonl`을 정형화→검증한 뒤 `graph.json`(수식 노드)과
`_provenance.json`(source sha256·카운트·결정성)을 저장한다. problem_type_graph transform-v1 출력
미러(수식은 PG 프로젝션만·Neo4j·엣지 없음 — canonical 노드만·연결은 참조 키[formula_refs·Phase 5b]).
...
from data_pipeline.formula_graph.models import SOURCE_CITATION
...
app = typer.Typer(
    name="whymath-formula-graph",
    help="수식 그래프 — formulas.jsonl(자체작성 canonical 수식) 정형화·검증.",
...
@app.callback()
def _main() -> None:
    """수식 그래프 파이프라인 CLI(서브커맨드: transform-v1).

    Typer가 단일 커맨드를 콜백으로 접지 않도록 명시 콜백을 둔다(problem_type_graph CLI 미러).
    """
...
def _read_jsonl(path: Path) -> list[dict[str, object]]:
    """jsonl → dict 목록(빈 줄 무시). 각 줄은 한 수식 레코드."""
...
def transform_v1(
    source: Annotated[
        Path,
        typer.Option("--source", "-s", help="자체작성 formulas.jsonl 경로."),
    ] = Path("data/corpus/formula_graph_v1/formulas.jsonl"),
    ...
    """formulas.jsonl → 정형화 + 검증 + (선택) 코퍼스 저장."""
    _setup_logging(verbose)
    print(SOURCE_CITATION)
    print()

    if not source.exists():
        typer.echo(f"[!] formulas.jsonl 없음: {source}", err=True)
...
def _write_graph_json(result: TransformResult, path: Path) -> None:
    """정형화 산출 → graph.json(수식 노드 배열·엣지 없음 — canonical 노드만·연결은 참조 키[5b])."""
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
        "source_citation": SOURCE_CITATION,
        "formulas": [f.model_dump() for f in result.formulas],
    }
