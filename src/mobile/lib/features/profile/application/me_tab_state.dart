// 나 탭 — 학습 경로·진단 결과 섹션 상태 (PATH-05).
//
// 경계(CLAUDE.md): 순수 표현 상태다. 진단(BKT/IRT)·학습 경로 위상정렬 판정은 전부 서버(L2)가
// 내리고, 이 상태는 응답 구조를 그대로 보관할 뿐이다(표현≠의미·수학 로직 클라 미구현).
//
// 정직 원칙(CLAUDE.md "확실하지 않을 때 자신 있게 말함 금지"): 401(미인증)·has_cycle(그래프
// 순환)·빈 경로(steps==0)는 각각 다른 의미의 *정상/이상* 신호라 하나의 "오류" 문구로 뭉개지
// 않는다. [SectionStatus]가 그 구분을 상태 트리에 그대로 반영한다 — unauthenticated는 error와
// 별개 값이고, has_cycle·빈 steps는 loaded 상태의 데이터([LearningPath])에서 그대로 읽는다.
...
abstract class MeTabState with _$MeTabState {
  ...
  /// 두 섹션 중 하나라도 조회 중인지(진입 시 전체 로딩 인디케이터 판단용).
  bool get isLoading =>
