...
# 계층별 바닥선(%) — 실측 ratchet floor(회귀 방지). 괄호는 testing.md 선언 지향치.
# 게이트 값은 CI 실측 이상으로만 상향한다(점진 강화). 실측 baseline은 2026-07-25 로컬 측정
# (data_pipeline 미설치로 l1은 CI에서 더 높음 → l1 바닥선은 보수적).
LAYER_FLOORS: dict[str, float] = {
    "l1": 50.0,  # 데이터(지향 80%)
    "l2": 50.0,  # ML 모델(지향 80%)
    "l3": 50.0,  # LLM 통합(지향 70%)
    "l4": 50.0,  # 도메인 로직(지향 90%)
    "api": 50.0,  # API/L5(지향 80%)
}
