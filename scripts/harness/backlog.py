...
def _fail_on_reason_feedback(
    backlog: object, task_id: str, before: set[tuple[str, str]], hint: str = ""
) -> int:
    ...
    return _fail(
        f"{task_id}: 이 명령이 새 의존 선언을 만든다 — 사유에서 선행 어구와 태스크 ID가 "
        "한 문장에 함께 오지 않게 고쳐 다시 실행하라(notes는 append 전용이라 기록된 뒤에는 "
        "되돌릴 수 없다)." + (f" {hint}" if hint else "")
    )
...
def cmd_amend(root: Path, args: argparse.Namespace) -> int:
    ...
    **여전히 열지 않는 것**: 태스크 삭제·ID 변경(rename)·acceptance 항목 개별 제거. 대장
    손편집의 우회 표면이 되거나 ID 계보를 끊는다. rename 미구현 사유는
    `docs/standards/build_harness.md` §7 정정 경로 표 참조.
    """
    backlog, _ = _load(root)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "amend",
        help="등재된 태스크의 acceptance·게이트·트랙·의존·notes 정정 (HARN-24+49+52+67)",
    )
    p.add_argument("id")
    p.add_argument(
        "--acceptance",
